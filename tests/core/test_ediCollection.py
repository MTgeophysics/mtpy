...
class TestUtilities(TestCase):
    def test_is_num_in_seq(self):
        self.assertTrue(is_num_in_seq(3, [1.0, 2.0, 3.0]))
        self.assertTrue(is_num_in_seq(2, [1.0, 2.19999999, 3], atol=0.2))
        self.assertTrue(is_num_in_seq(2, [1.811111, 4, 3], atol=0.2))
        ...
        self.assertFalse(is_num_in_seq(1, [0, 1.10000001, 2], atol=0.1))
        self.assertFalse(is_num_in_seq(1, [0, 0.89999999, 2], atol=0.1))


class _BaseTest(object):
    def setUp(self):
        self.edi_files = glob.glob(
            os.path.normpath(os.path.abspath(os.path.join(self.edi_path, "*.edi")))
        )
    ...
    def test_init(self):
        ...
        self.assertTrue(
            isinstance(self.edi_collection.all_frequencies, list)
        )  # not none and none-empty
    ...
    def test_get_periods_by_stats(self):
        ...
        for percent in percentages:
            new_periods = set(
                self.edi_collection.get_periods_by_stats(percentage=percent)
            )
    ...
    def test_get_tensor_tippers(self):
        ...
        pt_dict_list_no_interp = self.edi_collection.get_phase_tensor_tippers(
            p, interpolate=False
        )
    ...
    def test_export_edi_files(self):
        path3 = make_temp_dir(
            self.__class__.__name__ + "_export_edi_files_no_interp",
            base_dir=self._temp_dir,
        )
        path4 = make_temp_dir(
            self.__class__.__name__ + "_export_edi_files_interp",
            base_dir=self._temp_dir,
        )
...
for edi_path in edi_paths:
    if os.path.isdir(edi_path):
        ...
        globals()[cls_name] = type(
            cls_name, (_TestFromFile, unittest.TestCase), {"edi_path": edi_path}
        )
        cls_name = "TestEdiCollectionFromMTObj_%s" % (os.path.basename(edi_path))
        globals()[cls_name] = type(
            cls_name, (_TestFromMTObj, unittest.TestCase), {"edi_path": edi_path}
        )
