...
class TestModEMData(unittest.TestCase):
    ...
    """
    def setUp(self):
        self.data = Data(edi_list=list(EDI_DATA_DIR.glob("*.edi")),
    ...
    def test_station_list(self):
        ...
        self.assertListEqual(station_list, 
                             self.data.station_locations.station.tolist())
        
    def test_period_range(self):
        self.assertEqual(20, self.data.period_list.size)
        
    def test_add_station(self):
        new_data, new_mt_dict = self.data.add_station(fn=list(EDI_DATA_DIR2.glob("*.edi"))[0:2])
