...
class Test_VelocityModel(TestCase):
    def test_max_depth_reached(self):
        ...
        # end for
        maxDepths_all = numpy.max(v.getDepth(None, ts)) # using mean velocity across the profile
        ...
        self.assertGreater(mean_5nn, mean, 'Mean should be larger because of lateral averaging')
        ...
        self.assertGreater(mean_10nn, mean_5nn, 'Mean should be larger because of lateral averaging')
        self.assertLess(std_10nn, std_5nn, 'Std. should be smaller because of lateral averaging')

        self.assertTrue(numpy.allclose(maxDepths_all, mean),
