...
def plot_pt(edi_file_list, freq, save_path=None):
    ...
    # Try different size to find a suitable value for your case. as a guidance: 1 degree=100KM
    ellipse_dict = {'size': 0.2, 'colorby': 'phimin', 'range': (0, 90, 1), 'cmap': 'mt_bl2gr2rd'}
    ...
    ptObj = PlotPhaseTensorMaps(fn_list=edi_file_list,
                                ...
                                xpad=0.4,  # plot margin; change according to lat-lon of your dataset
                                ...
                                # fig_size=(6, 5),
                                save_fn=save_path)  # fig_dpi=300 default is OK. Higher dpi may distort figure)

    # 3) do the plot and save figure - if the param save_path provided
    path2figure = ptObj.plot(save_path=save_path)
