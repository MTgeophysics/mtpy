...
ptmap = PlotPTMaps(
    data_fn=op.join(workdir, "ModEM_Data_noise10inv.dat"),
    resp_fn=op.join(
        workdir, "Modular_MPI_NLCG_NLCG_015.dat"
    ),  # comment out to plot data only
    # cb_pt_pad=0.1,
    ellipse_dict={
        "size": 40,
        "ellipse_range": [0, 90],
...
for period_index in range(20):  # customise which periods to plot
    ptmap.plot(
        period=period_index,  # index of period to plot
