...
elst = [op.join(epath,ef) for ef in os.listdir(epath) if ef.endswith('.edi')] #make a list of EDIs
slst=[edi[0:-4] for edi in os.listdir(epath) if edi.find('.edi')>0] #make a list of MT stations
gstrike=-72
data = o2d.Data(edi_path=epath, model_mode='1',station_list=slst, 
...
elevation_projected_cubic=scipy.interpolate.griddata((eastings,northings),elev,(elev_points_easts,elev_points_norths),method='cubic')
...
with open(mare_fn,'w') as output:
   ...
   receiver_lst = np.vstack([zl,loc_at_sites]+[elevation_at_sites]+[zl]*4+[int(solve_statics)]+[slst]).T
   formats = ['{:>14.2f}','{:>13.2f}','{:>13.2f}','{:>8.2f}',
              '{:>8.2f}','{:>8.2f}','{:>8.2f}','{:>12.0f}','{:>6}\n']
   for i in range(ns):
       for j in range(9):
           if j < 8:
