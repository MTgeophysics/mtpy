...
surface_file = os.path.join(mtpy_dir, "examples", "data", "AussieContinent_etopo1.asc")
...
(
    ...
    o2d_data, rot_o2d_data, surface_file, elevation_sample_n=300
)

# Plot the profile
fig = m2d.plot(
    m2d_profile, profile_elevation, site_locations, site_elevations, site_names
)
# fig.show()
fig.savefig(os.path.join(savepath, f"m2d_plot.png"), dpi=400)

# Save the profile elevation to file that can be opened in Mamba2D
m2d.write_elevation_file(
    m2d_profile, profile_elevation, os.path.join(savepath, "elevation.txt")
)
