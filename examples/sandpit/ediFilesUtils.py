...
class EDIimporter:
    ...
    def __init__(self, EDIfilesList, compList=None, outEPSG=None):
        ...
        if compList is None:
            self.comps = ['ZXXR', 'ZXYR', 'ZYXR', 'ZYYR', 'ZXXI', 'ZXYI', 'ZYXI', 'ZYYI', 'ZXX.VAR', 'ZXY.VAR',
                          'ZYX.VAR', 'ZYY.VAR']
        else:
            self.comps = compList
    ...
    def importFiles(self):
        ...
        for nrEDI, EDIfile in enumerate(self.filesList):
            ...
            tArrRec = (np.nan * np.ones((len(freq), len(dtRI)))).view(
                dtRI)  # np.concatenate((freq*np.ones((locs.shape[0],1)),locs,np.nan*np.ones((locs.shape[0],8))),axis=1).view(dtRI)
            ...
            for comp in self.comps:
                ...
                key = [comp.lower().replace('.', '').replace(s, t) for s, t in
                       [['xx', 'yy'], ['xy', 'yx'], ['yx', 'xy'], ['yy', 'xx']] if s in comp.lower()][0]
                tArrRec[key] = mkvc(unitConvert * _findEDIcomp('>' + comp, EDIlines), 2)
            # Make a masked array
            mArrRec = np.ma.MaskedArray(rec_to_ndarr(tArrRec), mask=np.isnan(rec_to_ndarr(tArrRec))).view(
                dtype=tArrRec.dtype)
            try:
...
###################################################################################################
# python2.7 exp/ediFilesUtils.py /g/data/ha3/fxz547/Githubz/mtpy2/tests/data/AMT/15125A_imp.edi

if __name__=="__main__":
    print ("start main()")

    edi_files=sys.argv[1:]
    edi_obj= EDIimporter(edi_files)
