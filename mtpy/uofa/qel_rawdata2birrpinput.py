...
def run(dataDirA,dataDirB,dataDirC,channelsA,channelsB,channelsC,timestamp, duration,
    ...
    for idx_station,station in enumerate(stationlist):
        ...
        #get data from the first directory - from sample matching the starttime
        ...
        #add data from following directories - including interpolation within gaps
        ...
        #add data from the last directory - incl. interpolation - until endtime stamp
        ...
        if starttime <= dir_start < endtime:
            ...
            if len(lo_dirs) == 0:
                ...
                if idx > 0:
                    ...
                    tempend = find_directory_endtime(op.join(dataDir,tempdir),tempstart)
...
def evaluate_intermediate_directory(directory,samplingrate,olddata,oldend,SymResScriptbase):
    ...
    for i in range(no_gapsamples+1):
        data.append([int(round(ch1[i],0)),int(round(ch2[i])),int(round(ch3[i])),int(round(ch4[i]))])
...
def evaluate_last_directory(directory,samplingrate,olddata,oldend,finalsample,SymResScriptbase):
...
def prepare_data(dst,channelsA,channelsB,channelsC,firstsample,lastsample):
    ...
    filebase = '{0:02d}{1:02d}{2:02d}-{3:02d}h{4:02d}'.format(tA[0]%100,tA[1],tA[2],tA[3],tA[4])
