...
def main():
    ...
    if not op.isfile(responsefile):
        raise MTex.MTpyError_inputarguments(
            "Response file not existing: %s" % (responsefile)
        )
    ...
    if np.sum([len(i) for i in lo_lo_files_for_channels]) == 0:
        ...
        raise MTex.MTpyError_inputarguments(
            "No information for channels found in directory {0} - Check header lines!".format(
                directory
            )
        )
    ...
    for ch in lo_channels:
        ...
        for ta in lo_timeaxes:
            ...
            if (ta[-1] - ta[0]) < (3 * winmax):
                print(
                    "time axis short enough ({0} seconds) - reading all at once".format(
                        ta[-1] - ta[0]
                    )
                )
                ...
                print(
                    "corrected TS starting at {0}, length {1}".format(
                ...
                ta_length = ta[-1] - ta[0] + 1.0 / samplingrate
                ...
                for i in range(n_windows + 1):
                    t0 = ta[0] + remainder / 2.0 + i * winmax
                    lo_windowstarts.append(t0)
                print(
                    "time axis long ({0} seconds) - processing in {1} sections (window: {2})".format(
                ...
                for idx_t0, t0 in enumerate(lo_windowstarts):
                    ...
                    if idx_t0 == n_windows - 1:
                        ...
                        endtime = lo_windowstarts[idx_t0 + 3] - 1.0 / samplingrate
                    print(
                        "s1 = {0} - {1}, s2 = {2} - {3}, s3 = {4} - {5}\n".format(
                            t0,
                            lo_windowstarts[idx_t0 + 1] - 1.0 / samplingrate,
                            lo_windowstarts[idx_t0 + 1],
                            lo_windowstarts[idx_t0 + 2] - 1.0 / samplingrate,
                    ...
                    # be read
                        ...
                        section2_ta = (
                            np.arange(len(section2_data)) / samplingrate
                            + lo_windowstarts[idx_t0 + 1]
                        )
                    ...
                    if idx_t0 < n_windows - 1:
                        ...
                        (
                            ...
                            lo_windowstarts[idx_t0 + 2],
                            ta[-1] + 1.0 / samplingrate,
                            lo_t_mins,
                            lo_files,
                        )

                    section3_ta = (
                        np.arange(len(section3_data)) / samplingrate
                        + lo_windowstarts[idx_t0 + 2]
                    )
                    ...
                    if idx_t0 == 0:
                        ...
                        startidx = (
                            np.abs(timeaxis - lo_windowstarts[idx_t0 + 1])
                        ).argmin()
                    ...
                    if idx_t0 == n_windows - 1:
                        ...
                        # end
                        endidx = (
                            np.abs(timeaxis - lo_windowstarts[idx_t0 + 2])
                        ).argmin()
                    ...
                    while t < tmax:
                        ...
                        if file_open == False:
                            ...
                            print(
                                "read header of input file {0} ".format(lo_infiles[0])
                            )
                            header = MTfh.read_ts_header(lo_infiles[0])
                            ta_tmp = np.arange(float(header["nsamples"])) / float(
                                header["samplingrate"]
                            ) + float(header["t_min"])
                            unit = header["unit"]
                            if unit[-6:].lower() != "(true)":
                            ...
                            if tmax > ta_tmp[-1]:
                                print(
                                    "data longer than space left in file...storing first part "
                                )
                                # write as many samples to the files as there
                                # belong
                                np.savetxt(
                                    outF, data2write[: int(float(header["nsamples"]))]
                                )
                            ...
                            # of the newly opened file:
                                ...
                                print(
                                    "data fits into open file : {0}".format(outF.name)
                                )
                                # write everything
                                ...
                                if tmax == ta_tmp[-1]:
                                    ...
                                    print(
                                        "written data to file and closed it: {0}".format(
                                            outF.name
                                        )
                                    )
                                ...
                                # file (potentially) misses entries
                                    ...
                                    print(
                                        "file not full...{0} remains open - {1} of max. {2} samples written ".format(
                                            outF.name,
                                            datalength,
                                            int(float(header["nsamples"])),
                                        )
                        ...
                        # section has to be appended there:
                            ...
                            print(
                                "open file {0} is waiting for data...".format(outF.name)
                            )
                            header = MTfh.read_ts_header(lo_infiles[0])
                            ta_tmp = np.arange(float(header["nsamples"])) / float(
                                header["samplingrate"]
                            ...
                            if tmax > ta_tmp[-1]:
                                print(
                                    "data longer than space left in file...storing first {0} here and cut of the rest ({1}) for further processing...\n".format(
                                ...
                                # value!
                                endidx = (
                                    np.abs(timeaxis2write - ta_tmp[-1])
                                ).argmin() + 1
                                ...
                                # written
                                print(
                                    "cutting data to remaining bit ({0}->{1})\n".format(
                                        len(data2write),
                            ...
                            # of the open file:
                                ...
                                datalength += len(data2write)
                                print(
                                    "file contains {0} samples - {1} open ".format(
                                        datalength,
                                        int(float(header["nsamples"])) - datalength,
                                    )
                                )
                                # check, if by chance this is exactly the
                                # correct number of samples for this file:
