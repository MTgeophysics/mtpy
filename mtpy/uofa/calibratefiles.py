...
def main():
    ...
    if len(lo_allfiles) == 0:
        sys.exit(
            "Directory(ies) do(es) not contain files to calibrate:"
            " {0}".format(pathname)
        )
    ...
    if outdir is not None:
        try:
            ...
            print(
                "Output directory cannot be generated: "
    ...
    if orientation is True:
        ...
        try:
            ...
            # this only comes up, if the generic location cannot be generated
            sys.exit(
                "Re-orientation directory cannot be generated:"
                " {0}".format(ori_outdir)
            )

        MTfh.reorient_files(
            lo_allfiles, configfile, lo_stations=lo_allstations, outdir=ori_outdir
        )
        ...
        lo_allstations = []
        dirfiles = [
            op.abspath(op.join(new_inputdir, i)) for i in os.listdir(new_inputdir)
        ]
        for tmpfile in dirfiles:
            header = MTfh.read_ts_header(tmpfile)
            lo_allstations.append(header["station"].upper())
    ...
    for file_idx, filename in enumerate(lo_allfiles):
        ...
        try:
            ...
            print(
                "no entry for station {0} found in configuration file"
    ...
    print(
        "{0} files calibrated for stations"
