...
def findDataSubDirsB(starttimeA,dataDirB):
    ...
    for subdir in lo_subdirs:
        try:
            current_timestamp = calendar.timegm(time.strptime(subdir , "%Y-%m-%d-at-%H-%M-%S.bz"))
...
def prepare_data(dst,channelsA,channelsB,channelsC):
    ...
    filebase = '{0:04d}{1:02d}{2:02d}-{3:02d}h{4:02d}'.format(tA[0],tA[1],tA[2],tA[3],tA[4])
    ...
    if pid>0 :
        ...
        if pid2 > 0 :
            ...
            Bdata_section = build_section(fileB1,fileB2,channelsB,A0_int,A0_frac ,A1_int ,A1_frac,filebase+'.staB' )
        else:
            print 'child 2 ({0}) builds C'.format(os.getpid())
            Cdata_section = build_section(fileC1,fileC2,channelsC,A0_int,A0_frac ,A1_int ,A1_frac,filebase+'.staC'  )
            os._exit(0) 
