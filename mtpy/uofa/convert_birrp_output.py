...
def main():
            ...
            if o[0] == "-":
                ...
                if option not in ["b", "i"]:
                    ...
                    try:
                        ...
                        print(
                            'option "{0}" not followed by valid argument: "{1}"'
    ...
    print(
        "EDI/coh - files generated for station {0}:\n{1}\n{2}"
        "".format(stationname, edifn, cohfn)
    )


def convertbirrpoutput(
    stationname,
    datadir,
    survey_configfile,
    ...
    if birrp_configfile is not None:
        try:
            ...
            print(
                "Birrp config file not existing: {0} - using generic values"
    ...
    if instr_response_file is not None:
        ...
        try:
            edifn = MTbp.convert2edi_incl_instrument_correction(
                stationname, datadir, survey_configfile, birrp_configfile, ir_fn
            )
    ...
    try:
        edifn = MTbp.convert2edi(
            stationname, datadir, survey_configfile, birrp_configfile
        )
    except:
        try:
            edifn = MTbp.convert2edi(
                stationname.upper(), datadir, survey_configfile, birrp_configfile
            )
