...
def read_configfile(filename):
    ...
    try:
        ...
        # do NOT ask, why it does not work with reading from filename
        # directly...:
...
def read_survey_configfile(filename):
    ...
    for station in sorted(config_dict):
        ...
        # case insensitive - allow for short forms 'sampling', 'lat', 'lon',
        # and 'elev'
        for idx, req_keyword in enumerate(list_of_required_keywords):
            ...
            try:
                ...
                #import ipdb
                # ipdb.set_trace()
                if fromglobals(req_keyword, stationdict,
                               globaldict)[0] is False:
                ...
                if req_keyword in ['elevation', 'latitude', 'longitude']:
                    ...
                    try:
                        new_value = MTft._assert_position_format(
                            req_keyword, value)
                    except:
                ...
                stationdict[
                    req_keyword] = list_of_keyword_defaults_general[idx]
        ...
        if stationdict['station_type'] in ['mt', 'e']:
            # check for required electric field parameters - not done for QEL
            # loggers yet
    ...
    for station in config_dict.iterkeys():
        ...
        if stationdict['rr_station'] is not None:
            try:
                stationdict['rr_station_latitude'] = \
