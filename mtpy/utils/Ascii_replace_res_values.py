...
newgridx, newgridy = np.meshgrid(
    np.arange(gce[0], gce[-1] + cs, cs), np.arange(gcn[0], gcn[-1] + cs, cs)
)
...
for ascfn in ascfilelist:
    ...
    interpfunc = RegularGridInterpolator(
        (gce, gcn), np.log10(resgrid_nopad[:, :, di].T)
    )
    # evaluate on the regular grid points, which to be output into geogrid formatted files
    newgridres = 10 ** interpfunc(
        np.vstack([newgridx.flatten(), newgridy.flatten()]).T
    ).reshape(newgridx.shape)
    ...
    #                break
    np.savetxt(
        os.path.join(wd, ascfn), newgridres, header=header, comments="", fmt="%.3e"
    )
