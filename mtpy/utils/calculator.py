...
def invertmatrix_incl_errors(inmatrix, inmatrix_err=None):
    ...
    if inmatrix is None:
        raise MTex.MTexceptions.MTpyError_inputarguments(
    ...
    if dim != 2:
        raise MTex.MTexceptions.MTpyError_inputarguments(
            'Only 2D matrices supported yet')
            ...
            for j in range(2):
                ...
                for k in range(2):
                    for l in range(2):
                        ...
                        err += np.abs(- inv_matrix[i, k] *
                                      inv_matrix[l, j] * inmatrix_err[k, l])
...
def z_error2r_phi_error(x, x_error, y, y_error):
    ...
    # uncertainty in amplitude is defined by half the diameter of the box
    # around x,y
    rho_err = 0.5 * (max(lo_rho) - min(lo_rho))
...
# That is NOT the same as the rotated error matrix Zerr (although the
# result is similar)
...
def rotatematrix_incl_errors(inmatrix, angle, inmatrix_err=None):
    ...
    if (inmatrix_err is not None):
        ...
        errmat[0,
               0] = np.sqrt((cphi**2 * err_orig[0,
                                                0])**2 + (cphi * sphi * err_orig[0,
                                                                                 1])**2 + (cphi * sphi * err_orig[1,
                                                                                                                  0])**2 + (sphi**2 * err_orig[1,
                                                                                                                                               1])**2)
        errmat[0,
               1] = np.sqrt((cphi**2 * err_orig[0,
                                                1])**2 + (cphi * sphi * err_orig[1,
                                                                                 1])**2 + (cphi * sphi * err_orig[0,
                                                                                                                  0])**2 + (sphi**2 * err_orig[1,
                                                                                                                                               0])**2)
        errmat[1,
               0] = np.sqrt((cphi**2 * err_orig[1,
                                                0])**2 + (cphi * sphi * err_orig[1,
                                                                                 1])**2 + (cphi * sphi * err_orig[0,
                                                                                                                  0])**2 + (sphi**2 * err_orig[0,
                                                                                                                                               1])**2)
        errmat[1,
               1] = np.sqrt((cphi**2 * err_orig[1,
                                                1])**2 + (cphi * sphi * err_orig[0,
                                                                                 1])**2 + (cphi * sphi * err_orig[1,
                                                                                                                  0])**2 + (sphi**2 * err_orig[0,
                                                                                                                                               0])**2)
...
def reorient_data2D(x_values, y_values, x_sensor_angle=0, y_sensor_angle=90):
    ...
    T = np.matrix([[np.real(cmath.rect(1, x_angle)), np.imag(cmath.rect(1, x_angle))], [
