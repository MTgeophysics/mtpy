...
# second component is positve from West to East, third component is
# positive Downwards#
...
def main():
    ...
    X = np.zeros(dims[0])
    # X[0] = -0.5 * Xdist + 0.5*(Xspacing[0])# define zero at the center of
    # the model
    ...
    Y = np.zeros(dims[1])
    # Y[0] = -0.5 * Ydist + 0.5*(Yspacing[0])# define zero at the center of
    # the model
    # define zero at the lower left corner of the model
    Y[0] = 0.5 * (Yspacing[0])
