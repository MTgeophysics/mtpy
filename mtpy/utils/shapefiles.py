...
class PTShapeFile(object):
    ...
    * To project into another datum, set the projection attribute
    ...
    def __init__(self, edi_list=None, proj="WGS84", esize=0.03, **kwargs):
        ...
        )  # FZ: adjusted number of points in array
        self.ptol = 0.05  # period value tolerance to be considered as equal
    ...
    def _get_plot_period(self):
        ...
        if self.plot_period is None:
            ...
            self.plot_period = 1.0 / np.array(
                sorted(list(set(all_freqs)), reverse=True)
            )
        else:
            ...
            for ii, mt_obj in enumerate(self.mt_obj_list):
                ...
                if len(p_index) >= 1:
                    ...
                    if self.projection is None:  # geographic-coord lat lon
                        east, north, elev = (
                            mt_obj.longitude, mt_obj.latitude, 0)
                    ...
                    if pt_obj_list is None:
                        ...
                        if residual:
                            rpt_mean = 0.25 * np.linalg.norm(pt_obj.pt[p_index],
                                                             ord="fro")
                    ...
                    print(
                        (
                            "The period %s is NOT found for this station %s"
    ...
    def write_shape_files(self, periods=None, normalize="all"):
        ...
        for plot_per in periods:
            ...
            shape_fn = os.path.join(
                self.save_path, "PT_{0:.5g}s_{1}.shp".format(
                    plot_per, self.projection)
            )
...
class TipperShapeFile(object):
    ...
    def _get_tip_array(self):
        ...
        for plot_per in self.plot_period:
            ...
            self.tip_dict[plot_per] = np.array(
                self.tip_dict[plot_per],
                dtype=[
                    ("station", "|S15"),
                    ("east", np.float),
    ...
    def write_tip_shape_files_modem_residual(
        ...
        self.mt_obj_list = [modem_data_obj.mt_dict[key] for key in mt_keys]
...
if __name__ == "__main__d":
    ...
    if len(sys.argv) < 3:
        print(("USAGE: %s input_edifile_dir output_shape_file_dir" %
               sys.argv[0]))
        sys.exit(1)
    ...
    if src_file_dir[-4:].lower() == ".dat":  # modem dat file
        ...
        create_phase_tensor_shpfiles(
            ...
            dest_dir,
            proj="WGS84",
            ellipse_size=8000,  # UTM and size in meters. 1deg=100KM
