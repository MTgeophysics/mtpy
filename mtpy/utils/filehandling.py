...
def sort_folder_list(wkdir, order_file, indices=[0, 9999], delimiter=''):
    ...
    plst = []
    flst = [
        i for i in os.listdir(wkdir) if os.path.exists(
            os.path.join(
                wkdir,
                i))]
#    print flst
    for o in order:
        for f in flst:
            if str.lower(f.strip().split(delimiter)[0][indices[0]:indices[
                         1]]) == str.lower(o)[indices[0]:indices[1]]:
...
def EDL_make_Nhour_files(n_hours, inputdir, sampling,
                         stationname=None, outputdir=None):
    """
    ...
    for comp in components:
        ...
        for idx_f, f in enumerate(lo_sorted_files):
            ...
            if fileopen is False:
                ...
                # otherwise assuming that the first column is time, so just
                # take the second one
                else:
                ...
                # then delete ambiguous  parts of the buffer:
                    ...
                    # find point on the outfile time axis for the beginning of
                    # current file:
...
def EDL_make_dayfiles(inputdir, sampling, stationname=None, outputdir=None):
    ...
    for comp in components:
        ...
        for idx_f, f in enumerate(lo_sorted_files):
            ...
            # set the time as starting time for output file, if no output file
            # is open already
            if fileopen == 0:
                outfile_starttime = file_start_time
...
def reorient_files(lo_files, configfile, lo_stations=None, outdir=None):
    ...
    for file_idx, filename in enumerate(lo_files):
        ...
        if station.upper() not in [i.upper() for i in lo_stations]:
            # TODO: check, if this causes problems with the indices for the
            # current loop:
    ...
    for sta_idx, sta in enumerate(lo_stationnames):
        ...
        for sensor in ['e', 'b']:
            ...
            for idx_h_x, header_x in enumerate(lo_headers):
                ...
                # print t0
                # now look for the respective y-file and possible z-file -
                # unfortunately by another loop over all headers:
                y_file = None
                z_file = None
