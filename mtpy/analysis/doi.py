...
def old_bostick_resistivity(f, rho, pha):
    ...
    pha_rad[np.nonzero(pha_rad)] = pha_rad[np.nonzero(pha_rad)] % (2.0 * math.pi)
    print(pha_rad)
...
if __name__ == "__main__":
    ...
    sens_list = [
        np.absolute(sensitivity(zn, sigma_conduct=0.20, freq=10)) for zn in zn_list
    ]
    sens_list2 = [
        np.absolute(sensitivity(zn, sigma_conduct=2000.0, freq=0.1)) for zn in zn_list
    ]
