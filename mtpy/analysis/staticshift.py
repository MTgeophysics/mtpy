...
def estimate_static_spatial_median(
    edi_fn, radius=1000.0, num_freq=20, freq_skip=4, shift_tol=0.15
):
    """
    ...
    edi_list = [
        os.path.abspath(os.path.join(edi_path, edi))
    ...
    for kk, kk_edi in enumerate(edi_list):
        ...
        delta_d = np.sqrt(
            (mt_obj.lat - mt_obj_2.lat) ** 2 + (mt_obj.lon - mt_obj_2.lon) ** 2
        )
        if delta_d <= dm_deg:
    ...
    for kk, mt_obj_kk in enumerate(mt_obj_list):
        ...
        res_array[kk, interp_idx, :, :] = Z_interp.resistivity[
            0 : len(interp_freq_kk), :, :
        ]

    # compute the static shift of x-components
    static_shift_x = mt_obj.Z.resistivity[
        freq_skip : num_freq + freq_skip, 0, 1
    ...
    static_shift_y = mt_obj.Z.resistivity[
        freq_skip : num_freq + freq_skip, 1, 0
...
def remove_static_shift_spatial_filter(
    edi_fn, radius=1000, num_freq=20, freq_skip=4, shift_tol=0.15, plot=False
):
    """
    Remove static shift from a station using a spatial median filter.  This
    will look at all the edi files in the same directory as edi_fn and find
