...
def uniform_interior_grid(arr, spacing, mid):
    ...
    units = (
        int((end_points[0] - mid) / spacing),
        int((end_points[1] - mid) / spacing) + 1,
    )
...
def interpolate(
    resistivity_dict, source_proj, grid_proj, center, east_spacing, north_spacing
):
    """
    ...
    center_lon, center_lat, width, height = lon_lat_grid_spacing(
        center, east_spacing, north_spacing, to_grid
    )

    lon_list = [
        to_grid(x, y)[0] for x in resistivity_dict["x"] for y in resistivity_dict["y"]
    ]

    lat_list = [
        to_grid(x, y)[1] for x in resistivity_dict["x"] for y in resistivity_dict["y"]
    ]
    ...
    result["resistivity"] = np.zeros(
        tuple(result[key].shape[0] for key in ["depth", "latitude", "longitude"])
    )
    ...
    for z_index in range(result["depth"].shape[0]):
        result["resistivity"][z_index, :, :] = uniform_layer(
            interpolation_funcs[z_index], result["latitude"], result["longitude"]
        )
