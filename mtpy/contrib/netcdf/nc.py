...
def IDW(source_points, source_values, query_points, k=6, p=5):
    ...
    inv_dist = 1.0 / np.power(distances, p)
...
def write_resistivity_grid(
    output_file, epsg_code, latitude, longitude, elevation, resistivity_data, **kwargs
):
    ...
    with create_dataset(output_file) as dataset:
        ...
        resistivity = dataset.createVariable(
            "resistivity", "f4", (z_label, "latitude", "longitude")
        )
...
def clipping_mask(arr, grid):
    ...
    return and_(
        xbounds.clipping_mask(arr[:, 0]),
        and_(ybounds.clipping_mask(arr[:, 1]), zbounds.clipping_mask(arr[:, 2])),
    )
...
def grid_from_extent_and_resolution(left, right, resolution):
    ...
    x, y, z = [
        np.arange(left[dim] + resolution[dim] / 2.0, right[dim], resolution[dim])
        for dim in range(3)
    ]
