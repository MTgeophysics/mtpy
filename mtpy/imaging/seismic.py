...
class Segy:
    def __init__(self, segy_fn, pick_every=1):
        ...
        for itr, tr in enumerate(self._sf.traces):
            if itr % self._pick_every == 0:
                ...
                self._si.append(
                    tr.header.sample_interval_in_ms_for_this_trace / 1e6
                )  # convert from micro seconds to s
        ...
        self._station_space = numpy.sqrt(
            (self._xs[:-1] - self._xs[1:]) ** 2 + (self._ys[:-1] - self._ys[1:]) ** 2
        )
        ...
        self._ts = numpy.linspace(
            0, (numpy.max(self._ns) - 1) * numpy.max(self._si), numpy.max(self._ns)
        )
        self._mtraces = numpy.array(self._mtraces)
        self._mt, self._md = numpy.meshgrid(
            self._ts, self._dist
        )  # mesh grid of distance and time
        self._mt, self._mc = numpy.meshgrid(
            self._ts, self._cdps
        )  # mesh grid of cdp and time
    ...
    def getMigratedProfile(
        self, velocity_model, ntraces=-1, ndepths=-1, max_depth=60e3, time_shift=0, nn=1
    ):
        """
        ...
        if ntraces == -1:
            ntraces = self._ntraces
        if ndepths == -1:
            ndepths = self._mtraces.shape[1]
        ...
        for idist, dist in enumerate(gdistance):
            ...
            cdp = None
            if nn != -1:
                cdp = self.getAttribute("cdp", dist)
            ...
            io = interp1d(ds, amps, bounds_error=False, fill_value=-9999)
...
class VelocityModel:
    ...
    def _getIntervalVelocity(self, cdp, t):
        ...
        idx = numpy.argmax(
            numpy.logical_and(t >= self._intStarts[cdp], t <= self._intEnds[cdp])
        )
    ...
    def _generateDepthModel(self):
        ...
        for icdp, cdp in enumerate(self._cdps):
            for i in range(len(self._times[icdp]) - 1):
                self._intVels[cdp].append(
                    (
                        (
                            self._times[icdp][i + 1] * self._vels[icdp][i + 1] ** 2
                            - self._times[icdp][i] * self._vels[icdp][i] ** 2
                        )
                        / (self._times[icdp][i + 1] - self._times[icdp][i])
                    )
                    ** 0.5
                )
        ...
        self._mean_depth_profile_io = interp1d(self._ts, self._mean_depth_profile)
