...
class PlotResponse(mtpl.PlotSettings):
    ...
    def __init__(self, **kwargs):
        ...
        if mt_object is None:
            self.mt = mtpl.MTplot(fn=fn, z_object=z_object, tipper_object=tipper_object)
        ...
        # order of plots
        self.plot_order = kwargs.pop("plot_order", ["tip", "pt", "strike", "skew"])
    ...
    def plot(self):
        ...
        if self.x_limits == None:
            self.x_limits = (
                10 ** (np.floor(np.log10(self.mt.period[0]))),
        ...
        if self.res_limits == None:
            self.res_limits = (
                ...
                ** (
                    np.floor(
                        np.log10(min([self.mt.Z.res_xy.min(), self.mt.Z.res_yx.min()]))
                    )
                ),
                10
                ** (
                    np.ceil(
                        np.log10(max([self.mt.Z.res_xy.max(), self.mt.Z.res_yx.max()]))
                    )
                ),
            )
        ...
        # --> plot strike
        try:
            self.axst = self.fig.add_subplot(gs[pdict["strike"], :], sharex=self.axr)
        ...
        # --> plot skew
        try:
            self.axsk = self.fig.add_subplot(gs[pdict["skew"], :], sharex=self.axr)
        ...
        if self._plot_tipper.find("y") == 0:
            ...
            txr = self.mt.Tipper.mag_real * np.sin(
                self.mt.Tipper.angle_real * np.pi / 180 + np.pi * self.arrow_direction
            )
            tyr = self.mt.Tipper.mag_real * np.cos(
                self.mt.Tipper.angle_real * np.pi / 180 + np.pi * self.arrow_direction
            )

            txi = self.mt.Tipper.mag_imag * np.sin(
                self.mt.Tipper.angle_imag * np.pi / 180 + np.pi * self.arrow_direction
            )
            tyi = self.mt.Tipper.mag_imag * np.cos(
                self.mt.Tipper.angle_imag * np.pi / 180 + np.pi * self.arrow_direction
            )
            ...
            self.axt.plot(np.log10(self.mt.period), [0] * nt, "k", lw=0.5)
        ...
        if self._plot_strike.find("y") == 0:
            ...
            self.axst.plot(self.axr.get_xlim(), [0, 0], color="k", lw=0.5)

            self.axst.set_ylabel("Strike", fontdict=fontdict)
            self.axst.set_xlabel("Period (s)", fontdict=fontdict)
        ...
        if self._plot_skew == "y":
            ...
            self.axsk.plot(self.mt.period, [3] * len(self.mt.period), "-k", lw=self.lw)
            self.axsk.plot(self.mt.period, [-3] * len(self.mt.period), "-k", lw=self.lw)
            ...
            self.axsk.grid(True, alpha=0.25, color=(0.25, 0.25, 0.25))
        ...
        if self._plot_pt == "y":
            ...
            for ii, ff in enumerate(self.mt.period):
                ...
                eheight = (
                    self.mt.pt.phimin[ii] / self.mt.pt.phimax[ii] * self.ellipse_size
                )
                ewidth = (
                    self.mt.pt.phimax[ii] / self.mt.pt.phimax[ii] * self.ellipse_size
                )
        ...
        if self.plot_num == 3:
            ...
            # res_det
            self.ebdetr = mtpl.plot_errorbar(
                self.axr,
                self.mt.period,
                self.mt.Z.res_det,
            ...
            # phase_det
            self.ebdetp = mtpl.plot_errorbar(
                self.axp,
                self.mt.period,
                self.mt.Z.phase_det,
                marker=self.det_marker,
    ...
    plot_tipper = property(
        fget=_get_plot_tipper, fset=_set_plot_tipper, doc="""string to plot tipper"""
    )
    ...
    plot_strike = property(
        fget=_get_plot_strike, fset=_set_plot_strike, doc="""string to plot strike"""
    )
    ...
    plot_skew = property(
        fget=_get_plot_skew, fset=_set_plot_skew, doc="""string to plot skew"""
    )
    ...
    def save_plot(
        ...
        if os.path.isdir(save_fn) == False:
            ...
            self.fig.savefig(
                save_fn, fig_dpi=fig_dpi, format=file_format, orientation=orientation
            )
            plt.clf()
            plt.close(self.fig)
            ...
            save_fn = os.path.join(
                save_fn, self.mt.station + "_ResPhase." + file_format
            )
            self.fig.savefig(
                save_fn, fig_dpi=fig_dpi, format=file_format, orientation=orientation
            )
    ...
    def __str__(self):
        ...
        return (
            "Plots Resistivity and phase for the different modes of the"
            + "MT response."
        )
