...
class PlotResidualPTMaps(mtpl.MTEllipse):
    ...
    def __init__(
        self, fn_list1, fn_list2, frequencies=np.logspace(-3, 3, 40), **kwargs
    ):
        ...
        for ii, rpt in enumerate(self.rpt_array):
            ...
            if cmap.find("seg") > 0:
                ellipd.set_facecolor(
                    mtcl.get_plot_color(
                        rpt[ckey][f_index], ckey, cmap, ckmin, ckmax, bounds=bounds
                    )
                )
            else:
                ellipd.set_facecolor(
