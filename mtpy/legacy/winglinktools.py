...
def getXY(sitesfn, outfn, ncol=5):
    ...
    # redundant, but necessary, since station order in sites file can be
    # messed up
    xy_dict = {}
...
def getmeshblockcoordinates(WL_outfile):
    ...
    east_blockwidths, north_blockwidths, z_blockwidths = readWLOutFile(
        WL_outfile)
    ...
    for idx_z in range(n_down - 1):
        current_depth += (z_blockwidths[idx_z] /
                          2. + z_blockwidths[idx_z + 1] / 2.)
        lo_depths.append(current_depth)
