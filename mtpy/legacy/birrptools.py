...
#        print key+': '+str(prepdict[key])
    ...
    combcomplst = prepdict['elecori'].split(
        ',') + prepdict['magori'].split(',')
    ...
    if len(prepdict['day'].split(';')) > 1 or len(
            prepdict['day'].split(',')) > 1:
            ...
            if len(days.split(',')) > 1:
                ...
                for dd in range(len(daylst)):
                    ...
                    try:
                        ...
                        pass
                    dlst.append(ddict)
                    nreadi += float(ddict['stop'][0:2]) - \
                        float(ddict['start'][0:2])
                nreadi = int(
                    nreadi *
                    3600 *
                    float(
            ...
            # if multiple files but not continuous.
                ...
                if filt != '':
                    cdirpath = os.path.join(
                        prepdict['dirpath'], station, day, filt)
                else:
...
#        bfpath=os.path.join(bfpath,'Run{0}'.format(nruns+1))
    ...
    try:
        ...
        magtype = input(
            'input magnetometer type broadband (bb), long period(lp)')
...
def convertBIRRPoutputs(bfpath, stationinfofile, station, rrstation=None,
    ...
    count = 0
    while os.path.basename(stationdir).lower(
    ) != station.lower() and count < 12:
...
def plotBFfiles(edifile, cohfile=None, cfilelst=None, save='y',
    ...
    if save == 'y':
        ...
        if not os.path.exists(os.path.join(
                os.path.dirname(stationdir), 'Plots')):
...
def writeLogfile(bfpath, station, cohfile, datfile, impfile, scriptfile):
    ...
    try:
        ...
        pass
    logfid.write(
        '-----Resistivity and Phase Calculated from BIRRP-----' +
        '\n')
    try:
...
def sigfigs(numstr, digits=8, fmt='g'):
    ...
    if numstr.find('nan') >= 0 or numstr.find(
            'NaN') >= 0 or numstr.find('+Inf') >= 0:
...
def writecoh(dirpath, tsp='   '):
    ...
    if len(cohfilenames) == 3:
        period, freq, coh1, zcoh1 = read2c2(
            os.path.join(dirpath, cohfilenames[0]))
        period, freq, coh2, zcoh2 = read2c2(
            os.path.join(dirpath, cohfilenames[1]))
        period, freq, coh3, zcoh3 = read2c2(
            os.path.join(dirpath, cohfilenames[2]))
...
#    for ii in range(2,10,2):
    ...
    z[0:2, :, :] = z[0:2, :, :] * \
        float(dlgain) / (float(dlen[0]) * float(egain))
...
def readrf(dirpath, egain=1, dlgain=1, dlen=[1, 1], magtype='bb',
    ...
    try:
        ...
        z[0:2, :, :] = z[0:2, :, :] * \
            float(dlgain) / (float(dlen[0]) * float(egain))
        z[2:, :, :] = z[2:, :, :] * \
            float(dlgain) / (float(dlen[1]) * float(egain))
    except ValueError:
...
def readimp(filename):
    ...
    for ii in np.arange(start=2, stop=len(implines), step=1):
        ...
        zyyr = float(line[7])
        zyyi = float(line[8])
        z.append([[zxxr + 1j * zxxi, zxyr + 1j * zxyi],
                  [zyxr + 1j * zyxi, zyyr + 1j * zyyi]])
        try:
            txxr = float(line[9])
            txxi = float(line[10])
            tyyr = float(line[11])
    ...
    for ii in range(len(res)):
        ...
        for jj in range(len(period)):
            datfid.write(sigfigs(str(period[jj]), digits=7, fmt='e')
                         + tsp + sigfigs(str(res[ii][0][jj]),
                                         digits=7, fmt='e') + tsp
                         + sigfigs(str(res[ii][1][jj]),
                                   digits=7, fmt='e') + tsp
...
def readdat(filename):
    ...
    if numcomp != 4:
        raise ValueError(
            'Not right amoung of components in .dat file, check file')
    ...
    for ii in range(numcomp):
        ...
        phaseijerr = []
        period = []
        for jj in np.arange(start=rfind[ii] + 2,
                            stop=rfind[ii] + numper + 2, step=1):
...
def writeedi(dirpath, station, stationinfofile=None, rrstation=None,
    ...
    try:
        ...
        statdict['egain'] = str(
            input('Enter interface box gain (10 or 100). \n'))
    try:
        dlgain = statdict['dlgain']
    except KeyError:
...
#                                  bbfile=bbfile,ffactor=ffactor)
    ...
    if birrpdict is not None:
        ...
        try:
            ...
            pass
        try:
            edifid.write(lsp + 'Z component (NZ)=' +
                         str(birrpdict['nz']) + '\n')
        except KeyError:
            pass
        ...
        for rrs in rrstationlst:
            ...
            if rrs != station:
                if stationinfofile is None or stationinfofile == 'None':
                    ...
                    edifid.write(
                        lsp + 'Remote Reference Station: ' + rrs + '\n')
                    edifid.write(lsp + 'Remote Reference Lat='
                                 + '%2.8g' % float(rrdict['lat']) + '\n')
                    edifid.write(lsp + 'Remote Reference Long='
                                 + '%2.8g' % float(rrdict['long']) + '\n')
    ...
    implst = [['ZXXR', 0, 0], ['ZXXI', 0, 1], ['ZXX.VAR', 0, 2], ['ZXYR', 1, 0], ['ZXYI', 1, 1],
              ['ZXY.VAR', 1, 2], ['ZYXR', 2, 0], [
                  'ZYXI', 2, 1], ['ZYX.VAR', 2, 2],
...
def writeini(filepath, argsdict):
    ...
    if len(argsdict) != 38:
        ...
        inifid.write(
            'Number of reference timeseries=' +
            argsdict['tbw'] +
            '\n')
        ...
        inifid.write(
            'Remote Reference component list (in order)=' +
            argsdict['complstr'] +
            '\n')
