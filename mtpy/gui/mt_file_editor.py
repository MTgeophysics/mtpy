...
class SiteTab(QtWidgets.QWidget):
    ...
    def _set_ll(self):
        if (
            self.Site.Location.easting is not None
            and self.Site.Location.northing is not None
            and self.Site.Location.utm_zone is not None
        ):
            ...
            print(
                self.Site.Location.easting,
                self.Site.Location.northing,
                self.Site.Location.utm_zone,
            )

    def _set_utm(self):
        if (
            self.Site.Location.latitude is not None
            and self.Site.Location.longitude is not None
        ):
...
class FieldNotesTab(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.data_logger_man_edit = QtWidgets.QLineEdit(
        ...
        self.data_logger_type_edit = QtWidgets.QLineEdit(
            self.FieldNotes.DataLogger.type
        )
        self.data_logger_type_edit.editingFinished.connect(self.set_dl_type)

        # --> Instrument information
        self.ex_widget = Electrode_Widget(self.FieldNotes.Electrode_ex, comp="EX")
        self.ey_widget = Electrode_Widget(self.FieldNotes.Electrode_ey, comp="EY")

        self.hx_widget = Magnetometer_Widget(self.FieldNotes.Magnetometer_hx, comp="HX")
        self.hy_widget = Magnetometer_Widget(self.FieldNotes.Magnetometer_hy, comp="HY")
        self.hz_widget = Magnetometer_Widget(self.FieldNotes.Magnetometer_hz, comp="HZ")
        ...
        self.dq_warning_comments_edit.editingFinished.connect(
            self.set_dq_warning_comments
        )
    ...
    def set_dq_period_min(self):
        self.FieldNotes.DataQuality.good_from_period = _check_float(
            self.dq_good_periods_min.text()
        )
        self.dq_good_periods_min.setText(
            "{0:.5g}".format(self.FieldNotes.DataQuality.good_from_period)
        )

    def set_dq_period_max(self):
        self.FieldNotes.DataQuality.good_to_period = _check_float(
            self.dq_good_periods_max.text()
        )
        self.dq_good_periods_max.setText(
            "{0:.5g}".format(self.FieldNotes.DataQuality.good_to_period)
        )
...
class ProcessingTab(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.software_author_email_edit.editingFinished.connect(
            self.set_software_author_email
        )

        self.software_author_org_label = QtWidgets.QLabel("Author Organization")
        self.software_author_org_edit = QtWidgets.QLineEdit()
        self.software_author_org_edit.editingFinished.connect(
            self.set_software_author_org
        )

        self.software_author_url_label = QtWidgets.QLabel("URL")
        self.software_author_url_edit = QtWidgets.QLineEdit()
        self.software_author_url_edit.editingFinished.connect(
            self.set_software_author_url
        )
...
class ProvenanceTab(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        creation_layout.addRow(self.creating_app_label, self.creating_app_edit)
        creation_layout.addRow(self.creation_time_label, self.creation_time_edit)
