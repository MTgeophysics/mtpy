...
class ModEM_Mesh_Window(QtGui.QMainWindow):
    ...
    def get_model_fn(self):
        ...
        fn_dialog = QtGui.QFileDialog()
        fn = str(
            fn_dialog.getOpenFileName(caption="Choose ModEM model file", filter="*.rho")
        )
    ...
    def save_model_fn(self):
        ...
        fn_dialog = QtGui.QFileDialog()
        save_fn = str(
            fn_dialog.getSaveFileName(caption="Choose ModEM model file", filter="*.rho")
        )
...
class MeshWidget(QtGui.QWidget):
    ...
    def setup_ui(self):
        ...
        self.cell_size_edit_east.setText(
            "{0:.2f}".format(self.model_obj.cell_size_east)
        )
        self.cell_size_edit_east.editingFinished.connect(self.set_cell_size_east)

        self.cell_size_edit_north = QtGui.QLineEdit()
        self.cell_size_edit_north.setText(
            "{0:.2f}".format(self.model_obj.cell_size_north)
        )
    ...
    def _grid_to_nodes(self, grid_array):
        ...
        nodes_array[: nx / 2] = np.array(
            [abs(grid_array[ii] - grid_array[ii + 1]) for ii in range(int(nx / 2))]
        )
        nodes_array[nx / 2 :] = np.array(
            [
                abs(grid_array[ii] - grid_array[ii + 1])
    ...
    def load_edi_files(self):
        fn_list = QtGui.QFileDialog().getOpenFileNames(
            caption="Choose EDI Files", filter="*.edi"
        )
        self.edi_list = []
        for fn in fn_list:
    ...
    def save_mesh(self):
        ...
        # be sure to change the grid into nodes
        self.model_obj.nodes_east = self._grid_to_nodes(
            self.mpl_widget.model_obj.grid_east
        )
        self.model_obj.nodes_north = self._grid_to_nodes(
            self.mpl_widget.model_obj.grid_north
        )
    ...
    def set_rho(self):
        if self.model_obj.res_model is None:
            self.model_obj.res_model = np.zeros(
                (
                    self.model_obj.grid_north.shape[0],
        ...
        self.rho_start_edit.setText(
            "{0:.2f}".format(float(str(self.rho_start_edit.text())))
        )
...
class MeshPlot(QtGui.QWidget):
    ...
    def plot_mesh(self, model_obj, east_limits=None, north_limits=None, z_limits=None):
        ...
        station_plot_east = self.model_obj.station_locations["rel_east"] / 1000.0
        station_plot_north = self.model_obj.station_locations["rel_north"] / 1000.0

        self.plot_grid_east = self.model_obj.grid_east.copy() / 1000.0
        self.plot_grid_north = self.model_obj.grid_north.copy() / 1000.0
        self.plot_grid_z = self.model_obj.grid_z.copy() / 1000.0
        ...
        if east_limits == None:
            self.ax_map.set_xlim(self.plot_grid_east.min(), self.plot_grid_east.max())
            pass
        else:
            self.ax_map.set_xlim(east_limits)

        if north_limits == None:
            self.ax_map.set_ylim(self.plot_grid_north.min(), self.plot_grid_north.max())
            pass
        else:
        ...
        for xx in self.plot_grid_east:
            ...
            east_line_xlist.append(None)
            east_line_ylist.extend([0, self.plot_grid_z.max()])
            east_line_ylist.append(None)
        ...
        for zz in self.plot_grid_z:
            z_line_xlist.extend([self.plot_grid_east.min(), self.plot_grid_east.max()])
        ...
        if z_limits == None:
            self.ax_depth.set_ylim(model_obj.z_target_depth / 1000.0, -1)
        else:
            self.ax_depth.set_ylim(z_limits)

        self.ax_depth.set_xlim(
    ...
    def on_pick(self, event):
        ...
        if event.mouseevent.button == 1:
            if self.line_mode == "add_h" and self._ax == self.ax_map:
                ...
                north = np.round(float(data_point.ydata) * 1000.0, -2)

                self.model_obj.grid_north = np.append(self.model_obj.grid_north, north)
                self.model_obj.grid_north.sort()
                self.ax_map.plot(
                    [self.plot_grid_east.min(), self.plot_grid_east.max()],
                    [north / 1000.0, north / 1000.0],
                    lw=self.line_width,
                    color="r",
                    picker=3,
                )
                ...
                depth = np.round(float(data_point.ydata) * 1000.0, -2)

                self.model_obj.grid_z = np.append(self.model_obj.grid_z, depth)
                ...
                self.ax_map.plot(
                    [east / 1000.0, east / 1000.0],
                    [self.plot_grid_north.min(), self.plot_grid_north.max()],
                    lw=self.line_width,
                    color="r",
                    picker=3,
                )
                ...
                east = data_point.get_xdata()[event.ind] * 1000.0
                north = data_point.get_ydata()[event.ind] * 1000.0
                ...
                data_point = event.artist
                east = data_point.get_xdata()[event.ind] * 1000.0
                depth = data_point.get_ydata()[event.ind] * 1000.0
