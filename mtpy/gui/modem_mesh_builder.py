...
class MeshWidget(QtGui.QWidget):
    ...
    def setup_ui(self):
        ...
        self.cell_size_edit_east.editingFinished.connect(
            self.set_cell_size_east)

        self.cell_size_edit_north = QtGui.QLineEdit()
        self.cell_size_edit_north.setText(
            '{0:.2f}'.format(self.model_obj.cell_size_north))
        self.cell_size_edit_north.editingFinished.connect(
            self.set_cell_size_north)
        ...
        self.cell_pad_east_edit.setText(
            '{0:.0f}'.format(self.model_obj.pad_east))
        self.cell_pad_east_edit.editingFinished.connect(self.set_cell_pad_east)

        self.cell_pad_north_edit = QtGui.QLineEdit()
        self.cell_pad_north_edit.setText(
            '{0:.0f}'.format(self.model_obj.pad_north))
        self.cell_pad_north_edit.editingFinished.connect(
            self.set_cell_pad_north)
        ...
        self.rot_ang_edit.setText(
            '{0:.2f}'.format(
                self.model_obj.mesh_rotation_angle))
...
#            self.list_widget.addItem(item)
    ...
    def set_cell_size_east(self):
        self.model_obj.cell_size_east = float(
            str(self.cell_size_edit_east.text()))
        self.cell_size_edit_east.setText(
            '{0:.2f}'.format(
                self.model_obj.cell_size_east))

    def set_cell_size_north(self):
        self.model_obj.cell_size_north = float(
            str(self.cell_size_edit_north.text()))
        self.cell_size_edit_north.setText(
            '{0:.2f}'.format(self.model_obj.cell_size_north))
    ...
    def set_rotation_angle(self):
        self.model_obj.mesh_rotation_angle = float(
            str(self.rot_ang_edit.text()))
        self.rot_ang_edit.setText(
            '{0:.2f}'.format(
                self.model_obj.mesh_rotation_angle))
    ...
    def save_mesh(self):
        ...
        self.model_obj.nodes_z = self._grid_to_nodes(
            self.mpl_widget.model_obj.grid_z)
    ...
    def set_rho(self):
        if self.model_obj.res_model is None:
            self.model_obj.res_model = np.zeros((self.model_obj.grid_north.shape[0],
                                                 self.model_obj.grid_east.shape[
                                                     0],
                                                 self.model_obj.grid_z.shape[0]))

        self.model_obj.res_model[:, :, :] = float(
            str(self.rho_start_edit.text()))
        self.rho_start_edit.setText('{0:.2f}'.format(
            float(str(self.rho_start_edit.text()))))
...
class MeshPlot(QtGui.QWidget):
    ...
    def plot_mesh(self, model_obj, east_limits=None, north_limits=None,
        ...
        station_plot_east = self.model_obj.station_locations[
            'rel_east'] / 1000.
        station_plot_north = self.model_obj.station_locations[
            'rel_north'] / 1000.
