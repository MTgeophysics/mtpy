...
class ConvertModel2VTK(QtWidgets.QWidget):
    ...
    def get_cwd(self):
        ...
        self.cwd = os.path.abspath(
            str(dir_dialog.getExistingDirectory(caption="Choose Model Directory"))
        )
    ...
    def set_fn_lists(self):
        ...
        if self.model_type == "ModEM":
            ...
            self._mfn_list = [
                fn for fn in os.listdir(self.cwd) if fn.find("model") >= 0
            ]
    ...
    def set_model_fn(self, index):
        ...
        if self.model_type == "ModEM":
            ...
            self.vtk_model_fn = "{0}_res".format(
                self.model_fn[: self.model_fn.find(".")]
            )
    ...
    def set_resp_fn(self, index):
        ...
        if self.model_type == "ModEM":
            ...
            self.vtk_station_fn = "{0}_stations".format(
                self.resp_fn[: self.model_fn.find(".")]
            )
    ...
    def make_vtk_files(self):
        if self.model_type == "ModEM":
            ...
            m_obj.write_vtk_file(
                vtk_save_path=self.cwd, vtk_fn_basename=self.vtk_model_fn
            )

            d_obj = mtpy.modeling.modem.Data()
            d_obj.read_data_file(self.resp_fn)
            d_obj.write_vtk_station_file(
                vtk_save_path=self.cwd, vtk_fn_basename=self.vtk_station_fn
            )
            program = "modem2vtk"
        elif self.model_type == "WS":
            program = "ws2vtk"
