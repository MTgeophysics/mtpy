...
class TSData:
                ...
                for station in network:
                    ...
                    for channel in station:
                        ...
                        if wavename not in self.wavelist[network.code][station.code]:
                            self.wavelist[network.code][station.code][wavename] = [
                                str(channel)
                            ]
                        else:
                            self.wavelist[network.code][station.code][wavename].append(
                                str(channel)
                            )
    ...
    def getwaveform(
        self, waveform: str, starttime: datetime = None, endtime: datetime = None
    ):
        ...
        if waveform in self.wavecache and (
            timewindow is None
            or abs(self.wavecache[waveform][0] - timewindow) / timewindow < 0.1
        ):
            return self.readcache(waveform, starttime, endtime)
        elif self.writecache(waveform, timewindow):
            return self.readcache(waveform, starttime, endtime)
        else:
            outwave, wavename, start_date, end_date, gaps = self.readdisc(
                waveform, starttime, endtime
            )
            wave = np.vstack(
                (outwave.times() + outwave.meta["starttime"].timestamp, outwave.data)
            )
    ...
    def writecache(self, waveform, timewindow):
        ...
        if (
            channel.end_date - channel.start_date
        ) / channel.sample_rate < self.cachesize:
            outwave, wavename, start_date, end_date, gaps = self.readdisc(
                waveform, channel.start_date, channel.end_date, False
            )
    ...
    def readcache(self, waveform: str, starttime: datetime, endtime: datetime):
        ...
        timewindow, wave, wavename, start_date, end_date, gaps = self.wavecache[
            waveform
        ]
