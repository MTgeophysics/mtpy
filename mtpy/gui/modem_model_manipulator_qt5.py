...
class ModEM_Model_Manipulator(QtWidgets.QMainWindow):
    ...
    def set_res_limits(self):
        ...
        self.res_popup = ResLimits(
            self.model_widget.res_limits[0], self.model_widget.res_limits[1]
        )
        # self.popup.show()
...
class ModelWidget(QtWidgets.QWidget):
    ...
    def ui_setup(self):
        ...
        self.map_depth_label = QtWidgets.QLabel(
            "Depth {0:>10.2f} {1}".format(0, self.units)
        )
        ...
        self.east_canvas.setSizePolicy(
            QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding
        )
        ...
        self.east_label = QtWidgets.QLabel(
            "Easting {0:>10.2f} {1}".format(0, self.units)
        )
        ...
        self.north_canvas.setSizePolicy(
            QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding
        )
        ...
        self.location_canvas.setSizePolicy(
            QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding
        )
        ...
        self.cb_canvas.setMaximumWidth(int(self.screen_size.width() * 0.05))
        self.cb_canvas.setMinimumHeight(int(self.screen_size.height() * 0.85))
        self.cb_ax = self.cb_figure.add_axes([0.45, 0.005, 1.0, 1.0])
        self.cb_ax.pcolormesh(
            self.cb_x,
            self.cb_y,
            self.cb_bar,
        ...
        self.cb_ax.set_yticks(
            np.arange(
                self._res_limits[0],
                self._res_limits[1],
                (self.res_limits[1] - self._res_limits[0]) / 10.0,
            )
        )
    ...
    def model_fn(self, model_fn):
        ...
        self.east_ax = self.east_figure.add_subplot(
            1,
            1,
            1,
            # aspect='equal',
        ...
        self.east_line = self.location_ax.plot(
            [
        ...
        self.north_line = self.location_ax.plot(
            [
    ...
    def initialize_vectors(self):
        ...
        self.plot_east_map, self.plot_north_map = np.meshgrid(
            self.model_obj.grid_east / self.scale,
            self.model_obj.grid_north / self.scale,
            indexing="ij",
        )
        self.plot_east_z, self.plot_z_east = np.meshgrid(
            self.model_obj.grid_east / self.scale,
            self.model_obj.grid_z / self.scale,
            indexing="ij",
        )
        self.plot_north_z, self.plot_z_north = np.meshgrid(
            self.model_obj.grid_north / self.scale,
            self.model_obj.grid_z / self.scale,
            indexing="ij",
        )
        ...
        for xx in self.model_obj.grid_east:
            ...
            self.map_east_line_ylist.extend(
                [
                    self.model_obj.grid_north.min() / self.scale,
                    self.model_obj.grid_north.max() / self.scale,
                ]
        ...
        for yy in self.model_obj.grid_north:
            self.map_north_line_xlist.extend(
                [
                    self.model_obj.grid_east.min() / self.scale,
                    self.model_obj.grid_east.max() / self.scale,
                ]
        ...
        for xx in self.model_obj.grid_north:
            ...
            self.east_north_line_ylist.extend(
                [
                    self.model_obj.grid_z.min() / self.scale,
        ...
        for yy in self.model_obj.grid_z:
            self.north_z_line_xlist.extend(
                [
                    self.model_obj.grid_east.min() / self.scale,
                    self.model_obj.grid_east.max() / self.scale,
                ]
        ...
        if line is not None:
            ...
            for ss in line.itertuples():
                self.east_ax.text(
                    ...
                    clip_on=True,
                    bbox={"boxstyle": "square", "ec": "k", "fc": "w"},
                )
    ...
    def _get_change_index(self, y1, y2, grid_dir):
        ...
        if y1 < y2:
            ychange = np.where(
                (grid_dir / self.scale > y1) & (grid_dir / self.scale < y2)
            )[0]
            if len(ychange) == 0:
    ...
    def location_pick(self, event):
        ...
        if event.mouseevent.button == 1:
            ...
            try:
                x_index = np.where(
                    self.model_obj.grid_east / self.scale >= data_point.xdata
                )[0][0]
            except IndexError:
                return
            try:
                y_index = np.where(
                    self.model_obj.grid_north / self.scale >= data_point.ydata
                )[0][0]
            except IndexError:
                return
            dx = np.abs(
    ...
    def map_copy_down(self):
        ...
        """
        o_shape = (self.new_res_model.shape[0], self.new_res_model.shape[1], 1)
    ...
    def east_copy_east(self):
        ...
        """
        o_shape = (self.new_res_model.shape[0], 1, self.new_res_model.shape[2])
    ...
    def north_copy_south(self):
        ...
        """
        o_shape = (1, self.new_res_model.shape[1], self.new_res_model.shape[2])
