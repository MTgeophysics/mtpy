...
class Occam1D_GUI(QtWidgets.QMainWindow):
    ...
    def display_help(self):
        ll = [
            "***Be sure you have a working executable of Occam1D first***\n",
            "To begin: ",
    ...
    def get_data_file(self):
        ...
        fn = str(
            fn_dialog.getOpenFileName(
                caption="Choose Occam 1D data file",
                filter="(*.dat);; (*.data)",
        ...
        self.occam_widget.mpl_widget.plot_data(
            data_fn=self.occam_widget.occam_data.data_fn
        )
    ...
    def get_model_file(self):
        ...
        fn = str(
            fn_dialog.getOpenFileName(
                caption="Choose Occam 1D model file", directory=self.dir_path
            )[0]
        )
...
class OccamWidget(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.data_rotate_edit = QtWidgets.QLineEdit(
        ...
        self.start_roughness_edit.setText(
    ...
    def get_edi_file(self):
        ...
        self.station_dir = os.path.join(os.path.dirname(self.edi_fn), station)
        ...
        self.occam_data.write_data_file(
            edi_file=self.edi_fn,
    ...
    def apply_ss(self):
        self.mpl_widget.data_obj.res_te[0] /= 1.0 / self.ss
        self.mpl_widget.data_obj.res_tm[0] /= 1.0 / self.ss
        self.mpl_widget.data_obj.res_te[1] /= 1.0 / self.ss
        self.mpl_widget.data_obj.res_tm[1] /= 1.0 / self.ss
    ...
    def run_occam(self):
        ...
        warning_txt = "\n".join(
            [
                "Cannot find Occam1D executable. ",
        ...
        # run occam
        occam_run = occam1d.Run(
            startup_fn=self.occam_startup.startup_fn,
            occam_path=self.occam_exec,
            mode=self.data_mode,
        )

        ini_resp_fn = os.path.join(
            self.save_dir, "{0}_{1}.resp".format(self.data_mode, 1)
        )
        ini_model_fn = os.path.join(
            self.save_dir, "{0}_{1}.iter".format(self.data_mode, 1)
        )

        ini_resp_fn = os.path.abspath(ini_resp_fn)
        ini_model_fn = os.path.abspath(ini_model_fn)
        self.mpl_widget.plot_data(
            data_fn=self.occam_data.data_fn,
            resp_fn=ini_resp_fn,
            iter_fn=ini_model_fn,
            model_fn=self.occam_model.model_fn,
        )

        self.l2_widget.plot_l2(
            dir_path=self.save_dir, model_fn=self.occam_model.model_fn
        )
    ...
    def on_click(self, event):
        ...
        ini_resp_fn = os.path.join(
            self.save_dir, "{0}_{1}.resp".format(self.data_mode, iteration)
        )
        ini_model_fn = os.path.join(
            self.save_dir, "{0}_{1}.iter".format(self.data_mode, iteration)
        )
    ...
    def set_iteration(self, text):
        ...
        print(
            "Iteration {0}, RMS={1:.2f}, Roughnes={2:.2f}".format(
...
class OccamPlot(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.subplot_wspace = 0.15
        self.subplot_hspace = 0.2
        self.subplot_right = 0.90
    ...
    def plot_data(
        ...
        if data_fn is not None:
            ...
            # --> TE mode data
            if len(pxy) > 0:
                pte = self.axp.errorbar(
                    1.0 / self.data_obj.freq[pxy],
            ...
            # --> TE mode data
            if len(pyx) > 0:
                ptm = self.axp.errorbar(
                    1.0 / self.data_obj.freq[pyx],
        ...
        if resp_fn is not None:
            ...
            # --> TE mode Data
            if len(ryx) > 0:
                rtmr = self.axr.errorbar(
                    1.0 / self.resp_obj.freq[ryx],
        ...
        # --> set axis properties-----------------------------------------------
        x_limits = (
            10 ** np.floor(np.log10(1.0 / self.data_obj.freq.max())),
            10 ** np.ceil(np.log10(1.0 / self.data_obj.freq.min())),
        )
        ...
        self.axr.grid(True, alpha=0.75, which="both", color=(0.75, 0.75, 0.75))
        plt.setp(self.axr.xaxis.get_ticklabels(), visible=False)
        self.axp.grid(True, alpha=0.75, which="both", color=(0.75, 0.75, 0.75))
        ...
        if model_fn is not None:
            ...
            if max(self.depth_limits) > plot_depth.max():
                if self.depth_scale == "log":
                    self.axm.set_ylim(
                        ymin=max(self.depth_limits) / dscale,
        ...
        self.axm.grid(True, which="both", alpha=0.75, color=(0.75, 0.75, 0.75))
    ...
    def on_pick(self, event):
        ...
        p_index = np.where(1.0 / self.data_obj.freq == data_period)[0][0]
        ...
        if event.mouseevent.button == 3:
            ...
            if self._ax_index == 0:
                ...
                if self.data_obj.res_te[1, p_index] != 0:
                    print(
                        "Res err changed to: {0:.2f}".format(
                ...
                neb_l = eb[1, 1] + 0.1 * abs(eb[1, 1])
                ecap_l = ecap_l - 0.1 * abs(ecap_l)
                ecap_u = ecap_u + 0.1 * abs(ecap_u)
            ...
            if self._ax_index == 1:
                ...
                self.data_obj.phase_te[1, p_index] = te_err + te_err * 0.05
                self.data_obj.phase_tm[1, p_index] = tm_err + tm_err * 0.05

                if self.data_obj.phase_te[1, p_index] != 0:
                    print(
                        "Phase err changed to: {0:.2f}".format(
                ...
                neb_l = eb[1, 1] + 0.025 * abs(eb[1, 1])
                ecap_l = ecap_l - 0.025 * abs(ecap_l)
                ecap_u = ecap_u + 0.025 * abs(ecap_u)
...
class PlotL2(QtWidgets.QWidget):
    ...
    def __init__(self, dir_path=None, model_fn=None):
        ...
        self.subplot_right = 0.90
        self.subplot_left = 0.085
        self.subplot_top = 0.86
        self.subplot_bottom = 0.15
    ...
    def plot_l2(self, dir_path=None, model_fn=None):
        ...
        self.ax1.set_ylim(
            np.floor(self.rms_arr["rms"][1:].min()),
            np.ceil(self.rms_arr["rms"][1:].max()),
        )
        self.ax2.set_ylim(
            np.floor(self.rms_arr["roughness"][1:].min()),
