...
class Residual(object):
    ...
    def get_rms(self, residual_fn=None):
        ...
        for station_name in self.rms_array["station"]:
            rms_value_list = []
            rms_value_list_ztip = np.zeros((self.residual_array["z"].shape[1], 6))
            ...
            if np.amax(np.abs(res_vals["z"])) > 0:
                ...
                self.rms_array["rms_z_period"][sta_ind] = (
                    np.sum(z_norm.reshape(z_norm.shape[0], 4) ** 2, axis=1) / count_z
                ) ** 0.5
            ...
            if np.amax(np.abs(res_vals["tip"])) > 0:
                ...
                count_tip = np.count_nonzero(
                    np.nan_to_num(tip_norm.reshape(tip_norm.shape[0], 2)), axis=1
                ).astype(float)

                # normalized error split by period
                self.rms_array["rms_tip_period"][sta_ind] = (
                    np.nansum(tip_norm.reshape(tip_norm.shape[0], 2) ** 2, axis=1)
                    / count_tip
                ) ** 0.5
            ...
            rms_value = ((rms_value_list ** 2.0).sum() / rms_value_list.size) ** 0.5
            ...
            for i in range(ijvals[0]):
                for j in range(ijvals[1]):
                    self.rms_array["rms_{}_component".format(cpt)][:, i, j] = (
                        ...
                        / np.nansum(np.isfinite(res_vals_cpt[:, :, i, j]), axis=1)
                    ) ** 0.5
