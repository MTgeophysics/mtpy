...
#        xi.reshape(np.product(xishape[:-1]),xishape[-1])
        ...
        # values are a 1d array constructed by stacking individual model results
        for n in range(3):
...
#                self.resistivity[:,:,n] = f(xi[:,:,0],xi[:,:,1])
                self.resistivity[:,:,n] = si.griddata(points,values,xi,method=self.inversion1d_imethod).T
#                rvals = f(xi[:,:,0],xi[:,:,1])
        ...
        for s in range(len(self.station_numbers)):
            ...
            for i in range(2):
                for j in range(2):
                    ...
                    res[s,:,i,j] = self.period*(4e5/np.pi)*np.abs(z[s,:,i,j])**2
                    ii += 2
