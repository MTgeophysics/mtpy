...
try:
    ...
    print("If you want to write a vtk file for 3d viewing, pyevtk")
...
class WSData(object):
    ...
    def build_data(self):
        ...
        if self.edi_list is None:
            raise WSInputError(
                "Need to input a list of .edi files to build " "the data"
            )

        if self.period_list is None:
            raise WSInputError(
                "Need to input a list of periods to extract " "from the .edi files."
            )
        ...
        if self.wl_site_fn != None:
            if self.wl_out_fn is None:
                raise IOError(
                    "Need to input an .out file to get station"
                    "locations, this should be output by Winglink"
                )

            # get x and y locations on a relative grid
            east_list, north_list, station_list = wl.get_station_locations(
                self.wl_site_fn, self.wl_out_fn, ncol=self.ncol
            )
        ...
        if self.station_locations != None:
            try:
                for dd, sd in enumerate(self.station_locations):
                    ...
                    stations = WSStation()
                    stations.station_fn = os.path.join(
                        self.save_path, "WS_Station_locations.txt"
                    )
    ...
    def compute_errors(self):
        ...
        for d_arr in self.data:
            ...
            d_arr["z_err_map"] = np.reshape(
                len(self.period_list) * self.z_err_map, (len(self.period_list), 2, 2)
            )
    ...
    def write_data_file(self, **kwargs):
        ...
        ofid.write("{0:d} {1:d} {2:d}\n".format(n_stations, n_periods, self.n_z))
    ...
    def read_data_file(self, data_fn=None, wl_sites_fn=None, station_fn=None):
        ...
        if os.path.isfile(self.data_fn) is False:
            raise WSInputError("Could not find {0}, check path".format(self.data_fn))
...
class WSStation(object):
    ...
    def write_station_file(
        self, east=None, north=None, station_list=None, save_path=None, elev=None
    ):
        """
        ...
        if save_path is not None:
            ...
            self.save_path = os.getcwd()
            self.station_fn = os.path.join(self.save_path, "WS_Station_Locations.txt")
        elif os.path.isdir(self.save_path):
            self.station_fn = os.path.join(self.save_path, "WS_Station_Locations.txt")

        sfid = file(self.station_fn, "w")
        sfid.write(
            "{0:<14}{1:^14}{2:^14}{3:^14}\n".format("station", "east", "north", "elev")
        )
...
class WSMesh(object):
    ...
    def make_mesh(self):
        ...
        if self.station_locations is None:
            if self.edi_list is None:
                raise AttributeError(
                    "edi_list is None, need to input a list of " "edi files to read in."
                )
            ...
            if self.rotation_angle != 0:
                ...
                rot_matrix = np.matrix(
                    np.array([[cos_ang, sin_ang], [-sin_ang, cos_ang]])
                )

                coords = np.array(
                    [self.station_locations["east"], self.station_locations["north"]]
                )
                ...
                print(
                    "Rotated stations by {0:.1f} deg clockwise from N".format(
                        self.rotation_angle
                    )
                )
            ...
            east_center = (
                ...
                - np.abs(self.station_locations["east"].min())
            ) / 2
            north_center = (
                self.station_locations["north"].max()
                - np.abs(self.station_locations["north"].min())
            ) / 2
        ...
        midxgrid = np.arange(
            start=west, stop=east + self.cell_size_east, step=self.cell_size_east
        )
        ...
        pad_west = (
            np.round(
                -self.cell_size_east
                * self.pad_root_east
                ** np.arange(start=0.5, stop=3, step=3.0 / self.pad_east)
            )
            + west
        )
        ...
        midygrid = np.arange(
            start=south, stop=north + self.cell_size_north, step=self.cell_size_north
        )
        ...
        south_pad = (
            np.round(
                ...
                * self.pad_root_north
                ** np.arange(start=0.5, stop=3, step=3.0 / self.pad_north)
            )
            + south
        )
        ...
        east_nodes[: nx / 2] = np.array(
            [abs(east_gridr[ii] - east_gridr[ii + 1]) for ii in range(int(nx / 2))]
        )
        east_nodes[nx / 2 :] = np.array(
            [
                abs(east_gridr[ii] - east_gridr[ii + 1])
        ...
        north_nodes[: ny / 2] = np.array(
            [abs(north_gridr[ii] - north_gridr[ii + 1]) for ii in range(int(ny / 2))]
        )
        north_nodes[ny / 2 :] = np.array(
            [
                abs(north_gridr[ii] - north_gridr[ii + 1])
        ...
        east_grid[: int(nx / 2)] = -np.array(
            [east_nodes[ii : int(nx / 2)].sum() for ii in range(int(nx / 2))]
        )
        east_grid[int(nx / 2) :] = (
            np.array(
                [
                    east_nodes[int(nx / 2) : ii + 1].sum()
        ...
        north_grid[: int(ny / 2)] = -np.array(
            [north_nodes[ii : int(ny / 2)].sum() for ii in range(int(ny / 2))]
        )
        north_grid[int(ny / 2) :] = (
            np.array(
                [
                    north_nodes[int(ny / 2) : ii + 1].sum()
        ...
        for ii in range(n_stations):
            ...
            xx = [
                nn
                for nn, xf in enumerate(east_grid)
            ...
            yy = [
                mm
                for mm, yf in enumerate(north_grid)
        ...
        stations = WSStation()
        stations.write_station_file(
            east=self.station_locations["east_c"],
    ...
    def plot_mesh(self, east_limits=None, north_limits=None, z_limits=None, **kwargs):
        ...
        for xx in self.grid_east:
            ...
            east_line_ylist.extend([self.grid_north.min(), self.grid_north.max()])
        ...
        for yy in self.grid_north:
            north_line_xlist.extend([self.grid_east.min(), self.grid_east.max()])
        ...
        if east_limits == None:
            ax1.set_xlim(
                self.station_locations["east"].min() - 10 * self.cell_size_east,
                self.station_locations["east"].max() + 10 * self.cell_size_east,
            )
        else:
        ...
        if north_limits == None:
            ax1.set_ylim(
                self.station_locations["north"].min() - 10 * self.cell_size_north,
                self.station_locations["north"].max() + 10 * self.cell_size_east,
            )
        else:
    ...
    def convert_model_to_int(self):
        ...
        self.res_dict = dict(
            [(res, ii) for ii, res in enumerate(sorted(self.res_list), 1)]
        )
        ...
        for ii, res in enumerate(self.res_list):
            ...
            if ii == 0:
                ...
                indexes = np.where(
                    (self.res_model > self.res_list[l_index])
    ...
    def write_initial_file(self, **kwargs):
        ...
        if self.res_list is None:
            nr = 0
        elif type(self.res_list) is not list and type(self.res_list) is not np.ndarray:
            ...
            for ll in layers:
                ...
                for nn in range(self.nodes_north.shape[0]):
                    for ee in range(self.nodes_east.shape[0]):
                        if nr > 0:
                            ifid.write(
                                "{0:>3.0f}".format(write_res_model[nn, ee, ll[0]])
                            )
                        else:
                            ifid.write(
                                "{0:>8.1f}".format(write_res_model[nn, ee, ll[0]])
                            )
                    ifid.write("\n")
            ifid.close()
    ...
    def read_initial_file(self, initial_fn):
        ...
        self.grid_north[: int(n_north / 2)] = -np.array(
            [
                self.nodes_north[ii : int(n_north / 2)].sum()
        ...
        self.grid_north[int(n_north / 2) :] = (
            np.array(
                [
                    self.nodes_north[int(n_north / 2) : ii + 1].sum()
        ...
        self.grid_east[: int(n_east / 2)] = -np.array(
            [
                self.nodes_east[ii : int(n_east / 2)].sum()
        ...
        self.grid_east[int(n_east / 2) :] = (
            np.array(
                [
                    self.nodes_east[int(n_east / 2) : ii + 1].sum()
            ...
            while line_index < len(ilines):
                ...
                if len(iline) == 2:
                    ...
                    while count_e < n_east:
                        ...
                        # in ws3dinv
                        self.res_model[count_n, count_e, l1:l2] = self.res_list[
                            int(iline[count_e]) - 1
                        ]
                        self.res_model_int[count_n, count_e, l1:l2] = int(
                            iline[count_e]
                        )
                        count_e += 1
                    count_n += 1
...
class WSModel(object):
    ...
    def read_model_file(self):
        ...
        n_north, n_east, n_z, n_res = np.array(mlines[1].strip().split(), dtype=np.int)
...
class WSModelManipulator(object):
    ...
    def __init__(self, model_fn=None, initial_fn=None, data_fn=None, **kwargs):
        ...
        self.initial_fn_basename = kwargs.pop(
            "initial_fn_basename", "WSInitialModel_mm"
        )
        ...
        if self.res_list is None:
            self.set_res_list(
                np.array([0.3, 1, 10, 50, 100, 500, 1000, 5000], dtype=np.float)
            )
            ...
            try:
                if len(self.res_list) > 10:
                    print(
                        "!! Warning -- ws3dinv can only deal with 10 "
    ...
    def read_file(self):
        ...
        # --> read initial file
            ...
            if len(wsmesh.res_list) == 1:
                self.set_res_list([0.3, 1, 10, 100, 1000])
            else:
    ...
    def plot(self):
        ...
        plot_north = (
            np.append(self.grid_north, self.grid_north[-1] * 1.25) / self.dscale
        )
        ...
        self.mesh_plot = self.ax1.pcolormesh(
            self.mesh_east,
            self.mesh_north,
            plot_res,
            cmap=self.cmap,
        ...
        self.cid_depth = self.mesh_plot.figure.canvas.mpl_connect(
            "key_press_event", self._on_key_callback
        )
        ...
        if self.xlimits is not None:
            ...
            self.ax1.set_xlim(
                xmin=self.grid_east.min() / self.dscale,
        ...
        if self.ylimits is not None:
            ...
            self.ax1.set_ylim(
                ymin=self.grid_north.min() / self.dscale,
        ...
        self.ax1.set_title(
            "Depth = {:.3f} ".format(depth_title) + "(" + self.map_scale + ")",
            fontdict=self.fdict,
        )
        ...
        # plot the colorbar
        self.ax2 = mcb.make_axes(self.ax1, orientation="vertical", shrink=0.35)
        seg_cmap = cmap_discretize(self.cmap, len(self.res_list))
        self.cb = mcb.ColorbarBase(
            self.ax2[0],
            cmap=seg_cmap,
            norm=colors.Normalize(vmin=self.cmin, vmax=self.cmax),
        )

        self.cb.set_label(
            "Resistivity ($\Omega \cdot$m)", fontdict={"size": self.font_size}
        )
    ...
    def _on_key_callback(self, event):
        ...
        if self.event_change_depth.key == "=":
            ...
            print(
                "Plotting Depth {0:.3f}".format(
                    self.grid_z[self.depth_index] / self.dscale
                )
                + "("
                + self.map_scale
                + ")"
        ...
        # copy the layer above
            ...
            try:
                if self.depth_index == 0:
                    ...
                    self.res_model[:, :, self.depth_index] = self.res_model[
                        :, :, self.depth_index - 1
                    ]
            except IndexError:
                print("No layers above")
        ...
        # undo
            ...
            if type(self.xchange) is int and type(self.ychange) is int:
                self.res_model[
                    self.ychange, self.xchange, self.depth_index
    ...
    def _get_north_index(self, y1, y2):
        ...
        if y1 < y2:
            ychange = np.where(
                (self.grid_north / self.dscale > y1)
    ...
    def convert_res_to_model(self, res_array):
        ...
        for ii, res in enumerate(self.res_list):
            ...
            if ii == 0:
                ...
                h_index = min([len(self.res_list) - 1, ii + 1])
                indexes = np.where(
                    (res_array > self.res_list[l_index])
    ...
    def rewrite_initial_file(self, save_path=None):
        ...
        self.new_initial_fn = os.path.join(self.save_path, self.initial_fn_basename)
        wsmesh = WSMesh()
...
class PlotResponse(object):
    ...
    def plot(self):
        ...
        self.data_object.read_data_file(self.data_fn, station_fn=self.station_fn)
        ...
        if self.resp_fn != None:
            ...
            if type(self.resp_fn) is not list:
                self.resp_object = [
                    WSResponse(self.resp_fn, station_fn=self.station_fn)
                ]
            else:
                for rfile in self.resp_fn:
                    self.resp_object.append(
                        WSResponse(rfile, station_fn=self.station_fn)
                    )
        ...
        if self.plot_z == True:
            ...
            h_ratio = [2, 1.5]
        gs = gridspec.GridSpec(2, 2, height_ratios=h_ratio, hspace=0.1)
        ...
        for jj in pstation_list:
            ...
            data_z_err[np.where(data_z_err == 7.95204e5 - 7.95204e5j)] = 1.0 + 1.0j

            # convert to apparent resistivity and phase
            z_object = mtz.Z(z_array=data_z, z_err_array=data_z_err, freq=1.0 / period)
            ...
            if self.plot_style == 1:
                if self.plot_component == 2:
                    ...
                    ax_list = [axrxx, axrxy, axryx, axryy, axpxx, axpxy, axpyx, axpyy]
                    ...
                    if len(ax_list) == 4:
                        ...
                        if aa == 0:
                            if self.plot_z == False:
                                ...
                                ax.set_ylabel("Re[Z (m/s)]", fontdict=fontdict)
                        elif aa == 2:
                            if self.plot_z == False:
                                ax.set_ylabel("Phase (deg)", fontdict=fontdict)
                            elif self.plot_z == True:
                                ax.set_ylabel("Im[Z (m/s)]", fontdict=fontdict)
                    #                        else:
            ...
            # plot xy and yx together and xx, yy together
                ...
                if self.plot_component == 2:
                    ...
                    label_list = [["$Z_{xy}$", "$Z_{yx}$"], ["$Z_{xx}$", "$Z_{yy}$"]]
                # set axis properties
                for aa, ax in enumerate(ax_list):
                    ax.tick_params(axis="y", pad=self.ylabel_pad)
                    if len(ax_list) == 2:
                        ...
                        if aa == 0:
                            ...
                            if self.plot_z == False:
                                ...
                                ax.set_ylabel("Re[Impedance (m/s)]", fontdict=fontdict)
                            if self.res_limits is not None:
                                ax.set_ylim(self.res_limits)
                        else:
                            ax.set_ylim(self.phase_limits)
                            if self.plot_z == False:
                                ...
                                ax.set_ylabel("Im[Impedance (m/s)]", fontdict=fontdict)
                    elif len(ax_list) == 4:
                        if aa < 2:
                            plt.setp(ax.get_xticklabels(), visible=False)
                            if self.plot_z == False:
            ...
            if plotr == True:
                for rr in range(nr):
                    ...
                    resp_z_object = mtz.Z(
                        z_array=resp_z, z_err_array=resp_z_err, freq=1.0 / period
                    )
...
class PlotDepthSlice(object):
    ...
    def __init__(
        self, model_fn=None, data_fn=None, station_fn=None, initial_fn=None, **kwargs
    ):
        self.model_fn = model_fn
        self.data_fn = data_fn
        self.station_fn = station_fn
    ...
    def read_files(self):
        ...
        if self.initial_fn is not None:
            if os.path.isfile(self.initial_fn) == True:
                ...
                for ii in range(len(wsmesh.res_list)):
                    self.res_model[np.where(wsmesh.res_model == ii + 1)] = rdict[ii + 1]
            else:
                raise mtex.MTpyError_file_handling(
        ...
        if self.initial_fn is None and self.model_fn is None:
            raise mtex.MTpyError_inputarguments(
                "Need to input either a model" " file or initial file."
            )
    ...
    def plot(self):
        ...
        for ii in zrange:
            ...
            cb.set_label(
                "Resistivity ($\Omega \cdot$m)", fontdict={"size": self.font_size + 1}
            )
...
class PlotPTMaps(mtplottools.MTEllipse):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.cb_pt_pad = kwargs.pop("cb_pt_pad", 0.90)
    ...
    def plot(self):
        ...
        if self.ew_limits == None:
            if self.station_east is not None:
                self.ew_limits = (
                    np.floor(self.station_east.min()) - self.pad_east,
        ...
        if self.ns_limits == None:
            if self.station_north is not None:
                self.ns_limits = (
                    np.floor(self.station_north.min()) - self.pad_north,
        ...
        for ff, per in enumerate(self.plot_period_list):
            ...
            if self.model_fn is not None:
                approx_depth, d_index = estimate_skin_depth(
                    self.res_model, self.grid_z, per, dscale=self.dscale
                )
                for ax in ax_list:
            ...
            # --> get phase tensors
            pt = mtpt.PhaseTensor(
                z_array=self.data[:, ff], freq=np.repeat(per, n_stations)
            )
            if self.resp is not None:
                mpt = mtpt.PhaseTensor(
                    z_array=self.resp[:, ff], freq=np.repeat(per, n_stations)
                )
            ...
            for jj in range(n_stations):
                ...
                ellipse = Ellipse(
                    (self.station_east[jj], self.station_north[jj]),
                    width=ewidth,
                    height=eheight,
                ...
                if self.resp is not None:
                    ...
                    eheight = (
                        mpt.phimin[0][jj] / mpt.phimax[0].max() * self.ellipse_size
                    )
                    ewidth = mpt.phimax[0][jj] / mpt.phimax[0].max() * self.ellipse_size
                    ...
                    eheight = (
                        rpt.phimin[0][jj] / rpt.phimax[0].max() * self.ellipse_size
                    )
                    ewidth = rpt.phimax[0][jj] / rpt.phimax[0].max() * self.ellipse_size
...
def estimate_skin_depth(res_model, grid_z, period, dscale=1000):
    ...
    skin_depth_period = np.array(
        [(zz / ds) ** 2 * (1 / rho_a) for zz, rho_a in zip(grid_z, apparent_res_xy)]
    )
    ...
    print(
        " estimated skin depth period    {0:.6g} (s)".format(
            skin_depth_period[period_index]
        )
    )
    print(
        " estimate apparent resisitivity {0:.0f} (Ohm-m)".format(
...
class PlotSlices(object):
    ...
    def __init__(
        self, model_fn, data_fn=None, station_fn=None, initial_fn=None, **kwargs
    ):
        self.model_fn = model_fn
        self.data_fn = data_fn
        self.station_fn = station_fn
        ...
        self.subplot_wspace = 0.20
        self.subplot_hspace = 0.30
        self.subplot_right = 0.98
        self.subplot_left = 0.08
    ...
    def plot(self):
        ...
        if self.z_limits == None:
            self.z_limits = (self.grid_z[0] - 5000 / self.dscale, self.grid_z[-5])
        ...
        self.mesh_ez_east, self.mesh_ez_vertical = np.meshgrid(
            self.grid_east, self.grid_z, indexing="ij"
        )
        self.mesh_nz_north, self.mesh_nz_vertical = np.meshgrid(
            self.grid_north, self.grid_z, indexing="ij"
        )
        self.mesh_en_east, self.mesh_en_north = np.meshgrid(
            self.grid_east, self.grid_north, indexing="ij"
        )
        ...
        # plot color bar
        cbx = mcb.make_axes(self.ax_map, fraction=0.15, shrink=0.75, pad=0.1)
        cb = mcb.ColorbarBase(
            cbx[0],
            cmap=self.cmap,
            norm=Normalize(vmin=self.climits[0], vmax=self.climits[1]),
    ...
    def _update_ax_ez(self):
        ...
        self.ax_ez.set_ylabel(
            "Depth ({0})".format(self.map_scale), fontdict=self.font_dict
        )
        self.ax_ez.set_xlabel(
            "Easting ({0})".format(self.map_scale), fontdict=self.font_dict
        )
        self.fig.canvas.draw()
        self._update_map()
    ...
    def _update_ax_nz(self):
        ...
        self.ax_nz.set_xlabel(
            "Northing ({0})".format(self.map_scale), fontdict=self.font_dict
        )
        self.ax_nz.set_ylabel(
            "Depth ({0})".format(self.map_scale), fontdict=self.font_dict
        )
        self.fig.canvas.draw()
        self._update_map()
    ...
    def _update_ax_en(self):
        ...
        self.ax_en.set_ylabel(
            "Northing ({0})".format(self.map_scale), fontdict=self.font_dict
        )
        self.ax_en.set_xlabel(
            "Easting ({0})".format(self.map_scale), fontdict=self.font_dict
        )
        # --> plot the stations
        if self.station_east is not None:
    ...
    def _update_map(self):
        ...
        # --> e-w indication line
        self.ax_map.plot(
            [self.grid_east[self.index_east], self.grid_east[self.index_east]],
            [self.grid_north.min(), self.grid_north.max()],
            lw=1,
            color="b",
        )
    ...
    def get_station_grid_locations(self):
        ...
        if self.station_east is not None:
            for ss, sx in enumerate(self.station_east):
                ...
                self.station_dict_east[self.grid_east[gx]].append(
                    self.station_north[ss]
                )

            for ss, sy in enumerate(self.station_north):
                gy = np.where(self.grid_north <= sy)[0][-1]
                self.station_dict_north[self.grid_north[gy]].append(
                    self.station_east[ss]
                )
        else:
            return
...
class WSStartup(object):
    ...
    def write_startup_file(self):
        ...
        if self.save_path == None and self.data_fn != None:
            self.startup_fn = os.path.join(os.path.dirname(self.data_fn), "startup")
        elif os.path.isdir(self.save_path) == True:
        ...
        if os.path.dirname(self.startup_fn) == os.path.dirname(self.data_fn):
            slines.append(
                "{0:<20}{1}\n".format("DATA_FILE", os.path.basename(self.data_fn))
            )
        ...
        if os.path.dirname(self.startup_fn) == os.path.dirname(self.initial_fn):
            slines.append(
                "{0:<20}{1}\n".format(
                    "INITIAL_MODEL_FILE", os.path.basename(self.initial_fn)
                )
            )
        else:
            slines.append("{0:<20}{1}\n".format("INITIAL_MODEL_FILE", self.initial_fn))
        slines.append("{0:<20}{1}\n".format("PRIOR_MODEL_FILE", self.apriori_fn))
        slines.append("{0:<20}{1}\n".format("CONTROL_MODEL_INDEX ", self.control_fn))
        slines.append("{0:<20}{1}\n".format("TARGET_RMS", self.target_rms))
        slines.append("{0:<20}{1}\n".format("MAX_NO_ITERATION", self.max_iter))
        slines.append(
        ...
        slines.append("{0:<20}{1} \n".format("ERROR_TOL_LEVEL", self.error_tol))
...
def write_vtk_res_model(res_model, grid_north, grid_east, grid_z, save_fn):
    ...
    **Doesn't work properly under windows**
    ...
    save_fn = gridToVTK(
        save_fn, grid_north, grid_east, grid_z, cellData={"resistivity": res_model}
    )
