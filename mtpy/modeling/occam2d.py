...
class Setup():
    ...
    def __init__(self, configfile=None, **input_parameters):
        ...
        # list of arrays containing 2 columns, distance x along profile and
        # depth z of interface
        self.parameters_prejudice['interfaces'] = []
        ...
        if (edi_dir is not None):
            ...
            if (edi_dir):
                ...
                print 'Warning - given directory not found: {0} \n\t-'\
                    ' using current directory instead:{1}'.format(
                        edi_dir, os.curdir)

        edilist_raw = fnmatch.filter(os.listdir(
            self.edi_directory), '*.[Ee][Dd][Ii]')
        edilist_full = [op.abspath(op.join(self.edi_directory, i))
                        for i in edilist_raw]
        edilist = []
        for edi in edilist_full:
            if op.isfile(edi):
                edilist.append(edi)
    ...
    def write_datafile(self):
        ...
        try:
            data_object = Data(edilist=self.edifiles,
                               wd=self.wd, **self.parameters_data)
        except:
    ...
    def setup_mesh_and_model(self):
        ...
        # maximum width of MODEL block - implicitely defines finiteness of the
        # mesh
        ...
        first_layer_thickness = float(self.parameters_inmodel[
                                      'firstlayer_thickness'])
        # so "layers_per_decade" layers between "first_layer_thickness" and 10* "first_layer_thickness"
        # then the same number between 10* "first_layer_thickness" and 100*
        ...
        n_bottompadding = int(
            float(self.parameters_inmodel['no_bottomlayerelements']))
        bottompadding_scalefactor = self.parameters_inmodel[
            'bottompadding_scalefactor']

        # number of padding mesh columns to the sides
        n_sidepadding = int(
            float(self.parameters_inmodel['no_sideblockelements']))
        nmerge_sidepadding = int(
            float(self.parameters_inmodel['no_mergedsideblockelements']))

        if (n_sidepadding - nmerge_sidepadding) % 2 == 1:
            nmerge_sidepadding += 1
        sidepadding_increasefactor = float(
            self.parameters_inmodel['sidepadding_increasefactor'])
        ...
        totalmodelblocknumber = 4 + totalstations + \
            (n_sidepadding - nmerge_sidepadding) / 2
        ...
        for idx, station in enumerate(lo_allsites):
            ...
            meshnodelocations.append(
                station + (lo_allsites[idx + 1] - station) / 2.)
        ...
        # add 2 side padding blocks with expon. increasing width of N mesh
        # cells
        padding_absolute = 0
        padding_nmerge = 0

        for p in range(n_sidepadding):
            current_padding = sidepadding_increasefactor**(
                p + 1) * paddingwidth
        ...
        for loc in range(len(meshnodelocations) - 1):
            lo_meshblockwidths.append(
                meshnodelocations[loc + 1] - meshnodelocations[loc])

        # 5. build top layer modelblocks by merging paddings and then 2 blocks
        # each:
        lo_columns_to_merge = []
        lo_modelblockwidths = []
        ...
        # merge the extra padding cells that haven't been merged into a big
        # block at the edge
        ...
        log_part_thickness = model_depth_m - \
            (n_layers - 1) * first_layer_thickness
        ...
        lo_model_depths.append(
            lo_model_depths[-1] + n_bottompadding * max_thickness)
...
#        self.parameters_inmodel['no_layers'] = len(lo_model_depths)
        ...
        for layer_idx, thickness in enumerate(lo_model_thicknesses):
            ...
            while block_idx + 1 < ncol - 1:
                ...
                # neighboring blocks:
                    ...
                    lo_modelblockwidths[
                        block_idx] += lo_modelblockwidths[block_idx + 1]
                    lo_columns_to_merge[
                        block_idx] += lo_columns_to_merge[block_idx + 1]
    ...
    def write_inmodelfile(self):
        ...
        temptext = "Model Name:       {0}\n".format(
            self.parameters_inmodel['model_name'])
        model_outstring += temptext
    ...
    def write_startupfile(self):
        ...
        temptext = "Format:           {0}\n".format(
            self.parameters_startup['iter_format'])
        startup_outstring += temptext

        temptext = "Description:      {0}\n".format(
            self.parameters_startup['description'])
        startup_outstring += temptext
        ...
        temptext = "Date/Time:        {0}\n".format(
            self.parameters_startup['datetime_string'])
        startup_outstring += temptext

        temptext = "Max Iter:         {0}\n".format(
            int(float(self.parameters_startup['max_no_iterations'])))
        startup_outstring += temptext

        temptext = "Target Misfit:    {0:.2f}\n".format(
            float(self.parameters_startup['target_rms']))
        startup_outstring += temptext

        temptext = "Roughness Type:   {0}\n".format(
            int(self.parameters_startup['roughness_type']))
        startup_outstring += temptext

        temptext = "Debug Level:      {0}\n".format(
            int(self.parameters_startup['debug_level']))
        startup_outstring += temptext

        temptext = "Iteration:        {0}\n".format(
            int(self.parameters_startup['no_iteration']))
        startup_outstring += temptext

        temptext = "Lagrange Value:   {0}\n".format(
            self.parameters_startup['mu_start'])
        startup_outstring += temptext

        temptext = "Roughness Value:  {0}\n".format(
            self.parameters_startup['roughness_start'])
        startup_outstring += temptext

        temptext = "Misfit Value:     {0}\n".format(
            float(self.parameters_startup['rms_start']))
        startup_outstring += temptext

        temptext = "Misfit Reached:   {0}\n".format(
            int(self.parameters_startup['reached_misfit']))
        startup_outstring += temptext
        ...
        if type(self.parameters_startup['halfspace_resistivity']) not in [list, np.ndarray]:
            for l in range(self.no_parameters):
                temptext += "{0:.1g}  ".format(
                    np.log10(float(self.parameters_startup['halfspace_resistivity'])))
                counter += 1
                if counter == 20:
                    temptext += '\n'
                    counter = 0
        else:
            temptext += '  '.join([str(res)
                                   for res in self.parameters_startup['halfspace_resistivity']])
    ...
    def get_modelblock_info(self):
        ...
        blockmerges_x = [[int(val) for val in line.strip('\n').split()]
                         for line in self.parameters_inmodel['lo_modelblockstrings']]
...
#        meshlocations_z.insert(0,0)
        ...
        block_nums_array = np.zeros(
            (len(self.meshlocations_z) - 1, len(self.meshlocations_x) - 1))
    ...
    def project_interface(self, fn, skiprows=1, fmt='xy'):
        ...
        xp = self.Data.profile_origin[0] + \
            self.modelblocklocations_x[0] * np.cos(az)
        yp = self.Data.profile_origin[1] + \
            self.modelblocklocations_x[0] * np.sin(az)
    ...
    def build_roughness_exceptions(self):
        ...
        if not hasattr(self, 'interfaces'):
            ...
            block_z = (self.modelblocklocations_z[
                       1:] + self.modelblocklocations_z[:-1]) / 2.
            ...
            for interface in self.interfaces:
                ...
                for i in range(len(interface)):
                    # get out first block for which z_msl value is greater than
                    # z location of block
                    z = block_z[block_z - z_msl[i] < 0][-1]
                    # get the index j for the z value
                    j = list(block_z).index(z)

                    # define criteria to find closest x block
                    crit = abs(block_x[j] - x_vals[i]
                               ) == min(abs(block_x[j] - x_vals[i]))
    ...
    def project_well_loc(self):
        ...
        for well_xy in self.well_xy:
            ...
            # project drillhole location onto profile and get distance along
            # profile
    ...
    def setup_prejudice(self):
        ...
        if self.parameters_prejudice['interface_filelist'] is not None:
            for intfile in self.parameters_prejudice['interface_filelist']:
                ...
                self.parameters_prejudice[
                    'interfaces'].append(np.vstack([ld, zd]).T)
    ...
    def write_prejudicefile(self):
        ...
        data = np.vstack([np.hstack(self.block_nums),
                          np.log10(prejvals), weights]).T
        ...
        np.savetxt(op.join(self.wd, self.prejudicefile), data, fmt=[
                   '%1i', '%.3f', '%.3f'], header=header, comments='')
...
class Prejudice():
    ...
    def __init__(self, Setup, **input_parameters):
        ...
        # list (in order of increasing depth) of arrays containing 2 columns,
        self.interfaces = None
        # distance x along profile and depth z of interface
        # list of prejudice resistivities, 1 longer than interfaces. in same
        # order as interfaces
        self.interface_resistivity_values = None
    ...
    def assign_interface_prejudice(self):
        ...
        if ((self.interfaces is None) or (self.interface_resistivity_values is None)):
            ...
            self.interface_resistivity_values = [
                max(1e-6, rv) for rv in self.interface_resistivity_values]
            # initialise a prejudice array. Starting resistivity is the value
            # for the 1st layer then successively move down the model
            prejudice_array = np.ones_like(
                self.block_nums_array) * self.interface_resistivity_values[0]
...
#            blockx,blockz = self.modelblocklocations_x,self.modelblocklocations_z
            meshx, meshz = np.meshgrid(
                self.meshlocations_x, self.meshlocations_z)
...
#                    print zc0,zc1
                    ...
                    # otherwise use the interpolation function to get value at
                    # the mesh node
                    else:
                        zval = f(meshx[0][ii])
#                        print blockx[jj][ii],blockx[jj][ii+1]
                ...
                prejudice_array[
                    meshz > zline] = self.interface_resistivity_values[i + 1]
                weights_array[
                    meshz > zline] = self.interface_prejudice_weights[i + 1]
            ...
            for jj in range(len(prejudice)):
                for ii in range(len(prejudice[jj])):
                    #                    print prejudice[jj]
                    # print
                    # self.block_nums[jj][ii],prejudice_array[self.block_nums_array==self.block_nums[jj][ii]]
                    prejudice[jj][ii] = np.median(
                        prejudice_array[self.block_nums_array == self.block_nums[jj][ii]])
                    weights[jj][ii] = np.median(
                        weights_array[self.block_nums_array == self.block_nums[jj][ii]])
    ...
    def subsample_welldata(self):
        ...
        blockmerges_z = self.parameters_inmodel['lo_merged_lines']
        blockedges_z = [0] + [self.meshlocations_z[i - 1]
                              for i in [int(sum(blockmerges_z[:ii])) for ii in range(1, len(blockmerges_z) + 1)]]
        ...
        for w in range(len(self.welldata)):
            ...
            for i in range(1, len(blockedges_z)):
                ...
                if len(temp_res) > 0:
                    self.prejudice_resistivity[-1].append(
                        np.around(np.median(temp_res), 1))
    ...
    def get_blocknums_from_wells(self):
        ...
        for w in range(len(res_values)):
            ...
            for i in range(len(res_values[w])):
                ...
                col = list(abs(modc - distance)
                           ).index(np.min(abs(modc - distance)))
                # append block nums
                ...
                self.prejudice_fstrings.append(' '.join(['%01i' % blocknum,
                                                         '%.03f' % np.log10(
                                                             self.prejudice_resistivity[w][i]),
        ...
        if no_freqs_max is not None:
            ...
            if no_freqs_max < len(lo_all_freqs_tmp):
                ...
                if excess < 2:
                    ...
                    offset = stepsize / 2.
                indices = np.array(np.around(np.linspace(offset, len(
                    lo_all_freqs_tmp) - 1 - offset, no_freqs_max), 0))
                ...
                for mode in lo_modes:
                    ...
                    if mode in [9, 1, 2]:
                        ...
                        try:
                            if raw_rho_value == 0:
                                raise
                            relative_rho_error = np.abs(
                                absolute_rho_error / raw_rho_value)
                        except:
                        ...
                        if mode == 9:
                            if self.rho_errorfloor is not None:
                                if self.rho_errorfloor / 100. > relative_rho_error:
                                    relative_rho_error = self.rho_errorfloor / 100.
                            # relative_error/np.log(10.)
                            error = np.abs(relative_rho_error * raw_rho_value)
                            ...
                            if relative_rho_error >= 2.:
                                ...
                                # relative_error*100.*0.285
                                error = np.degrees(
                                    np.arcsin(0.5 * relative_rho_error))
                    ...
                    if mode in [10, 5, 6]:
                        ...
                        if T.tipper is None:
                            # print 'no Tipper data for {0} Hz at station
                            # {1}'.format(freq, station_number)
                            continue
                        ...
                        try:
                            ...
                            # print 'no Tipper error for station {0}/frequency
                            # {1}'.format(station_number,frequency_number)
                            tipper_err = None
                        ...
                        if tipper_err is None:
                            ...
                            if self.tipper_errorfloor is not None:
                                raw_error = np.abs(
                                    (self.tipper_errorfloor / 100.) * value)
                        else:
                        ...
                        # set error floor
                        if self.tipper_errorfloor is not None:
                            error = max(error, np.abs(
                                (self.tipper_errorfloor / 100.) * value))

                        # set an absolute minimum tipper error to apply when
                    ...
                    if append:
                        self.data.append(
                            [station_number, frequency_number, mode, value, np.abs(error)])
    ...
    def generate_profile(self):
        ...
        for edifile in self.edilist:
            ...
            if self.strike is None:
                try:
                    lo_strike_angles.extend(list(MTgy.strike_angle(
                        edi.Z.z[np.where(MTgy.dimensionality(edi.Z.z) != 1)])[:, 0] % 90))
                except:
                    pass
        ...
        for idx, zone in enumerate(utmzones):
            if zone == main_utmzone:
                continue
            utm = MTcv.LLtoUTM(23, self.station_coords[idx][
                               0], self.station_coords[idx][1], main_utmzone)
        ...
        # rotate tipper to profile azimuth, not strike. Need angle to be
        # between
        ...
        for idx, sta in enumerate(self.stations):
            station_vector = np.array(
                [lo_easts[idx], lo_norths[idx] - profile_line[1]])
            ...
            projected_stations.append(
                [position[0], position[1] + profile_line[1]])
        ...
        self.station_frequencies = [
            self.station_frequencies[i] for i in profile_idxs]
        ...
        if 0:
            ...
            lax.scatter(projected_stations[:, 0], projected_stations[
                        :, 1], color='r', marker='x')
    ...
    def writefile(self, filename=None):
        ...
        if filename is not None:
            try:
                ...
                self.datafile = op.abspath(
                    op.join(self.wd, 'OccamDataFile.dat'))
