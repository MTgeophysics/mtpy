...
class ZongeMTFT:
    ...
    def __init__(self):
        ...
        self.MTFT_BandFrqMin = [7.31000e-4, 7.31000e-4, 7.31000e-4, 0.25, 0.25, 0.25, 1]
        ...
        self.Chn_dict = dict(
            [
                (chkey, [cid, cg, cl])
                for chkey, cid, cg, cl in zip(
                    self.Chn_Cmp, self.Chn_ID, self.Chn_Gain, self.Chn_Length
                )
            ]
        )
    ...
    def make_value_dict(self):
        ...
        """
        self.value_lst = [
            self.__dict__[key.replace(".", "_")] for key in self.meta_keys
        ]

        self.meta_dict = dict(
            [(mkey, mvalue) for mkey, mvalue in zip(self.meta_keys, self.value_lst)]
        )
    ...
    def get_rr_ts(self, ts_info_lst, remote_path=None):
        ...
        new_ts_info_lst = []
        rrfnlst = [
            rrfn for rrfn in os.listdir(self.Remote_Path) if rrfn.find(".cac") > 0
        ]

        for ts_dict in ts_info_lst:
            local_zc = zen.ZenCache()
            local_zc.read_cache_metadata(
                os.path.join(self.cache_path, ts_dict["LocalFile"])
            )
            try:
                local_start_date = local_zc.meta_data["DATA.DATE0"][0]
                local_start_time = local_zc.meta_data["DATA.TIME0"][0].replace(":", "")
            except KeyError:
                lsd_lst = local_zc.meta_data["DATE0"][0].split("/")
                local_start_date = "20{0}-{1}-{2}".format(
                    lsd_lst[2], lsd_lst[0], lsd_lst[1]
                )

                local_start_time = local_zc.meta_data["TIMEO"][0].replace(":", "")
            ...
            self.log_lines.append(
                "{0} {1} {2} \n".format(
                    ts_dict["LocalFile"], local_start_date, local_start_time
                )
            )
            ...
            for rrfn in rrfnlst[::-1]:
                ...
                remote_zc.read_cache_metadata(os.path.join(self.Remote_Path, rrfn))
                ...
                try:
                    ...
                    remote_start_date = remote_zc.meta_data["DATE0"][0].replace(
                        "/", "-"
                    )
                    remote_start_time = remote_zc.meta_data["TIMEO"][0].replace(":", "")
                remote_df = remote_zc.meta_data["TS.ADFREQ"][0]
                remote_npts = int(remote_zc.meta_data["TS.NPNT"][0])

                if (
                    local_start_date == remote_start_date
                    and local_df == remote_df
                    and local_start_time[0:2] == remote_start_time[0:2]
                ):
                    print(rrfn, remote_start_date, remote_start_time)
                    self.log_lines.append(
                        "{0} {1} {2}\n".format(
                            rrfn, remote_start_date, remote_start_time
                        )
                    )
                    ...
                    if local_start_time == remote_start_time:
                        if local_npts == remote_npts:
                            ...
                            for ii in range(self.num_comp + 1, self.num_comp + 3):
                            ...
                            shutil.copy(
                                ...
                                os.path.join(self.new_remote_path, rrfn),
                            )
                            rrfind = True
                            break
                        ...
                        # if time series is longer than remote reference
                            ...
                            local_zc.ts = np.resize(
                                local_zc.ts, (remote_npts, local_zc.ts.shape[1])
                            )

                            # reset some meta data
                            local_zc.meta_data["TS.NPNT"] = [str(local_zc.ts.shape[0])]

                            print(
                                "Resized Local TS in {0} to {1}".format(
                                    os.path.join(self.cache_path, ts_dict["LocalFile"]),
                                    local_zc.ts.shape,
                                )
                            )
                            self.log_lines.append(
                                "Resized Local TS in {0} to {1}\n".format(
                                    os.path.join(self.cache_path, ts_dict["LocalFile"]),
                                    local_zc.ts.shape,
                                )
                            )
                            ...
                            ts_dict["LocalFile"] = os.path.basename(local_zc.save_fn_rw)
                            ts_dict["RemoteFile"] = rrfn
                            ts_dict["RemoteBlock"] = ts_dict["LocalBlock"]
                            ts_dict["RemoteByte"] = ts_dict["LocalByte"]
                        ...
                        # if remote reference is longer than time series
                            ...
                            remote_zc.read_cache(os.path.join(self.Remote_Path, rrfn))
                            # resize remote ts accordingly
                            remote_zc.ts = np.resize(
                                remote_zc.ts, (local_npts, remote_zc.ts.shape[1])
                            )
                            # reset some meta data
                            ...
                            print(
                                "Resized Remote TS in {0} to {1}".format(
                                    os.path.join(self.Remote_Path, rrfn),
                                    remote_zc.ts.shape,
                                )
                            )
                            self.log_lines.append(
                                "Resized Remote TS in {0} to {1}\n".format(
                                    os.path.join(self.Remote_Path, rrfn),
                                    remote_zc.ts.shape,
                                )
                            )
                            ...
                            # reset some of the important parameters
                            ts_dict["RemoteFile"] = os.path.basename(
                                remote_zc.save_fn_rw
                            )
                            ts_dict["RemoteBlock"] = ts_dict["LocalBlock"]
                            ts_dict["RemoteByte"] = ts_dict["LocalByte"]
                    ...
                    # if the starting time is different
                    elif abs(int(local_start_time) - int(remote_start_time)) < int(
                        tdiff
                    ):
                        ...
                        if time_diff > 0:
                            ...
                            print(("Time difference is {0} seconds".format(time_diff)))
                            self.log_lines.append(
                                "Time difference is {0} seconds\n".format(time_diff)
                            )
                            print(
                                "Skipping {0} points in {1}".format(
                                    skip_points,
                                    os.path.join(self.cache_path, ts_dict["LocalFile"]),
                                )
                            )
                            self.log_lines.append(
                                "Skipping {0} points in {1}\n".format(
                                    skip_points,
                                    os.path.join(self.cache_path, ts_dict["LocalFile"]),
                                )
                            )
                            ...
                            local_zc.meta_data["DATA.TIME0"] = [
                                "{0}:{1}:{2}".format(
                                    local_hour + int(hour_diff / 3600.0),
                                    local_minute + int(minute_diff / 60.0),
                            ...
                            if remote_zc.ts.shape[0] < local_zc.ts.shape[0]:
                                print(
                                    "{0} local_npts > remote_npts {0}".format("*" * 4)
                                )
                                self.log_lines.append(
                                    "{0} local_npts > remote_npts {0}\n".format("*" * 4)
                                )
                                ...
                                local_zc.ts = np.resize(
                                    local_zc.ts,
                                    (remote_zc.ts.shape[0], local_zc.ts.shape[1]),
                                )
                                ...
                                print(
                                    "Resized Local TS in {0} to {1}".format(
                                        os.path.join(
                                            self.cache_path, ts_dict["LocalFile"]
                                        ),
                                        local_zc.ts.shape,
                                    )
                                )
                                ...
                                # reset some of the important parameters
                                ts_dict["LocalFile"] = os.path.basename(
                                    local_zc.save_fn_rw
                                )
                                ts_dict["RemoteFile"] = rrfn
                                ts_dict["RemoteBlock"] = ts_dict["LocalBlock"]
                            ...
                            # than the remote reference.
                                ...
                                remote_zc.ts = np.resize(
                                    remote_zc.ts,
                                    (local_zc.ts.shape[0], remote_zc.ts.shape[1]),
                                )
                                # reset some meta data
                                remote_zc.meta_data["TS.NPNT"] = [
                                    str(remote_zc.ts.shape[0])
                        ...
                        # local start time is later than remote start time
                            ...
                            print(
                                "Skipping {0} points in {1}".format(
                                    skip_points, os.path.join(self.Remote_Path, rrfn)
                                )
                            )
                            self.log_lines.append(
                                "Skipping {0} points in {1}\n".format(
                            ...
                            remote_zc.meta_data["DATA.TIME0"] = [
                                "{0}:{1}:{2}".format(
                                    rr_hour - int(hour_diff / 3600.0),
                                    rr_minute - int(minute_diff / 60.0),
            ...
            if rrfind == False:
                print(
                    (
                        "Did not find remote reference time series "
            ...
            for cfn in os.listdir(cache_path):
                if cfn[-4:] == ".cac" and cfn.find("$") == -1:
                    ...
                    if info_dict["Date"].find("/") >= 0:
                        ...
                        info_dict["Date"] = "20{0}-{1}-{2}".format(
                            dlst[2], dlst[0], dlst[1]
                        )
                    info_dict["Time0"] = "0"
                    # try:
    ...
    def set_remote_reference_info(self, remote_path):
        ...
        for setup_dict in self.setup_lst:
            ...
            setup_dict["ts_info_keys"] += [
                "ChnGain{0}".format(ii) for ii in range(num_comp - 1, num_comp + 1)
            ]
    ...
    def get_survey_info(self, survey_file, station_name, rr_station_name=None):
        ...
        for setup_dict in self.setup_lst:
            ...
            try:
                ...
                try:
                    setup_dict["chn_dict"]["Ex"][2] = survey_dict["e_xaxis_length"]
                except KeyError:
                    print("No ex data")
                    self.log_lines.append("No ex data from survey file.\n")
                try:
                    setup_dict["chn_dict"]["Ey"][2] = survey_dict["e_yaxis_length"]
                except KeyError:
                    print("No ey data")
                ...
                print(
                    (
                        "Could not find survey information from "
    ...
    def write_mtft_cfg(
        ...
        if remote_path is not None:
            if rrstation is None:
                rrstation = os.path.basename(
                    os.path.dirname(os.path.dirname(remote_path))
                )
        ...
        for ii, mkey in enumerate(self.meta_keys[:-1]):
            ...
            if type(self.meta_dict[mkey]) is list:
                cfid.write(
                    "${0}={1}\n".format(
                        mkey,
                        ",".join(["{0}".format(mm) for mm in self.meta_dict[mkey]]),
                    )
                )
            else:
        ...
        for setup_dict in self.setup_lst:
            ...
            setup_dict["Chn.Gain"] = [
                setup_dict["chn_dict"][ckey][1] for ckey in setup_dict["Chn.Cmp"]
            ]
            setup_dict["Chn.ID"] = [
                setup_dict["chn_dict"][ckey][0] for ckey in setup_dict["Chn.Cmp"]
            ]
            setup_dict["Chn.Length"] = [
                setup_dict["chn_dict"][ckey][2] for ckey in setup_dict["Chn.Cmp"]
            ]
            ...
            for ii, mkey in enumerate(self.setup_keys):
                ...
                if type(setup_dict[mkey]) is list:
                    cfid.write(
                        "${0}={1}\n".format(
                            mkey,
                            ",".join(["{0}".format(mm) for mm in setup_dict[mkey]]),
                        )
                    )
                else:
        ...
        ts_key_len = np.array([len(sd["ts_info_keys"]) for sd in self.setup_lst])
...
class ZongeMTEdit:
    ...
    def __init__(self):
        ...
        self.ZPhzXYmin = [-3150.0] * self.num_freq
        self.ZPhzXYmax = [3150.0] * self.num_freq

        self.AResYXmin = [1.0e-2] * self.num_freq
        self.AResYXmax = [1.0e6] * self.num_freq
        self.ZPhzYXmin = [-3150.0] * self.num_freq
        self.ZPhzYXmax = [3150.0] * self.num_freq
    ...
    def make_meta_dict(self):
        ...
        self.meta_dict = dict(
            [(mkey, mvalue) for mkey, mvalue in zip(self.meta_keys, self.meta_lst)]
        )
    ...
    def make_param_dict(self):
        ...
        self.param_dict = dict(
            [(mkey, mvalue) for mkey, mvalue in zip(self.param_header, self.param_lst)]
        )
    ...
    def read_config(self, cfg_fn):
        ...
        for ii, cline in enumerate(clines):
            ...
            # --> get filter parameters header
                ...
                self.param_dict = dict([(pkey, np.zeros(nparams)) for pkey in pkeys])
                jj = 0
        ...
        for ii in range(self.num_freq):
            for jj, pkey in enumerate(self.param_header[:-1]):
                cfid.write(
                    "{0:>11},".format(
                        "{0:{1}}".format(
                            self.param_dict[pkey][ii], self.string_fmt_lst[jj]
                        )
                    )
                )
            cfid.write(
    ...
    def read_avg_file(self, avg_fn):
        ...
        if not self.comp_dict:
            ...
            if len(alines) > 140:
                self.comp_dict = dict(
                    [
                        (ckey, np.zeros(int(len(alines) / 4), dtype=self.info_dtype))
            ...
            # if there are only 2
                ...
                self.comp_dict = dict(
                    [
                        (ckey, np.zeros(int(len(alines) / 2), dtype=self.info_dtype))
                        for ckey in list(self.comp_flag.keys())
                    ]
                )
        self.comp_lst_z = []
