...
class ZenBIRRP():
    ...
    def get_survey_parameters(self, survey_file=None, rrstation=None):
        """
        ...
        if self.rrstation is not None:
            for sdict in survey_lst:
                if sdict['station'] == self.rrstation:
                    sdict['sampling_rate'] = self.df
                    scfg_fn = os.path.join(self.station_path, 
                                           'survey_param.cfg') 
                    mtcf.write_dict_to_configfile({self.station:self.survey_dict, 
                                                   'RR_'+self.rrstation:sdict}, 
                                                  scfg_fn)
                    print 'Wrote survey parameters to: {0}'.format(scfg_fn)
                    self.survey_config_fn = scfg_fn
                    self.rrsurvey_dict = sdict
                
                elif sdict['station'] == self.station:
                    sdict['sampling_rate'] = self.df
                    scfg_fn = os.path.join(self.station_path, 'survey_param.cfg') 
                    mtcf.write_dict_to_configfile({self.station:sdict}, scfg_fn)
                    print 'Wrote survey parameters to: {0}'.format(scfg_fn)
                    self.survey_config_fn = scfg_fn
                    self.survey_dict = sdict
        else:
            for sdict in survey_lst:
    ...
    def write_script_file(self, df=None, processing_file=None,
                          ...
                          ncomps=5, jmode=0, survey_file=None):
        """
        ...
        cfg_fn = mtfh.make_unique_filename('{0}_birrp_params.cfg'.format(script_file[:-7]))
        print cfg_fn        
        mtcf.write_dict_to_configfile(birrp_dict, cfg_fn)
        print 'Wrote BIRRP config file for edi file to {0}'.format(cfg_fn)
        
        self.birrp_config_fn = script_file[:-6]+'cfg'
