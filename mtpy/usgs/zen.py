...
class Zen3D(object):
    ...
    * gps_dtype is formated as np.dtype([('flag0', np.int32),
    ...
    def read_z3d(self):
        ...
        # get the file size to get an estimate of how many data points there
        # are
    ...
    def validate_gps_time(self):
        ...
        for ii in range(len(t_diff) - 1):
            t_diff[ii] = self.gps_stamps['time'][
                ii] - self.gps_stamps['time'][ii + 1]
...
class Zen3D_old(object):
                ...
                for ll in hline:
                    if ll.find('builddate') > 0:
                        ...
                        for kk in hlist:
                            klist = kk.split(':')
                            header_dict[
                                klist[0].strip().lower()] = klist[1].strip()
                    else:
                        hlist = ll.split(':')
                        try:
                            header_dict[
                                hlist[0].strip().lower()] = hlist[1].strip()
                        except IndexError:
                            pass
    ...
    def read_3d(self):
        ...
        # check for any spots where gps was not locked or mised a sampling
        # interval
        ...
        for ll, kk in enumerate(gps_list[0:-1]):
            pdiff = (
                (gps_list[ll + 1] - (kk + self._stamp_len)) - (df * 4)) / 4
    ...
    def get_gps_stamp(self, gps_index):
        ...
        try:
            ...
            while gps_info['time'] < 0:
                gps_index = self.get_gps_stamp_location(
                    start_index=gps_index + 7)
...
class ZenCache(object):
    ...
    def read_cache(self, cache_fn):
        ...
        self.ts = np.resize(
            self.ts, (int(self.ts.shape[0] / num_chn), num_chn))
...
class Z3D_to_edi(object):
    ...
    def run_birrp(self, script_fn_list=None, birrp_exe=None):
        ...
        if script_fn_list is None:
            raise IOError(
                'Need to input a script file or list of script files')

        if birrp_exe is not None:
            self.birrp_exe = birrp_exe

        if isinstance(script_fn_list, list):
            self.edi_fn = []
        ...
        elif isinstance(script_fn_list, str):
    ...
    def plot_responses(self, edi_fn_list=None):
        ...
        if isinstance(self.edi_fn, list):
            ...
            resp_plot = plotnresponses.PlotMultipleResponses(fn_list=self.edi_fn,
                                                             plot_style='compare',
                                                             plot_tipper='yri')
        elif isinstance(self.edi_fn, str):
    ...
    def process_data(self, df_list=None, max_blocks=2, num_comp=5):
        ...
        if df_list is not None:
            if isinstance(df_list, float) or isinstance(df_list, int) or\
               isinstance(df_list, str):
                df_list = [df_list]
