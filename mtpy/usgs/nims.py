...
class NIMS(NIMSHeader):
    ...
    def read_nims(self, fn=None):
        ...
        ### get data
        data_array = np.zeros(
            data.shape[0] * self.sampling_rate,
            dtype=[
                ("hx", np.float),
                ("hy", np.float),
                ("hz", np.float),
        ...
        self.stamps = self.match_staus_with_gps_stamps(
    ...
    def align_data(self, data_array, stamps):
        ...
        dt_index = self.make_dt_index(
            start_time.isoformat(), self.sampling_rate, n_samples=data_array.shape[0]
        )
