...
class MTTS(object):
    ...
    def sampling_rate(self):
        ...
        if self._check_for_index():
            if isinstance(self._ts.index[0], int):
                ...
                sr = 1e9 / float(
                    (self._ts.index[1:] - self._ts.index[:-1]).unique()[0].to_numpy()
                )
        else:
            sr = self._sampling_rate
        ...
        if self._check_for_index():
            if isinstance(self._ts.index[0], int):
                ...
                if (
                    1e9
                    / float(
                        (self._ts.index[1:] - self._ts.index[:-1])
                        .unique()[0]
                        .to_numpy()
                    )
                    == self._sampling_rate
    ...
    def resample(self, new_sample_rate):
        ...
        ndf = "{0:.0f}N".format(1.0 / (new_sample_rate) * 1e9)
    ...
    def write_ascii_file(self, fn_ascii, chunk_size=4096):
        ...
        # make header lines
        header_lines = [f"# *** MT time series text file for {self.station} ***"]
        header_lines += [
            f"# {attr} = {getattr(self, attr)}" for attr in sorted(self._attr_list)
        ]
        header_lines += ["# *** time_series ***\n"]

        # write to file in chunks
        with open(fn_ascii, "w") as fid:
            # np.savetxt(fid, self.ts.data, header="\n".join(header_lines), fmt="%16.10e")
