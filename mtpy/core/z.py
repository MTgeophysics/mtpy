    ...
    def set_res_phase(
        ...
        if np.linalg.norm(np.imag(res_array)) != 0:
            raise MTex.MTpyError_inputarguments(
                'Error - array "res" is not' + "real valued !"
            )

        if np.linalg.norm(np.imag(phase_array)) != 0:
            raise MTex.MTpyError_inputarguments(
                'Error - array "phase" is' + "not real valued !"
            )
...
class Z(ResPhase):
        ...
        if self.z is not None:
            if len(self.z.shape) == 3:
                if self._freq.size != len(self.z):
                    self._logger.warn(
                        "length of freq list/array not correct"
                        "({0} instead of {1})".format(self._freq.size, len(self.z))
                    )
                    return
                else:
                    try:
    ...
    def z(self, z_array):
        ...
        try:
            ...
            try:
                ...
                self._logger.error(
                    "provided Z array does not have correct dimensions- Z unchanged"
                )
        ...
        for idx_f in range(len(inverse)):
            try:
                ...
                raise MTex.MTpyError_Z(
                    "The {0}ith impedance".format(idx_f + 1)
    ...
    def rotate(self, alpha):
        ...
        if len(lo_angles) != len(self.z):
            self._logger.warn(
                'Wrong number of "angles" - I need {0}'.format(len(self.z))
            )
    ...
    def remove_ss(self, reduce_res_factor_x=1.0, reduce_res_factor_y=1.0):
        ...
        if len(lo_x_factors) != len(self.z):
            self._logger.error(
                "Wrong number Number of reduce_res_factor_x - need {0}".format(
                    len(self.z)
                )
            )
            return
        ...
        if len(lo_y_factors) != len(self.z):
            self._logger.error(
                'Wrong number Number of "reduce_res_factor_y"'
    ...
    def remove_distortion(self, distortion_tensor, distortion_err_tensor=None):
        ...
        try:
            ...
            if (
                ...
                self._logger.info(
                    "Distortion is not time-dependent - take only first"
            ...
            raise MTex.MTpyError_Z(
                "The array provided is not a proper" + "distortion tensor"
            )
        ...
        try:
            ...
            raise MTex.MTpyError_Z(
                "The provided distortion tensor is" + "singular - I cannot invert that!"
            )
...
class Tipper(object):
    ...
    def tipper_err(self, tipper_err_array):
        ...
        if tipper_err_array is not None:
            if len(tipper_err_array.shape) == 3 and tipper_err_array.shape[1:3] == (
                1,
                2,
            ):
