...
class Data(object):
    ...
    def __init__(self, edi_list=None, **kwargs):
        ...
        if self.comp_error_type is not None:
            self.header_strings += [
                "# component '{}' used error type: {}\n".format(comp, err_type)
                for comp, err_type in self.comp_error_type.iteritems()
            ]
    ...
    def project_sites_pyproj(self):
        ...
        for c_arr in self.data_array:
            if c_arr["lat"] != 0.0 and c_arr["lon"] != 0.0:
                ...
                c_arr["east"], c_arr["north"] = mtpy.utils.gis_tools.epsg_project(
                    c_arr["lon"], c_arr["lat"], 4326, self.epsg
                )
    ...
    def get_relative_station_locations(self):
        ...
        self.center_position_EN = 0.5 * np.array(
            [
                self.data_array["east"].min() + self.data_array["east"].max(),
        ...
        self.center_position2 = 0.5 * np.array(
            [
                self.data_array["lon"].min() + self.data_array["lon"].max(),
                self.data_array["lat"].min() + self.data_array["lat"].max(),
            ]
        )

        logger.debug(
            "compare center positions: %s, %s",
            self.center_position,
            self.center_position2,
        )

        # remove the average distance to get coordinates in a relative space
        self.data_array["rel_east"] = (
            self.data_array["east"] - self.center_position_EN[0]
        )
        self.data_array["rel_north"] = (
            self.data_array["north"] - self.center_position_EN[1]
        )
    ...
    def _initialise_empty_data_array(
        self, stationlocations, period_list, location_type="LL", stationnames=None
    ):
        """
    ...
    def _fill_data_array(self, new_edi_dir=None, use_original_freq=False):
        ...
        for ii, s_key in enumerate(sorted(self.mt_dict.keys())):
            logger.debug(
                "mt_dict key: %s and ii= %s", s_key, ii
            )  # s_key is station name
            ...
            if d_array is True:
                ...
                logger.debug(
                    "populate d_array from edi file mt_obj of station %s !!!", s_key
                )
                ...
                try:  # this block will raise exception. see get_relative_station_locations()
                    self.data_array[ii][
                        "rel_east"
                    ] = mt_obj.grid_east  # does not exist attribute grid.east
                    self.data_array[ii]["rel_north"] = mt_obj.grid_north
                    rel_distance = False
                except AttributeError:
                    logger.debug(
                        "skipping - self.data_array[ii]['rel_east'] was not assigned here !!!"
                    )
                    pass
            ...
            if len(interp_periods) > 0:  # not empty
                ...
                for kk, ff in enumerate(interp_periods):
                    ...
                    if mt_obj.Tipper.tipper is not None:
                        ...
                        self.data_array[ii]["tip_err"][jj] = interp_t.tipper_err[
                            kk, :, :
                        ]
    ...
    def write_data_file(
        ...
        if fill is True:
            ...
            self._fill_data_array(
                new_edi_dir=new_edi_dir, use_original_freq=use_original_freq
            )
        ...
        for inv_mode in self.inv_mode_dict[self.inv_mode]:
            ...
            dlines.append(
                "> {0: >10.6f} {1:>10.6f}\n".format(
                    self.center_position[1], self.center_position[0]
                )
            )  # (lat,long) correct order
            dlines.append("> {0} {1}\n".format(nper, nsta))
                    ...
                    for comp in self.inv_comp_dict[inv_mode]:
                        ...
                        if (
                            ...
                            if compute_error:
                                ...
                                if comp.find("t") == 0:
                                    abs_err = self._vertical_components_error_floor(
                                        ff, c_key, ss, z_ii, z_jj
                                    )
                                elif comp.find("z") == 0:
                                    comp_error_type = (
                                        self.comp_error_type[comp]
                                        if self.comp_error_type is not None
                                    ...
                                    if comp_error_type == "floor":
                                        abs_err = self._impedance_components_error_floor(
                                            c_key, ff, ss, z_ii, z_jj, zz
                                        )
                                    elif comp_error_type == "value":
                                        abs_err = self._impedance_components_error_value(
                                            zz
                                        )
                                    elif comp_error_type == "egbert":
                                        abs_err = self._impedance_components_error_egbert(
                                            ff, ss
                                        )
                                    elif comp_error_type == "floor_egbert":
                                        abs_err = self._impedance_components_error_floor_egbert(
                                            c_key, ff, ss, z_ii, z_jj
                                        )
                                    elif comp_error_type == "stddev":
                                        abs_err = self._impedance_components_error_stddev(
                                            c_key, ss, z_ii, z_jj
                                        )
                                    elif comp_error_type == "sqr":
                                        abs_err = self._impedance_components_error_sqr(
                                            c_key, ff, ss, z_ii, z_jj
                                        )
                                    elif comp_error_type == "meansqr":
                                        abs_err = self._impedance_components_error_meansqr(
                                            c_key, ss, z_ii, z_jj
                                        )
                                ...
                                if abs_err == 0.0:
                                    abs_err = 1e3
                                    print (
                                        "error at {0} is 0 for period {1}".format(
                                            sta, per
                                        )
                                        + "set to 1e3"
                                    )
        ...
        if hasattr(self, "center_position_EN"):
            if self.center_position_EN is not None:
                np.savetxt(
                    op.join(self.save_path, "center_position.txt"),
                    self.center_position_EN,
                    fmt="%.1f",
                )
        if hasattr(self, "epsg"):
            if self.epsg is not None:
                np.savetxt(
                    op.join(self.save_path, "epsg.txt"),
                    np.array([self.epsg]),
                    fmt="%1i",
                )
    ...
    def _impedance_components_error_egbert(self, ff, ss):
        ...
        abs_err = np.sqrt(abs(d_zxy * d_zyx)) * self.error_egbert / 100.0
        return abs_err
    ...
    def _impedance_components_error_floor_egbert(self, c_key, ff, ss, z_ii, z_jj):
        ...
        abs_err = max(
            np.sqrt(abs(d_zxy * d_zyx)) * self.error_egbert / 100.0,
    ...
    def _vertical_components_error_floor(self, ff, c_key, ss, z_ii, z_jj):
        if "floor" in self.error_type:
            ...
            abs_err = max(
                self.error_tipper, self.data_array[ss][c_key + "_err"][ff, z_ii, z_jj]
            )
        else:
