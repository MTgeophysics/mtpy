    ...
    def plot1D(self, iternum=10, savepath=None, iterfn=None, respfn=None,
        ...
        # if the response files are input read them in
            ...
            if imode == 'TE':
                ...
                if not isinstance(iterfn, list) or not isinstance(
                        iterfn, tuple):
                    raise IOError(
...
class Occam2DData:
    ...
    def make2DdataFile(self, edipath, mmode='both', savepath=None, stationlst=None,
        ...
        if isinstance(freqstep, list) or not isinstance(freqstep, int):
            if isinstance(freqstep[0], int):
            ...
            for jj, ff in enumerate(freq):
                ...
                try:
                    ...
                    if mmode == 'both':
                        ...
                        if tippererr is not None and tipyn == 'y':
                    ...
                    try:
                        for key in fdict.keys():
                            if ff > float(
                                    key) * (1 - ftol) and ff < float(key) * (1 + ftol):
                                nn = fdict[key]
                                wt = .2 / (ff)
                                resxy = wt * abs(z[jj, 0, 1])**2
                                resyx = wt * abs(z[jj, 1, 0])**2
                                ...
                                if tippererr is not None:
                                    if tip[jj, 0].real == 0.0 or tip[
                                        ...
                                        # real and imaginary
                                        tipphir = np.arctan(
                                            tip[jj, 0].real / tip[jj, 1].real) - theta
                                        tipphii = np.arctan(
                                            tip[jj, 0].imag / tip[jj, 1].imag) - theta
    ...
    def rewrite2DdataFile(self, edipath=None, thetar=0, resxyerr='prev',
        ...
        if removestation is not None:
            # if removestation is not a list make it one
            if not isinstance(removestation, list):
                        ...
                        if mmode == 'both':
                            ...
                            if rpdict[station]['phaseyx'][0, jj - 1] != 0.0:
                                reslst.append(ss + str(kk) + ss + str(jj) + ss + '6' + ss +
                                              fmt % phaseyx + ss + fmt % dphaseyxerr + '\n')
                            if tippererr is not None and tipyn == 'y':
                                if rpdict[station]['realtip'][
                                        0, jj - 1] != 0.0:
                                    reslst.append(ss + str(kk) + ss + str(jj) + ss + '3' + ss +
                                                  fmt % projtipr + ss + fmt % projtiperr +
                                                  '\n')
                                if rpdict[station]['imagtip'][
                                        0, jj - 1] != 0.0:
...
#                rms=np.sqrt(np.sum(ms**2 for ms in rmslst)/len(rmslst))
                ...
                rmste = np.sqrt(
                    np.sum(
                        rms**2 for rms in rmslstte) /
                    len(rmslstte))
                rmstm = np.sqrt(
                    np.sum(
                        rms**2 for rms in rmslsttm) /
                    len(rmslsttm))
...
class Occam2DModel(Occam2DData):
    ...
    def read2DIter(self):
        ...
        # if the file path does not exist look for .dat or data in the
        # filenames
