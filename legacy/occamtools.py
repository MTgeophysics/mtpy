...
class Occam1D:
    ...
    def make1DdataFile(
        ...
        if polarization == "both" or polarization == "det":
            for pol in ["xy", "yx"]:
                if pol == "xy":
                    if polarization == "det":
                        dfilesave = os.path.join(
                            self.savepath, impz.station + "Det_TE.dat"
                        )
                    else:
                    ...
                    if polarization == "det":
                        dfilesave = os.path.join(
                            self.savepath, impz.station + "Det_TM.dat"
                        )
                    else:
                ...
                if pol == "yx":
                    ...
                    if len(negphase) > 0:
                        rp.phaseyx[negphase[0]] = rp.phaseyx[negphase[0]] - 360
                        ...
                        if pol == "xy":
                            if polarization == "det":
                                datafid.write(
                                    ...
                                    + string_fmt % rp.resdet[ii]
                                    + 2 * ss
                                    + string_fmt % (rp.resdet[ii] * reserr / 100.0)
                                    + "\n"
                                )
                            else:
                                datafid.write(
                                    ...
                                    + string_fmt % rp.resxy[ii]
                                    + 2 * ss
                                    + string_fmt % (rp.resxy[ii] * reserr / 100.0)
                                    + "\n"
                                )
                        elif pol == "yx":
                            if polarization == "det":
                                ...
                                datafid.write(
                                    ...
                                    + string_fmt % rp.resyx[ii]
                                    + 2 * ss
                                    + string_fmt % (rp.resyx[ii] * reserr / 100.0)
                                    + "\n"
                                )
                        ...
                        if pol == "xy":
                            if polarization == "det":
                                datafid.write(
                                    ...
                                    + string_fmt % rp.phasedet[ii]
                                    + 2 * ss
                                    + string_fmt % (phaseerr / 100.0 * (180 / np.pi))
                                    + "\n"
                                )
                            else:
                                datafid.write(
    ...
    def make1DInputFile(
        ...
        infid.write(
            "Description:        " + description + "     !For your own notes. \n"
        )
        ...
        infid.write(
            "!Model Bounds:      min,max             ! Optional, places bounds"
            + " on log10(rho) values. \n"
        )
        infid.write(
            "!Model Value Steps: stepsize            ! Optional, forces model"
        ...
        infid.write(
            ...
            + str(misfit)
            + " " * 15
            + "! Misfit of model listed"
            + "below. Ignored on startup.\n"
        )
        infid.write(
            "Misfit Reached:     0	                ! 0: not reached,"
    ...
    def plot1D(
        ...
        if respfn == None:
            if imode == "TE":
                try:
                    self.respfn_te = [
                        os.path.join(self.savepath, rr)
                        for rr in os.listdir(self.savepath)
                        if rr.find("TE_{0}.resp".format(self.iternum)) > 0
                    ][0]
                ...
                try:
                    self.respfn_tm = [
                        os.path.join(self.savepath, rr)
                        for rr in os.listdir(self.savepath)
                        if rr.find("TM_{0}.resp".format(self.iternum)) > 0
                    ][0]
            ...
            if imode == "TE":
                try:
                    self.iterfn_te = [
                        os.path.join(self.savepath, rr)
                        for rr in os.listdir(self.savepath)
                        if rr.find("TE_{0}.iter".format(self.iternum)) > 0
                    ][0]
                ...
                try:
                    self.iterfn_tm = [
                        os.path.join(self.savepath, rr)
                        for rr in os.listdir(self.savepath)
                        if rr.find("TM_{0}.iter".format(self.iternum)) > 0
                    ][0]
        ...
        gs = gridspec.GridSpec(6, 5, hspace=0.25, wspace=0.75)
        ...
        # -------------Plot Both Modes--------------------------------
            ...
            # add legend
            axr.legend(
                [r1te[0], r2te[0], r1tm[0], r2tm[0]],
                ["Data$_{TE}$", "Model$_{TE}$", "Data$_{TM}$", "Model$_{TM}$"],
                loc="upper left",
                markerscale=1,
                borderaxespad=0.15,
                labelspacing=0.18,
        ...
        axr.grid(True, alpha=0.4, which="both")
        plt.setp(axr.xaxis.get_ticklabels(), visible=False)
        axp.grid(True, alpha=0.4, which="both")
        ...
        if imode == "TE":
            ...
            axm.loglog(modelresp[::-1], depthp[::-1], ls="steps-", color="b", lw=lw)
        elif imode == "TM":
            modelresp = abs(10 ** self.mdict["res"][1:, 2])
            axm.loglog(modelresp[::-1], depthp[::-1], ls="steps-", color="b", lw=lw)
        elif imode == "both":
            modelrespte = abs(10 ** self.mdict["res"][1:, 1])
            axm.loglog(modelrespte[::-1], depthp[::-1], ls="steps-", color="b", lw=lw)
            modelresptm = abs(10 ** self.mdict["res"][1:, 2])
            axm.loglog(modelresptm[::-1], depthp[::-1], ls="steps-", color="r", lw=lw)
        ...
        axm.set_xlabel(
            "Resistivity ($\Omega \cdot m$)", fontdict={"size": fs, "weight": "bold"}
        )
        axm.grid(True, which="both", alpha=0.4)
    ...
    def plotL2Curve(self, savepath=None, imode="TE", fignum=1, dpi=150, fs=10):
        ...
        if imode == "TE":
            ...
            iterlstte = [
                os.path.join(self.savepath, itfn)
        ...
        if imode == "TE":
            ...
            (l1,) = ax1.plot(
                np.arange(1, nr, 1), self.rms_te[1:], "-k", lw=1, marker="d", ms=5
            )

            # plot the median of the RMS
            medte = np.median(self.rms_te[1:])
            (m1,) = ax1.plot(np.arange(0, nr, 1), np.repeat(medte, nr), "--r", lw=0.75)
            ...
            ax1.legend(
                [l1, l2, m1],
                ["RMS_TE", "Roughness_TE", "Median_RMS={0:.2f}".format(medte)],
                ncol=4,
                loc="upper center",
                columnspacing=0.25,
                markerscale=0.75,
            ...
            ax2.set_xlabel(
                "Roughness", fontdict={"size": fs, "weight": "bold", "color": "blue"}
            )
        ...
        # -------Plot TM mode-------------------
            ...
            # plot the rms vs iteration
            (l1,) = ax1.plot(
                np.arange(1, nr, 1), self.rms_tm[1:], "-k", lw=1, marker="d", ms=5
            )

            # plot the median of the RMS
            medtm = np.median(self.rms_tm[1:])
            (m1,) = ax1.plot(np.arange(0, nr, 1), np.repeat(medtm, nr), "--r", lw=0.75)
            ...
            ax1.legend(
                [l1, l2, m1],
                ["RMS_TM", "Roughness_TM", "Median_RMS={0:.2f}".format(medtm)],
                ncol=4,
                loc="upper center",
                columnspacing=0.25,
                markerscale=0.75,
            ...
            # plot the median of the RMS
            medtm = np.median(self.rms_tm[1:])
            (m3,) = ax3.plot(np.arange(0, nr, 1), np.repeat(medtm, nr), "--r", lw=0.75)
...
def makestartfiles(parameter_dict):
    ...
    parameter_dict["rlast"] = 1.0e07
    parameter_dict["tobt"] = 100.0
...
def get_model_setup(parameter_dict):
    ...
    for layer_idx in range(n_layer):
        ...
        while block_idx + 2 < ncol - 1:
            ...
            if not dlz[layer_idx] > (
                trigger * (width[block_idx] + width[block_idx + 1])
            ):
                block_idx += 1
                continue
...
def blocks_elements_setup(parameter_dict):
    ...
    for idx in range(1, n_sites - 1):
        ...
        for idx2 in range(n_localextrasites):
            sitlok.append(
                lo_sites[idx - 1] + (idx2 + 1.0) / float(n_localextrasites) * spacing
            )
            j += 1
    ...
    for idx in range(1, nrk - 1):
        spacing2 = (sitlok[idx + 1] - sitlok[idx]) / 2.0
        dly.append(spacing1)
        dly.append(spacing2)
        j += 2
        nfe.append(2)
    ...
    dlz.append(thickness[0] / 2.0)
    dlz.append(thickness[0] / 2.0)
    nfev.append(2)

    k += 2

    dlz.append(thickness[1] / 2.0)
    dlz.append(thickness[1] / 2.0)
    nfev.append(2)
...
class OccamPointPicker(object):
    ...
    def __init__(
        self, axlst, linelst, errlst, reserrinc=0.05, phaseerrinc=0.02, marker="h"
    ):
    ...
    def __call__(self, event):
        ...
        # if the left button is clicked change both resistivity and phase points
            ...
            try:
                ...
                self.axlst[self.fignum][self.kk].plot(
        ...
        # errorbars by the given amount
            ...
            self.error[self.fignum][self.jj][ll] = abs(
                nebu - self.data[self.fignum][self.jj][ll]
            )
...
class Occam2DData:
    ...
    def make2DdataFile(
        ...
        if stationlst == None:
            stationlst = [
                edifile[:-4] for edifile in os.listdir(edipath) if edifile.find(".edi")
            ]
            ...
            for filename in os.listdir(edipath):
                if fnmatch.fnmatch(filename, station + "*.edi"):
                    ...
                    if freq[0] < freq[-1]:
                        ...
                        print (
                            "Flipped frequency to descending for station: "
                            "{0}".format(station)
                        )
                    else:
                        z = z1.Z.z
                    ...
                    zone, east, north = mtpy.utils.gis_tools.ll_to_utm(
                        23, z1.lat, z1.lon
                    )
        ...
        if plotyn == "y":
            ...
            lax.set_ylim(ploty.min() - 1000.0, ploty.max() + 1000.0)
            lax.set_xlim(self.eastlst.min() - 1000, self.eastlst.max() + 1000.0)
        ...
        for ii in range(len(surveylst)):
            ...
            if lineori == "ew":
                if surveylst[0]["east"] < surveylst[ii]["east"]:
                    surveylst[ii]["offset"] = np.sqrt(
                        (surveylst[0]["east"] - surveylst[ii]["east"]) ** 2
                        + (surveylst[0]["north"] - surveylst[ii]["north"]) ** 2
                    )
                elif surveylst[0]["east"] > surveylst[ii]["east"]:
                    surveylst[ii]["offset"] = -1 * np.sqrt(
                        (surveylst[0]["east"] - surveylst[ii]["east"]) ** 2
                        + (surveylst[0]["north"] - surveylst[ii]["north"]) ** 2
                    )
                else:
        ...
        if type(freqstep) is list or type(freqstep) is not int:
            ...
            fdict = dict(
                [
                    ("%.6g" % ff, ii)
                    for ii, ff in enumerate(mfreq[range(0, maxflen, freqstep)], 1)
                    if ff != 0
                ]
            )
        ...
        for key in klst:
            print fdict[key], key, 1.0 / float(key)
        ...
        for kk in range(nstat):
            ...
            for jj, ff in enumerate(freq):
                ...
                try:
                    ...
                    phaseyx = (
                        np.arctan2(z[jj, 1, 0].imag, z[jj, 1, 0].real) * (180 / np.pi)
                        + 180
                    )
                    ...
                    if resxyerr == "data":
                        dresxyerr = (
                            wt * (abs(z[jj, 0, 1]) + zvar[jj, 0, 1]) ** 2 - resxy
                        )
                        lresxyerr = (dresxyerr / resxy) / np.log(10)

                    else:
                        lresxyerr = (resxyerr / 100.0) / np.log(10)

                    # Res_yx(TM)
                    if resyxerr == "data":
                        dresyxerr = (
                            wt * (abs(z[jj, 1, 0]) + zvar[jj, 1, 0]) ** 2 - resyx
                        )
                        lresyxerr = (dresyxerr / resyx) / np.log(10)
                    else:
                        lresyxerr = (resyxerr / 100.0) / np.log(10)
                    ...
                    if phasexyerr == "data":
                        ...
                        dphasexyerr = (phasexyerr / 100.0) * 57 / 2.0
                    ...
                    if phaseyxerr == "data":
                        ...
                        dphaseyxerr = (phaseyxerr / 100.0) * 57 / 2.0
                    ...
                    if tipper_err != None:
                        if tip[jj, 0, 0].real == 0.0 or tip[jj, 0, 1] == 0.0:
                            ...
                            tipphir = (
                                np.arctan(tip[jj, 0, 0].real / tip[jj, 0, 1].real)
                                - theta
                            )
                            tipphii = (
                                np.arctan(tip[jj, 0, 0].imag / tip[jj, 0, 1].imag)
                                - theta
                            )

                            # project the tipper onto the profile line
                            projtipr = np.sqrt(
                                tip[jj, 0, 0].real ** 2 + tip[jj, 0, 1].real ** 2
                            ) * np.cos(tipphir)
                            projtipi = np.sqrt(
                                tip[jj, 0, 0].imag ** 2 + tip[jj, 0, 1].imag ** 2
                            ) * np.cos(tipphii)

                            # error of tipper is a decimal percentage
                            projtiperr = tipper_err / 100.0
                    ...
                    try:
                        for key in fdict.keys():
                            if ff > float(key) * (1 - ftol) and ff < float(key) * (
                                1 + ftol
                            ):
                                nn = fdict[key]
                                wt = 0.2 / (ff)
                                ...
                                # calculate the phase putting the yx in the 1st quadrant
                                phasexy = np.arctan2(
                                    z[jj, 0, 1].imag, z[jj, 0, 1].real
                                ) * (180 / np.pi)
                                phaseyx = (
                                    np.arctan2(z[jj, 1, 0].imag, z[jj, 1, 0].real)
                                    * (180 / np.pi)
                                    + 180
                                )
                                # put phases in correct quadrant if should be negative
                                if phaseyx > 180:
                                    phaseyx = phaseyx - 360
                                    print "Found Negative Phase", surveylst[kk][
                                        "station"
                                    ], ff
                                ...
                                # phase_xy(TE)
                                if phasexyerr == "data":
                                    dphasexyerr = np.arcsin(
                                        zvar[jj, 0, 1] / abs(z[jj, 0, 1])
                                    ) * (180 / np.pi)
                                else:
                                    dphasexyerr = (phasexyerr / 100.0) * 57 / 2.0
                                ...
                                # phase_yx (TM)
                                if phaseyxerr == "data":
                                    dphaseyxerr = np.arcsin(
                                        zvar[jj, 1, 0] / abs(z[jj, 1, 0])
                                    ) * (180 / np.pi)
                                else:
                                    dphaseyxerr = (phaseyxerr / 100.0) * 57 / 2.0
                                ...
                                if tipper_err != None:
                                    if tip[jj, 0].real == 0.0 or tip[jj, 1] == 0.0:
                                        ...
                                        tipphir = (
                                            np.arctan(
                                                tip[jj, 0, 0].real / tip[jj, 0, 1].real
                                            )
                                            - theta
                                        )
                                        tipphii = (
                                            np.arctan(
                                                tip[jj, 0, 0].imag / tip[jj, 0, 1].imag
                                            )
                                            - theta
                                        )
        ...
        datfid.write(
            "SITE" + ss + "FREQ" + ss + "TYPE" + ss + "DATUM" + ss + "ERROR" + "\n"
        )
    ...
    def rewrite2DdataFile(
        ...
        if thetar != 0:
            if edipath == None:
                raise IOError(
                    "Need to input the edipath to original edifiles to"
                    + " get rotations correct"
                )

            # get list of edifiles already in data file
            edilst = [
                os.path.join(edipath, edi)
                for stat in stationlst
                for edi in os.listdir(edipath)
                if edi[0 : len(stat)] == stat
            ]
            reslst = []
            for kk, edifn in enumerate(edilst, 1):
                ...
                fdict = dict(
                    [("{0:.6g}".format(fr), ii) for ii, fr in enumerate(imp1.frequency)]
                )
                # loop over frequencies to pick out the ones desired
                    ...
                    try:
                        ...
                        if tipper_err != None:
                            if tip[nn, 0] == 0.0 or tip[nn, 1] == 0.0:
                                ...
                                # calculate the projection angle for real and imaginary
                                tipphir = (
                                    np.arctan(tip[nn, 0].real / tip[nn, 1].real)
                                    - self.theta_profile
                                )
                                tipphii = (
                                    np.arctan(tip[nn, 0].imag / tip[nn, 1].imag)
                                    - self.theta_profile
                                )

                                # project the tipper onto the profile line
                                projtipr = np.sqrt(
                                    tip[nn, 0].real ** 2 + tip[nn, 1].real ** 2
                                ) * np.cos(tipphir)
                                projtipi = np.sqrt(
                                    tip[nn, 0].imag ** 2 + tip[nn, 1].imag ** 2
                                ) * np.cos(tipphii)
            ...
            for kk, station in enumerate(stationlst, 1):
                ...
                if resxyerr != None:
                    if resxyerr == "prev":
                        ...
                        lresxyerr = np.repeat((resxyerr / 100.0) / np.log(10), nr)
                ...
                if resyxerr != None:
                    if resyxerr == "prev":
                        ...
                        lresyxerr = np.repeat((resyxerr / 100.0) / np.log(10), nr)
                ...
                if phasexyerr != None:
                    if phasexyerr == "prev":
                        ...
                        dphasexyerr = np.repeat((phasexyerr / 100.0) * 57 / 2.0, nr)
                ...
                if phaseyxerr != None:
                    if phaseyxerr == "prev":
                        ...
                        dphaseyxerr = np.repeat((phaseyxerr / 100.0) * 57 / 2.0, nr)
        ...
        # write offsets
        datfid.write("OFFSETS (M):" + "\n")
        projangle = (thetar - self.theta_profile) * np.pi / 180.0
        for station in stationlst:
            # need to project the stations on to the strike direction
            datfid.write(
                ss + string_fmt % (rpdict[station]["offset"] * np.cos(projangle)) + "\n"
            )
    ...
    def makeModelFiles(
        ...
        shutil.copy(
            os.path.join(occampath, "inputMakeModel.txt"),
        ...
        if os.path.getctime(os.path.join(savepath, dfnb)) < os.path.getctime(
            self.datafn
        ):
            shutil.copy(self.datafn, os.path.join(savepath, dfnb))
    ...
    def plotMaskPoints(
        ...
        gs = gridspec.GridSpec(6, 2, wspace=0.1, left=0.1, top=0.93, bottom=0.07)
        for jj, ii in enumerate(pstationlst):
            ...
            for nn, xx in enumerate(axplst):
                ...
                if nn == 0:
                    xx.set_title(
                        self.stationlst[ii] + " Obs$_{xy}$ (TE-Mode)",
                        fontdict={"size": 9, "weight": "bold"},
                    )
                    xx.yaxis.set_label_coords(-0.075, 0.5)
                    xx.set_ylabel(
                ...
                if nn == 1:
                    xx.set_title(
                        self.stationlst[ii] + " Obs$_{yx}$ (TM-Mode)",
                        fontdict={"size": 9, "weight": "bold"},
                    )
                ...
                if nn == 2 or nn == 3:
                    ...
                    if nn == 2:
                        xx.set_ylabel(
                            "Phase (deg)", fontdict={"size": 9, "weight": "bold"}
                        )
    ...
    def maskPoints(self):
        ...
        rploc = dict(
            [
                ("{0}".format(self.points.fndict[key]), int(key) - 1)
                ...
                for frpkey in frpdict.keys():
                    try:
                        ...
                        if ss == 0 or ss == 1:
                            ...
                            if rplst[rploc[str(dd)]][skey][0][ff] != np.log10(
                                dat[ss][floc]
                            ):
                                if dat[ss][floc] == 0:
                                    rplst[rploc[str(dd)]][skey][0][ff] = 0.0
                                else:
                                    rplst[rploc[str(dd)]][skey][0][ff] = np.log10(
                                        dat[ss][floc]
                                    )
        ...
        # make a dictionary of rplst for easier extraction of data
        rpdict = dict(
            [(station, rplst[ii]) for ii, station in enumerate(self.stationlst)]
        )
        ...
        if plottype == "1":
            ...
            if plotnum == 1:
                gs = gridspec.GridSpec(
                    6, 2, wspace=0.1, left=0.09, top=0.93, bottom=0.1, hspace=hspace
                )
            elif plotnum == 2:
                gs = gridspec.GridSpec(
                    6, 2, wspace=0.1, left=0.07, top=0.93, bottom=0.1, hspace=hspace
                )
            ...
            for ii, station in enumerate(self.stationlst):
                ...
                rmslstte = np.hstack((rplst[ii]["resxy"][3], rplst[ii]["phasexy"][3]))
                rmslsttm = np.hstack((rplst[ii]["resyx"][3], rplst[ii]["phaseyx"][3]))
                ...
                if plotresp == True:
                    ...
                    if len(mrxy) > 0:
                        r3 = axrte.errorbar(
                            ...
                            yerr=10
                            ** (
                                rplst[ii]["resxy"][3][mrxy]
                                * rplst[ii]["resxy"][2][mrxy]
                                / np.log(10)
                            ),
                            ecolor=ctem,
                        )
                    ...
                    if len(mryx) > 0:
                        r4 = axrtm.errorbar(
                            ...
                            yerr=10
                            ** (
                                rplst[ii]["resyx"][3][mryx]
                                * rplst[ii]["resyx"][2][mryx]
                                / np.log(10)
                            ),
                            ecolor=ctmm,
                        )
                ...
                if addwl == 1:
                    ...
                    if plotnum == 1:
                        ...
                        axrte.set_title(
                            self.stationlst[ii] + " rms_TE={0:.2f}".format(rmste),
                            fontdict={"size": fs + 1, "weight": "bold"},
                        )
                        axrtm.set_title(
                            self.stationlst[ii] + " rms_TM={0:.2f}".format(rmstm),
                            fontdict={"size": fs + 1, "weight": "bold"},
                        )
                ...
                for aa, axr in enumerate([axrte, axrtm]):
                    ...
                    axr.yaxis.set_label_coords(-0.07, 0.5)
                ...
                for aa, axp in enumerate([axpte, axptm]):
                    ...
                    # set the x axis label
                    axp.set_xlabel(
                        "Period (s)", fontdict={"size": fs, "weight": "bold"}
                    )

                    # put the y label on the far left plot
                    axp.yaxis.set_label_coords(-0.07, 0.5)
                    if aa == 0:
                        axp.set_ylabel(
                            "Phase (deg)", fontdict={"size": fs, "weight": "bold"}
                        )
                    ...
                    if plotnum == 1:
                        ...
                        if aa == 0:
                            rlst = [
                                legend_dict[lkey]
                                for lkey in legend_keys
                                if legend_dict[lkey] != None and lkey.find("te") >= 0
                            ]
                            llst = [
                                legend_label[lkey]
                                for lkey in legend_keys
                                if legend_label[lkey] != None and lkey.find("te") >= 0
                            ]
                            if plotresp == True:
                        ...
                        if aa == 1:
                            rlst = [
                                legend_dict[lkey]
                                for lkey in legend_keys
                                if legend_dict[lkey] != None and lkey.find("tm") >= 0
                            ]
                            llst = [
                                legend_label[lkey]
                                for lkey in legend_keys
                                if legend_label[lkey] != None and lkey.find("tm") >= 0
                            ]
                            if plotresp == True:
                                axr.legend(
                                    rlst,
    ...
    def plotPseudoSection(
       ...
       :Example: ::
        ...
        nf = len(self.freq)
        ylimits = (1.0 / self.freq.min(), 1.0 / self.freq.max())
        #    print ylimits
        ...
        dgrid, fgrid = np.meshgrid(offsetlst, 1.0 / self.freq[::-1])

        # make list for station labels
        slabel = [
            self.stationlst[ss][stationid[0] : stationid[1]] for ss in range(0, ns, ml)
        ]
        labellst = [
            "$r_{TE-Data}$",
            "$r_{TE-Model}$",
        ...
        if respfn != None:
            ...
            gs1 = gridspec.GridSpec(
                2, 2, left=0.06, right=0.48, hspace=0.1, wspace=0.005
            )
            gs2 = gridspec.GridSpec(
                2, 2, left=0.52, right=0.98, hspace=0.1, wspace=0.005
            )
            ...
            for xx, ax in enumerate(axlst):
                ...
                if np.remainder(xx, 2.0) == 1:
                    ...
                    cbx = mcb.make_axes(ax, shrink=0.7, pad=0.015)
                    if xx < 4:
                        if xx == 1:
                            cb = mcb.ColorbarBase(
                                cbx[0],
                                cmap=rcmap,
                                norm=Normalize(vmin=rlim[0][0], vmax=rlim[0][1]),
                            )
                        if xx == 3:
                            cb = mcb.ColorbarBase(
                                cbx[0],
                                cmap=rcmap,
                                norm=Normalize(vmin=rlim[1][0], vmax=rlim[1][1]),
                            )
                            cb.set_label(
                                "App. Res. ($\Omega \cdot$m)", fontdict={"size": 9}
                            )
                    else:
                        if xx == 5:
                            cb = mcb.ColorbarBase(
                                cbx[0],
                                cmap=pcmap,
                                norm=Normalize(vmin=plim[0][0], vmax=plim[0][1]),
                            )
                        if xx == 7:
                            cb = mcb.ColorbarBase(
                                cbx[0],
                                cmap=pcmap,
                                norm=Normalize(vmin=plim[1][0], vmax=plim[1][1]),
                            )
                ...
                if xx == 0 or xx == 4:
                    ax.set_ylabel("Period (s)", fontdict={"size": 10, "weight": "bold"})
                if xx > 3:
            ...
            for xx, ax in enumerate(axlst):
                ...
                if xx == 0:
                    ...
                    cb.set_label("App. Res. ($\Omega \cdot$m)", fontdict={"size": 9})
                elif xx == 2:
                    cb = mcb.ColorbarBase(
                        cbx[0],
                        cmap=pcmap,
    ...
    def plotAllResponses(self, station, fignum=1):
        ...
        gs = gridspec.GridSpec(6, 2, wspace=0.20)
        ...
        colorlst = [(cc, 0, 1 - cc) for cc in np.arange(0, 1, 1.0 / nresp)]
        ...
        for jj, rfile in enumerate(rlst):
            ...
            rmslstte = np.hstack(
                (self.rplst[ii]["resxy"][3], self.rplst[ii]["phasexy"][3])
            )
            rmslsttm = np.hstack(
                (self.rplst[ii]["resyx"][3], self.rplst[ii]["phaseyx"][3])
            )
            ...
            (r3,) = axrte.loglog(
                period[mrxy],
                10 ** self.rplst[ii]["resxy"][2][mrxy],
                ls="-",
                color=colorlst[jj],
            )
            (r4,) = axrtm.loglog(
                period[mryx],
                10 ** self.rplst[ii]["resyx"][2][mryx],
                ls="-",
                color=colorlst[jj],
            )
            ...
            axpte.semilogx(
                period[mpxy],
                self.rplst[ii]["phasexy"][2][mpxy],
                ls="-",
                color=colorlst[jj],
            )
            axptm.semilogx(
                period[mpyx],
                self.rplst[ii]["phaseyx"][2][mpyx],
                ls="-",
                color=colorlst[jj],
            )

        axrte.grid(True, alpha=0.4)
        axrtm.grid(True, alpha=0.4)
        ...
        axrte.set_title(
            "TE rms={0:.2f}".format(rmstestr), fontdict={"size": 10, "weight": "bold"}
        )
        axrtm.set_title(
            "TM rms={0:.2f}".format(rmstmstr), fontdict={"size": 10, "weight": "bold"}
        )

        axpte.grid(True, alpha=0.4)
        axpte.yaxis.set_major_locator(MultipleLocator(10))
        axpte.yaxis.set_minor_locator(MultipleLocator(1))

        axrte.set_ylabel(
            "App. Res. ($\Omega \cdot m$)", fontdict={"size": 10, "weight": "bold"}
        )
        axpte.set_ylabel("Phase (deg)", fontdict={"size": 10, "weight": "bold"})
        axpte.set_xlabel("Period (s)", fontdict={"size": 10, "weight": "bold"})

        axrte.yaxis.set_label_coords(-0.08, 0.5)
        axpte.yaxis.set_label_coords(-0.08, 0.5)

        axrtm.set_xticklabels(["" for ii in range(10)])
        axptm.grid(True, alpha=0.4)
        ...
        axptm.set_ylabel("Phase (deg)", fontdict={"size": 12, "weight": "bold"})
        axptm.set_xlabel("Period (s)", fontdict={"size": 12, "weight": "bold"})

        axrtm.yaxis.set_label_coords(-0.08, 0.5)
        axptm.yaxis.set_label_coords(-0.08, 0.5)
...
class Occam2DModel(Occam2DData):
    ...
    def get2DModel(self):
        ...
        plotx = np.array(
            [self.hnodes[: ii + 1].sum() for ii in range(len(self.hnodes))]
        )
        ploty = np.array(
            [self.vnodes[: ii + 1].sum() for ii in range(len(self.vnodes))]
        )
    ...
    def plot2DModel(
        ...
        plt.rcParams["font.size"] = int(dpi / 40.0)
        plt.rcParams["figure.subplot.left"] = 0.08
        ...
        cbx = mcb.make_axes(ax, shrink=0.8, pad=0.01)
        cb = mcb.ColorbarBase(
            cbx[0], cmap=cmap, norm=Normalize(vmin=climits[0], vmax=climits[1])
        )
        cb.set_label(
            "Resistivity ($\Omega \cdot$m)", fontdict={"size": fs, "weight": "bold"}
        )
        ...
        cb.set_ticklabels(
            [
                "10$^{0}$".format(nn)
                for nn in np.arange(int(climits[0]), int(climits[1]) + 1)
            ]
        )
        ...
        if ylimits == None:
            ax.set_ylim(
                abs(max(self.offsetlst) - min(self.offsetlst)) / dfactor,
                -ypad * pfactor,
            )
        else:
            ax.set_ylim(ylimits[1] * pfactor, (ylimits[0] - ypad) * pfactor)
        ax.set_xlim(
            min(self.offsetlst) / dfactor - (xpad * pfactor),
        ...
        if yscale == "km":
            ax.set_xlabel(
                "Horizontal Distance (km)", fontdict={"size": fs, "weight": "bold"}
            )
            ax.set_ylabel("Depth (km)", fontdict={"size": fs, "weight": "bold"})
        elif yscale == "m":
            ax.set_xlabel(
                "Horizontal Distance (m)", fontdict={"size": fs, "weight": "bold"}
            )
        ...
        if type(title) is str:
            if title == "on":
                titlestr = os.path.join(
                    os.path.basename(os.path.dirname(self.iterfn)),
            ...
            print "RMS {0:.2f}, Roughness={1:.0f}".format(
                float(self.idict["misfit value"]), float(self.idict["roughness value"])
            )

        # plot forward model mesh
        if femesh == "on":
            for xx in self.plotx / dfactor:
                ax.plot([xx, xx], [0, self.ploty[0] / dfactor], color="k", lw=0.5)
            for yy in self.ploty / dfactor:
                ax.plot(
                    [self.plotx[0] / dfactor, self.plotx[-1] / dfactor],
                    [yy, yy],
                    color="k",
                    lw=0.5,
                )
        ...
        if regmesh == "on":
            ...
            for ii in range(len(self.rows)):
                ...
                yline = ax.plot(
                    [self.plotx[0] / dfactor, self.plotx[-1] / dfactor],
                    [self.ploty[-ny1] / dfactor, self.ploty[-ny1] / dfactor],
                    color="b",
                    lw=0.5,
                )
                ...
                for jj in range(len(self.cols[ii])):
                    ...
                    try:
                        if ny1 == 0:
                            ny1 = 1
                        xline = ax.plot(
                            [self.plotx[nx1] / dfactor, self.plotx[nx1] / dfactor],
                            [self.ploty[-ny1] / dfactor, self.ploty[-ny2] / dfactor],
                            color="b",
                            lw=0.5,
                        )
            ...
            for ii in range(len(self.rows)):
                ...
                for jj in range(len(self.cols[ii])):
                    ...
                    try:
                        ...
                        yy = (
                            self.ploty[-ny1] - (self.ploty[-ny1] - self.ploty[-ny2]) / 2
                        )
    ...
    def plotL2Curve(self, fnstem=None, fignum=1, dpi=300):
        ...
        if fnstem == None:
            ...
            iterlst = [
                os.path.join(invpath, itfile)
                for itfile in os.listdir(invpath)
                if itfile.find(".iter") > 0 and itfile.find(fnstem) > 0
            ]
        ...
        # plot the rms vs iteration
        (l1,) = ax1.plot(
            np.arange(1, nr, 1), rmsarr[1:, 0], "-k", lw=1, marker="d", ms=5
        )

        # plot the median of the RMS
        (m1,) = ax1.plot(
            np.arange(0, nr, 1), np.repeat(np.median(rmsarr[1:, 0]), nr), "--r", lw=0.75
        )

        # plot the mean of the RMS
        (m2,) = ax1.plot(
            np.arange(0, nr, 1),
            np.repeat(np.mean(rmsarr[1:, 0]), nr),
            ls="--",
            color="orange",
            lw=0.75,
        )
        ...
        # plot the rms vs roughness
        (l2,) = ax2.plot(
            rmsarr[1:, 1], rmsarr[1:, 0], "--b", lw=0.75, marker="o", ms=7, mfc="white"
        )
        ...
        ax1.grid(alpha=0.25, which="both")
        ax2.set_xlabel(
            "Roughness", fontdict={"size": 8, "weight": "bold", "color": "blue"}
        )
        for t2 in ax2.get_xticklabels():
            t2.set_color("blue")
    ...
    def plotDepthModel(
        ...
        xavg = np.floor(
            np.mean(
                [
                    abs(self.plotx[ii] - self.plotx[ii + 1])
        ...
        for ff in pstationlst:
            ...
            for ii, xx in enumerate(self.plotx):
                if offset >= xx - xavg / 2.0 and offset <= xx + xavg / 2.0:
                    slst.append(ii)
        ...
        if plotnum == "all":
            # set the dimesions of the figure
            plt.rcParams["font.size"] = int(dpi / 60.0)
            ...
            for ii, ss in enumerate(slst):
                ...
                if yscale == "linear":
                    (p1,) = ax.semilogx(
                        10 ** self.resmodel[:, ss], self.ploty, ls="steps-"
                    )
                elif yscale == "log":
                    if self.ploty[-1] == 0.0:
                        self.ploty[-1] = 1
                ...
                ax.set_title(
                    self.data.stationlst[pstationlst[ii]],
            ...
            for ii, ss in enumerate(slst):
                ...
                ax.set_title(
                    self.stationlst[pstationlst[ii]],
                    fontdict={"size": 10, "weight": "bold"},
                )
                ax.set_ylabel("Depth (m)", fontdict={"size": 8, "weight": "bold"})
                ax.set_xlabel(
