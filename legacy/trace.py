...
class Trace(object):
    ...
    def __str__(self):
        ...
        s = 'Trace (%s, %s, %s, %s)\n' % self.nslc_id
        s += '  timerange: %s - %s\n' % (util.time_to_str(
            self.tmin, format=fmt), util.time_to_str(self.tmax, format=fmt))
        s += '  delta t: %g\n' % self.deltat
        if self.meta:
    ...
    def add(self, other, interpolate=True):
        ...
        *interpolate* is ``False``, the sampling rates of the two traces must
        ...
        if interpolate:
            assert self.deltat <= other.deltat or same_sampling_rate(
                self, other)
            other_xdata = other.get_xdata()
            xdata = self.get_xdata()
            xmin, xmax = other_xdata[0], other_xdata[-1]
            self.ydata += num.interp(xdata, other_xdata,
                                     other.ydata, left=0., right=0.)
        else:
    ...
    def set_codes(self, network=None, station=None,
    ...
    def downsample(self, ndecimate, snap=False, initials=None, demean=True):
        ...
        if snap:
            ilag = (math.ceil(self.tmin / newdeltat) *
                    newdeltat - self.tmin) / self.deltat
    ...
    def downsample_to(self, deltat, snap=False,
                      allow_upsample_max=1, initials=None, demean=True):
            ...
            if allow_upsample_max <= 1:
                ...
                if upsratio > 1:
                    ...
                    for i in range(1, upsratio):
                        self.ydata[i::upsratio] = float(
                            i) / upsratio * ydata[:-1] + float(upsratio - i) / upsratio * ydata[1:]
    ...
    def nyquist_check(self, frequency, intro='Corner frequency',
    ...
    def bandpass(self, order, corner_hp, corner_lp, demean=True):
        ...
        (b, a) = _get_cached_filter_coefs(order, [
            corner * 2.0 * self.deltat for corner in (corner_hp, corner_lp)], btype='band')
        data = self.ydata.astype(num.float64)
        if demean:
            data -= num.mean(data)
        self.drop_growbuffer()
    ...
    def ampspec_whiten(self, width, td_taper='auto',
                       fd_taper='auto', pad_to_pow2=True, demean=True):
    ...
    def _get_cached_freqs(self, nf, deltaf):
        ...
        if ck not in Trace.cached_frequencies:
            Trace.cached_frequencies[ck] = num.arange(
                nf, dtype=num.float) * deltaf
    ...
    def peaks(self, threshold, tsearch, deadtime=False,
        ...
        for itrig_pos in itrig_positions:
            ...
            if deadtime:
                ...
                while True:
                    ...
                    izero_positions = num.nonzero(
                        deriv > 0)[0] + iblock * nblock
                    if len(izero_positions) > 0:
                        tzero = self.tmin + \
                            (ibeg + izero_positions[0]) * self.deltat
                        break
                    iblock += 1
            else:
    ...
    def transfer(self, tfade, freqlimits, transfer_function=None,
                 cut_off_fading=True):
    ...
    def _get_tapered_coefs(self, ntrans, freqlimits, transfer_function):
        ...
        a, b, c, d = freqlimits
        freqs = num.arange(hi(d) - hi(a), dtype=num.float) * \
            deltaf + hi(a) * deltaf
    ...
    def plot(self):
        ...
        name = self.channel + ' ' + self.station + ' ' + time.strftime("%d-%m-%y %H:%M:%S", time.gmtime(
            self.tmin)) + ' - ' + time.strftime("%d-%m-%y %H:%M:%S", time.gmtime(self.tmax))
        pylab.title(name)
        pylab.show()
...
def degapper(traces, maxgap=5, fillmethod='interpolate',
             deoverlap='use_second', maxlap=None):
                ...
                if 1 < idist <= maxgap:
                    if not virtual:
                        if fillmethod == 'interpolate':
                            filler = a.ydata[-1] + (((1. + num.arange(idist - 1, dtype=num.float)) / idist) * (
                                b.ydata[0] - a.ydata[-1])).astype(a.ydata.dtype)
...
def rotate_to_rt(n, e, source, receiver, out_channels=('R', 'T')):
    ...
    out = rotate([n, e], azimuth, in_channels=in_channels,
...
def correlate(a, b, mode='valid', normalization=None, use_fft=False):
    ...
    yc = numpy_correlate_fixed(
        ...
        ya,
        mode=mode,
        use_fft=use_fft)  # need reversed order here
    ...
    if normalization == 'normal':
        ...
        normfac = normfac_short * \
            num.sqrt(
                moving_sum(
                    ylong**2,
                    yshort.size,
                    mode='valid')) + normfac_short * epsilon
...
def deconvolve(a, b, waterlevel, tshift=0., pad=0.5,
               fd_taper=None, pad_to_pow2=True):
...
def assert_same_sampling_rate(a, b, eps=1.0e-6):
    assert same_sampling_rate(
        a, b, eps), 'Sampling rates differ: %g != %g' % (a.deltat, b.deltat)
...
class SampledResponse(FrequencyResponse):
    ...
    def inverse(self):
        ...
        return SampledResponse(self.freqs, 1. / self.vals,
                               left=inv_or_none(self.left), right=inv_or_none(self.right))
...
def numpy_correlate_emulate(a, b, mode='valid'):
    ...
    for k in xrange(kmin, kmin + klen):
        ...
        c[k - kmin] = num.sum(a[imin + k:imin + ilen + k]
                              * num.conj(b[imin:imin + ilen]))
...
def apply_costaper(a, b, c, d, y, x0, dx):
    ...
    y[:hi(a)] = 0.
    y[hi(a):hi(b)] *= 0.5 - 0.5 * \
        num.cos((dx * num.arange(hi(a), hi(b)) - (a - x0)) / (b - a) * num.pi)
    y[hi(c):hi(d)] *= 0.5 + 0.5 * \
        num.cos((dx * num.arange(hi(c), hi(d)) - (c - x0)) / (d - c) * num.pi)
    y[hi(d):] = 0.
...
def costaper(a, b, c, d, nfreqs, deltaf):
    ...
    tap[hi(a):hi(b)] = 0.5 - 0.5 * \
        num.cos((deltaf * num.arange(hi(a), hi(b)) - a) / (b - a) * num.pi)
    tap[hi(b):hi(c)] = 1.
    tap[hi(c):hi(d)] = 0.5 + 0.5 * \
        num.cos((deltaf * num.arange(hi(c), hi(d)) - c) / (d - c) * num.pi)
