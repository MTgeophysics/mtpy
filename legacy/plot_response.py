...
class PlotResponse(object):
                ...
                if self.plot_component == 2:
                    ...
                    if self.plot_z == False:
                        ...
                        erxy = mtplottools.plot_errorbar(
                            axrxy, period, rp.resxy[nzxy], rp.resxy_err[nzxy], **kw_xx
                        )
                        ...
                        if self.plot_component == 2:
                            if self.plot_z == False:
                                ...
                                reryx = mtplottools.plot_errorbar(
                                    axryx, period[nzyx], rrp.resyx[nzyx], **kw_yy
                                )
                                # plot phase
                                rerxy = mtplottools.plot_errorbar(
                                ...
                                reryx = mtplottools.plot_errorbar(
                                    axpyx, period[nzyx], rrp.phaseyx[nzyx], **kw_yy
                                )
                            elif self.plot_z == True:
                                # plot real
                                rerxy = mtplottools.plot_errorbar(
                            ...
                            if self.plot_z == False:
                                ...
                                reryy = mtplottools.plot_errorbar(
                                    axryy, period[nzyy], rrp.resyy[nzyy], **kw_yy
                                )
                                # plot phase
                                rerxx = mtplottools.plot_errorbar(
                                ...
                                reryy = mtplottools.plot_errorbar(
                                    axpyy, period[nzyy], rrp.phaseyy[nzyy], **kw_yy
                                )
                            elif self.plot_z == True:
                                # plot real
                                rerxx = mtplottools.plot_errorbar(
    ...
    def _save_figure(self, save_fn, orientation="portrait", fig_dpi=200, close_fig="n"):
        """
...
@click.option(
    ...
    "--directory",
    type=str,
    default=r"examples/model_files/ModEM_2",
...
@click.option(
    ...
    "--stem_data_file",
    type=str,
    default="Modular_MPI_NLCG_004.dat",
    help="file stem",
)
@click.option(
    "-i", "--input_data_file", type=str, default="ModEM_Data.dat", help="Data File"
)
@click.option(
    "-c",
    "--collection_station",
...
@click.option(
    ...
    type=bool,
    default=False,
    help="[True | False ] Plot True for Impedence, False for Resistivity and Phsse",
)
@click.option("-f", "--font_size", type=int, default=2, help="Plot Text Fond Size ")
def merge_plotting(
    directory, stem_data_file, input_data_file, collection_station, plot_z, font_size
):
    ...
    print (
        "python mtpy/imaging/plot_response.py -d examples\data\ModeEM_files_Test "
        + "-s Modular_MPI_NLCG_094.dat -i ModEM_Data.dat -c GB09 -p False -f 3"
    )
    print (
        "python mtpy/imaging/plot_response.py -d examples\data\ModeEM_files_Test -p False ( Changing Plot types ) "
    )
    print ("")
    print (
        "============================================================================"
    ...
    ro = PlotResponse(
        data_fn=os.path.join(directory, input_data_file),
