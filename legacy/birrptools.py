...
def readProDict(
    processinginfofile, dirpath, filtered=None, rrfiltered=None, kwargs=None
):
    """
    ...
    if len(prepdict["day"].split(";")) > 1 or len(prepdict["day"].split(",")) > 1:
        ...
        for ii, days in enumerate(dayslst):
            ...
            if len(days.split(",")) > 1:
                ...
                cfilelsti, fileslsti = mt.combineFiles(
                    prepdict["dirpath"],
                    prepdict["station"],
                    dlst,
                    cacherate,
                ...
                if rrstation[ii] != station:
                    ...
                    rrcfilelsti, rrfileslsti = mt.combineFiles(
                        prepdict["dirpath"],
                        rrstation[ii],
                        rrdlst,
                        cacherate,
                    ...
                    nskipri = int(
                        (float(dlst[0]["start"][0:2]) - float(rrdlst[0]["start"][0:2]))
            ...
            # if multiple files but not continuous.
                ...
                nreadi = int(
                    (float(stop[0:2]) - float(start[0:2]))
                    * 3600
                    * float(prepdict["df"])
                    / dec
                )
                ...
                # combine files
                # print 'combining
                # ',cdirpath,start,stop,cacherate,combcomplst,dec
                cfilelsti, fileslsti = mt.combineFewFiles(
                    cdirpath,
                    station,
                    start,
                    stop,
                ...
                if rrstation[ii] != station:
                    ...
                    try:
                        ...
                        filt = ""
                    if filt != "":
                        rrcdirpath = os.path.join(
                            prepdict["dirpath"], rrstation[ii], rrday, filt
                        )
                    else:
                        rrcdirpath = os.path.join(
                            prepdict["dirpath"], rrstation[ii], rrday
                        )

                    rrcfilelsti, rrfileslsti = mt.combineFewFiles(
                        rrcdirpath,
                        rrstation[ii],
                        rrstart,
                        rrstop,
                    ...
                    # get number of points to skip for remote reference
                    nskipri = int(
                        (float(dstartlst[ii][0:2]) - float(drrstartlst[ii][0:2]))
                        * (float(prepdict["df"]) / float(dec))
                    )
                else:
                    rrcfilelsti = []
    ...
    # else normal input for one day
        ...
        try:
            ...
            try:
                ...
                cdirpath = os.path.join(
                    prepdict["dirpath"], prepdict["station"], prepdict["day"]
                )
        ...
        try:
            ...
            try:
                ...
                cdirpathr = os.path.join(
                    prepdict["dirpath"], rrstation[0], prepdict["day"]
                )
        ...
        cfilelst, fileslst = mt.combineFewFiles(
            cdirpath,
            station,
            stime,
            etime,
        ...
        if rrstation[0] != station:
            rrcfilelst, rrfileslst = mt.combineFewFiles(
                cdirpathr,
                rrstation[0],
                stimer,
                etimer,
            ...
            nskipr = int(
                (float(stime[0:2]) - float(stimer[0:2]))
                * (float(prepdict["df"]) / float(dec))
            )
        else:
            rrcfilelst = []
...
def writeScriptfile(processingdict):
    ...
    if nds == 0:
        bfpath = os.path.join(os.path.dirname(processingdict["cfilelst"][0]), "BF")
        npcs = 1
    elif nds == 1:
        nds = 0
        npcs = 1
        ...
        bfpath = os.path.join(os.path.dirname(processingdict["cfilelst"][0][0]), "BF")
        npcs = int(nds)
    ...
    if ilev == 0:
        ...
        fid.write("{0:d} \n".format(nref))
        if nref > 3:
            nrrlst = np.array([len(rrlst) for rrlst in processingdict["rrcfilelst"]])
...
def callBIRRP(scriptfile, birrploc):
    ...
    print "BIRRP Run time (m:s) for " + station + ": %.0f" % int(
        np.floor(dtburp / 60)
    ) + ":%2f" % int(np.round(np.remainder(dtburp, 60)))

    bfpath = os.path.dirname(scriptfile)
    return bfpath


def convertBIRRPoutputs(
    bfpath,
    stationinfofile,
    station,
    rrstation=None,
    ...
    if os.path.basename(spath)[6] == "t":
        ...
        st = ""
    while day is None and count < 10:
        try:
    ...
    try:
        edifile = writeedi(
            bfpath,
            station,
            stationinfofile=stationinfofile,
            rrstation=rrstation,
            birrpdict=birrpdict,
            bbfile=bbfile,
            tsp="   ",
...
def plotBFfiles(
    edifile, cohfile=None, cfilelst=None, save="y", ffactor=1, show="n", sdict=None
):
    """
    ...
    if save == "y":
        ...
        if cohfile is not None:
            fig1 = mtplot.plotcoh(
                cohfile,
                fignum=1,
                savefigfilename=os.path.join(plotpath, station + day + st + "coh.pdf"),
            )
...
def runBIRRPpp(
    dirpath, processingdict, stationinfofile, birrploc, ffactor=1, edipath=None
):
    """
    ...
    # print birrpdict
    cohfile, edifile, datfile, impfile = convertBIRRPoutputs(
        bfpath,
        stationinfofile,
        station,
        rrstation=rrstation,
...
def bbcalfunc(bbfile, nfreqlst):
    ...
    for idx, f in enumerate(freqs_of_interest):
        logfreq = np.log(f)
        # find the calibration value closest to the current freq, assume it's
        # lower
        ...
        # or the lowest
            ...
            # for low frequencies take the log of the values to get into loglog
            # space:
            if freq <= 5:
                ...
                interpval_real = np.exp(
                    weight * logrealval1 + (1 - weight) * logrealval2
                )
                interpval_imag = np.exp(
                    weight * logimagval1 + (1 - weight) * logimagval2
                )
...
def readj(
    ...
    dlen=[50, 50],
    magtype="bb",
    bbfile=r"c:\Peacock\PHD\BIRRP\bbconv.txt",
    ffactor=1,
):
    """
    ...
    for kk in range(4):
        for ll, nn in enumerate(
            range(jj + 2 * (kk + 1) + kk * nf, jj + 2 * (kk + 1) + (kk + 1) * nf)
        ):
    ...
    if tz != -1:
        for kk in range(2):
            for ll, nn in enumerate(
                range(tz + 2 * (kk + 1) + kk * nf, tz + 2 * (kk + 1) + (kk + 1) * nf)
            ):
                jline = jlines[nn].strip().split()
                tr = float(jline[1])
                ti = float(jline[2])
...
def readrf(
    ...
    try:
        ...
        raise ValueError(
            "BIRRP has output uneven file lengths, try running "
...
def bbconvz(z, freq, bbfile, dlgain):
    ...
    for jj in range(len(z)):
        ...
        for ii in range(len(freq)):
            ...
            zcal = ztest * bcal * (1.0 / dlgain)
            zr.append(zcal.real)
            zi.append(zcal.imag)
            ze.append(float(z[jj, 2, ii]) * abs(bcal) * (1.0 / dlgain))
...
def lpconvz(z, dlgain=1):
    ...
    """
    zconv = (z * 10.0e7) / (70000.0 * float(dlgain))
...
def writeimp(
    ...
    ofil, period, freq, z, tip = readrf(
        dirpath, egain=egain, dlgain=dlgain, dlen=dlen, magtype=magtype, bbfile=bbfile
    )
...
def readimp(filename):
    ...
    for ii in np.arange(start=2, stop=len(implines), step=1):
        ...
        zyyi = float(line[8])
        z.append(
            [[zxxr + 1j * zxxi, zxyr + 1j * zxyi], [zyxr + 1j * zyxi, zyyr + 1j * zyyi]]
        )
        try:
            txxr = float(line[9])
            txxi = float(line[10])
...
def writedat(
    ...
    for ii in range(len(res)):
        ...
        for jj in range(len(period)):
            datfid.write(
                sigfigs(str(period[jj]), digits=7, fmt="e")
                + tsp
                + sigfigs(str(res[ii][0][jj]), digits=7, fmt="e")
                + tsp
                + sigfigs(str(res[ii][1][jj]), digits=7, fmt="e")
                + tsp
                + sigfigs(str(phase[ii][0][jj]), digits=7, fmt="e")
                + tsp
                + sigfigs(str(phase[ii][1][jj]), digits=7, fmt="e")
                + "\n"
            )
...
def writeedi(
    ...
    if stationinfofile is None or stationinfofile == "None":
        statdict = {}
    else:
        statdict = mt.getStationInfo(stationinfofile, station)
    ...
    try:
        ...
        magtype = input(
            "Enter magtype bb for broadband, lp for longperiod. "
    ...
    try:
        ...
        dlgain = input(
            "Enter data logger gain (verylow=2.5,low=1,high=.1)"
            + "be sure to put quotes around answer. \n"
        )
    ...
    ofil, period, freq, z, tip = readj(
        os.path.join(dirpath, station + ".j"),
        egain=float(statdict["egain"]),
        dlgain=float(dlgain),
        dlen=[float(statdict["ex"]), float(statdict["ey"])],
        magtype=magtype,
    ...
    edifid.write(
        tsp
        + "FILEDATE="
        + datetime.date.strftime(datetime.date.today(), "%B %d, %Y")
        + "\n"
    )
    # survey location
    try:
    ...
    try:
        edifid.write(
            lsp + "Coil Numbers (BX,BY,BZ): " + statdict["coil no(bx,by)"] + "\n"
        )
    except KeyError:
        pass
    ...
    if birrpdict is not None:
        edifid.write(lsp + "Coil Calibration File=" + bbfile + "\n")
        try:
            edifid.write(
                lsp + "Interaction Level (ILEV)=" + str(birrpdict["ilev"]) + "\n"
            )
        ...
        try:
            edifid.write(
                lsp + "Number of outputs (NOUT)=" + str(birrpdict["nout"]) + "\n"
            )
        except KeyError:
            pass
        try:
            edifid.write(
                lsp + "Number of inputs (NINP)=" + str(birrpdict["ninp"]) + "\n"
            )
        except KeyError:
            pass
        if ilevyn == 1:
            try:
                edifid.write(
                    lsp
                    + "Number of Remote Reference time series (NREF)="
                    + str(birrpdict["nref"])
                    + "\n"
                )
            except KeyError:
            ...
            try:
                edifid.write(
                    lsp
                    + "Remote reference(0) or bounded influence(1)"
                    + "(NRR)="
                    + str(birrpdict["nrr"])
                    + "\n"
                )
        ...
        try:
            edifid.write(
                lsp + "Slepian Filter order (TBW)=" + str(birrpdict["tbw"]) + "\n"
            )
        except KeyError:
            pass
        try:
            edifid.write(
                lsp + "Max length of fft window (NFFT)=" + str(birrpdict["nfft"]) + "\n"
            )
        except KeyError:
            pass
        if ilevyn == 1:
            try:
                edifid.write(
                    lsp
                    + "Section Increment divisor (NSCTINC)="
        ...
        try:
            edifid.write(
                lsp
                + "Maximum number of fft sections (NSCTMAX)="
        ...
        if ilevyn == 1:
            ...
            try:
                edifid.write(
                    lsp
                    + "Frequency increment per window (NFINC)="
                    + str(birrpdict["nfinc"])
                    + "\n"
                )
            except KeyError:
            ...
            try:
                edifid.write(
                    lsp
                    + "Number of frequencies per window (NFSECT)="
                    + str(birrpdict["nf1"])
                    + "\n"
                )
            except KeyError:
        ...
        try:
            edifid.write(
                lsp
                + "Small leverage point control (UIN)="
                + str(birrpdict["uin"])
                + "\n"
            )
        except KeyError:
        ...
        if ilevyn == 1:
            try:
                edifid.write(
                    lsp
                    + "Lower leverage point control (AINLIN)="
        ...
        try:
            edifid.write(
                lsp
                + "Large leverage point control (AINUIN)="
        ...
        if ilevyn == 1:
            try:
                edifid.write(
                    lsp
                    + "Magnetic coherence threshold (C2THRESHEB)="
        ...
        try:
            edifid.write(
                lsp
                + "Electric coherence threshold (C2THRESHE)="
        ...
        try:
            edifid.write(
                lsp
                + "Coherence threshold z channel (c2threshe1)="
        ...
        if ilevyn == 1:
            try:
                edifid.write(
                    lsp
                    + "Low and high periods for coherence threshold "
                    + "(PERLO,PERHI)="
                    + str(birrpdict["perlo"])
                    + ","
                    + str(birrpdict["perhi"])
            ...
            try:
                edifid.write(
                    lsp
                    + "Number of periods to reject (NPREJ)="
                    + str(birrpdict["nprej"])
                    + "\n"
                )
            except KeyError:
            ...
            try:
                edifid.write(
                    lsp + "Periods to reject (PREJ)=" + str(birrpdict["prej"]) + "\n"
                )
            except KeyError:
                pass
        try:
            edifid.write(
                lsp
                + "Order of prewhitening filter (NAR)="
                + str(birrpdict["nar"])
                + "\n"
            )
        except KeyError:
        ...
        try:
            edifid.write(
                lsp
                + "Electric channel rotation angles (THETAE)="
                + birrpdict["thetae"]
                + "\n"
            )
        except KeyError:
        ...
        try:
            edifid.write(
                lsp
                + "Magnetic channel rotation angles (THETAB)="
                + birrpdict["thetab"]
                + "\n"
            )
        except KeyError:
        ...
        try:
            edifid.write(
                lsp
                + "Final channel rotation angles (THETAF)="
                + birrpdict["thetaf"]
                + "\n"
            )
        except KeyError:
    ...
    if len(rrstationlst) <= 1:
        ...
        if rrstation is not None and rrstation != station:
            if stationinfofile is None or stationinfofile == "None":
                pass
            else:
                ...
                edifid.write(
                    lsp
                    + "Remote Reference Lat="
                    + "%2.8g" % float(rrdict["lat"])
                    + "\n"
                )
                edifid.write(
                    lsp
                    + "Remote Reference Long="
            ...
            edifid.write(
                lsp + "Remote Reference Lat=" + "%2.8g" % float(statdict["lat"]) + "\n"
            )
            edifid.write(
                lsp
                + "Remote Reference Long="
    ...
    edifid.write(
        ">EMEAS ID=1003.001 CHTYPE=EX X=0 Y=0 X2=" + statdict["ex"] + " Y2=0" + "\n"
    )
    edifid.write(
        ">EMEAS ID=1004.001 CHTYPE=EY X=0 Y=0 X2=0 Y2=" + statdict["ey"] + "\n"
    )
...
def readini(inifilename):
    ...
    if len(inilines) == 38:
        ...
        raise ValueError(
            "Length of .ini file is not correct (len= %.3g" % len(inilines)
...
def writeini(filepath, argsdict):
    ...
    if len(argsdict) != 38:
        raise ValueError(
            "Length of argsdict is not correct (len= %.3g" % len(argsdict)
            + ") check to make sure all values are entered."
        )
    else:
        ...
        inifid.write(
            "Long period magnetic fields converted to microV/nT="
            + argsdict["lpyn"]
            + "\n"
        )
        inifid.write("Electric fields converted to microV/m=" + argsdict["eyn"] + "\n")
        inifid.write(
            "Measurement components in order for BIRRP=" + argsdict["mcomps"] + "\n"
        )
        ...
        inifid.write(
            "Magnectic measured orientation relative to Cartesian "
            + "Bx,By,Bz="
            + argsdict["magori"]
            + "\n"
        )
        ...
        inifid.write(
            "Residual rejection factor low end (usually 0)=" + argsdict["uin"] + "\n"
        )
        inifid.write(
            "Residual rejection factor high end (.95-.99)=" + argsdict["ainuin"] + "\n"
        )
        inifid.write(
            "Coherence threshold (0 if not desired)=" + argsdict["c2threshe"] + "\n"
        )
        ...
        inifid.write(
            "Prewhitening filter (3< >15) or 0 if not desired=" + argsdict["nar"] + "\n"
        )
        ...
        inifid.write(
            "Rotation angles for electrics (relative to geomagnetic "
            + "North)(N,E,rot)="
            + argsdict["thetae"]
            + "\n"
        )
        inifid.write(
            "Rotation angles for magnetics (relative to geomagnetic "
            + "North)(N,E,rot)="
            + argsdict["thetab"]
            + "\n"
        )
        inifid.write(
            "Rotation angles for calculation (relative to geomagnetic"
            + "North)(N,E,rot)="
            + argsdict["thetaf"]
            + "\n"
        )
