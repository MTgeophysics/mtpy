...
class OccamGui(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        QtCore.QObject.connect(
            self.ui.button_browse_wd,
            QtCore.SIGNAL("clicked()"),
            lambda: self.set_path_in_browsefield(self.ui.lineEdit_browse_wd),
        )
        QtCore.QObject.connect(
            self.ui.button_browse_edis,
            QtCore.SIGNAL("clicked()"),
            lambda: self.set_path_in_browsefield(self.ui.lineEdit_browse_edi),
        )
        QtCore.QObject.connect(
            self.ui.pushButton_loadstations,
            QtCore.SIGNAL("clicked()"),
            lambda: self.set_filename_in_browsefield(self.ui.lineEdit_browse_stations),
        )
        QtCore.QObject.connect(
            self.ui.button_browse_occam,
            QtCore.SIGNAL("clicked()"),
            lambda: self.set_filename_in_browsefield(self.ui.lineEdit_browse_occam),
        )
        ...
        QtCore.QObject.connect(
            self.ui.pushButton_loaddatafile,
            QtCore.SIGNAL("clicked()"),
            lambda: self.set_filename_in_browsefield(self.ui.lineEdit_browse_datafile),
        )
        QtCore.QObject.connect(
            self.ui.pushButton_loaditerationfile,
            QtCore.SIGNAL("clicked()"),
            lambda: self.set_filename_in_browsefield(
                self.ui.lineEdit_browse_iterationfile
            ),
        )

        QtCore.QObject.connect(
            self.ui.button_browse_configfile,
            QtCore.SIGNAL("clicked()"),
            lambda: self.set_filename_in_browsefield(
                self.ui.lineEdit_browse_configfile
            ),
        )
        QtCore.QObject.connect(
            self.ui.button_load_configfile,
            QtCore.SIGNAL("clicked()"),
            self.load_old_configfile,
        )
        ...
        QtCore.QObject.connect(
            self.ui.pushButton_checkparameter,
            QtCore.SIGNAL("clicked()"),
            self.check_input,
        )
        ...
        QtCore.QObject.connect(
            self.ui.pushButton_generateinputfile,
            QtCore.SIGNAL("clicked()"),
        ...
        QtCore.QObject.connect(
            self.ui.pushButton_runoccam, QtCore.SIGNAL("clicked()"), self.run_occam
        )
        QtCore.QObject.connect(
    ...
    def load_old_configfile(self):
        ...
        try:
            ...
            messagetext = ""
            messagetext += (
                "<P><FONT COLOR='#000000'>File name: "
                "{0}  </FONT></P> \n".format(old_cfg_filename)
            )
            messagetext += (
                "<P><b><FONT COLOR='#800000'>Error:  Not a valid "
        ...
        try:
            ...
            messagetext += (
                "<P><b><FONT COLOR='#800000'>Error: File not valid or "
        ...
        messagetext = ""
        messagetext += (
            "<P><FONT COLOR='#000000'>Configuration file: "
            "{0}  </FONT></P> \n".format(old_cfg_filename)
        )
        messagetext += (
            "<P><b><FONT COLOR='#008080'>Read in {0} parameters"
    ...
    def check_input(self):
        ...
        if (edis_text.strip() == "") or (
            not op.isdir(op.abspath(op.realpath(edis_text)))
        ):
        ...
        if (occ_exe_text.strip() == "") or (
            not op.isfile(op.abspath(op.realpath(op.join(self.ui.wd, occ_exe_text))))
        ):
        ...
        if (stations_text.strip() == "") or (
            not op.isfile(op.abspath(op.realpath(op.join(self.ui.wd, stations_text))))
        ):
        ...
        if self.ui.checkBox_useiterationfile.checkState():
            ...
            if (
                (startup_text.strip() == "")
                or (startup_text is None)
                or (not op.isfile(op.realpath(op.join(self.ui.wd, startup_text))))
            ):
        ...
        if (datafile_text.strip() == "") or (
            not op.isfile(op.abspath(op.realpath(op.join(self.ui.wd, datafile_text))))
        ):
        ...
        if invalid_flag != 0:
            ...
            messagetext = (
                "<P><b><FONT COLOR='#800000'>Error: %i parameters are invalid !  </FONT></b></P><br> %s"
                % (invalid_flag, conc_mess)
            )
    ...
    def setup_parameter_dict(self):
        ...
        D["occam_exe"] = op.abspath(
            op.realpath(op.join(self.ui.wd, str(self.ui.lineEdit_browse_occam.text())))
        )

        D["use_iterationfile"] = self.ui.checkBox_useiterationfile.checkState()
        if D["use_iterationfile"]:
            D["iterationfile"] = op.abspath(
                op.realpath(
                    op.join(
                        self.ui.wd, str(self.ui.lineEdit_browse_iterationfile.text())
                    )
                )
            )
        else:
        ...
        if D["use_stationfile"]:
            D["stationlistfile"] = op.abspath(
                op.realpath(
                    op.join(self.ui.wd, str(self.ui.lineEdit_browse_stationfile.text()))
                )
            )
        else:
        ...
        if D["use_olddatafile"]:
            D["olddatafile"] = op.abspath(
                op.realpath(
                    op.join(self.ui.wd, str(self.ui.lineEdit_browse_datafile.text()))
                )
            )
        else:
            D["olddatafile"] = None
    ...
    def build_inputfiles(self):
        ...
        if olddatafile is not None:
            ...
            try:
                ...
                messagetext += (
                    "<P><FONT COLOR='#000000'>Working directory: "
            ...
            try:
                ...
                messagetext += (
                    "<P><b><FONT COLOR='#800000'>Error:  Could not "
            ...
            try:
                ...
                messagetext += (
                    ...
                    "Data file: {0}  </FONT></b></P> \n".format(
                        op.split(setup_object.datafile)[1]
                    )
                )
            except:
                messagetext += (
        ...
        if D["check_useiterationfile"]:
            try:
                ...
                messagetext += (
                    "<P><b><FONT COLOR='#008080'>Using old "
                    "iteration file for startup: {0}  </FONT></b></P> \n".format(
                        setup_object.startupfile
                    )
                )
                ...
                messagetext += (
                    "<P><b><FONT COLOR='#800000'>Error: Could not "
                    "find old iteration file: {0}  </FONT></b></P> \nUsing default 'startup' instead ".format(
                        D["iterationfile"]
                    )
                )
        ...
        try:
            ...
            messagetext += (
                ...
                "Configuration file: {0}  </FONT></b></P> \n".format(
                    op.split(setup_object.configfile)[1]
                )
            )
        except:
            messagetext += (
    ...
    def run_occam(self):
        ...
        class OccamThread(QtCore.QThread):
            ...
            def run(self):
                ...
                subprocess.Popen(
                    [self.exec_file, self.startup, self.outname]
