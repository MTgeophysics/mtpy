...
def winglinkmesh2modelfile(
    ...
    init_modelFH.write(
        '#Initial halfspace model, based on WingLink generated mesh \n')
...
def edis2datafile(winglink_outfile, edilist, sites_file,
...
def generate_edilist(edifolder):
    ...
    lo_edifiles = [
        op.abspath(i) for i in glob.glob(
            op.join(
                edifolder,
                '*.[eE][dD][iI]'))]
...
def winglink2modem(edifolder, winglinkoutput, sites_file,
                   modelfilename='init_model', resistivity=100):
    """
    ...
    if not op.isdir(edifolder):
        sys.exit(
            'cannot find EDI files: no such directory: \n%s' %
            (edifolder))
    ...
    if len(edilist) == 0:
        sys.exit(
            'cannot find EDI files in given directory: \n%s' %
            (edifolder))
    ...
    try:
        ...
        sys.exit(
            'cannot find specified WingLink output file: \n%s' %
            (winglinkoutput))
...
def wsinv2modem_data(wsinv_datafile, sites_file=None):
    ...
    if not int(indata_list[2]) == 8:
        outFH.close()
        sys.exit('cannot handle input file - need 8 components, but %i given!' %
                 int(indata_list[2]))
...
def wsinv2modem_model(wsinv_modelfile, modeltype='halfspace'):
    ...
    if not op.isfile(wsinv_modelfile):
        sys.exit(
            'ERROR - could not find input model file:\n%s' %
            (wsinv_modelfile))
    ...
    lo_blockwidths_north = [int(float(i))
                            for i in lo_blockwidths[6:6 + n_north_blocks]]
    lo_blockwidths_east = [int(float(j)) for j in lo_blockwidths[
        6 + n_north_blocks:6 + n_north_blocks + n_east_blocks]]
    lo_blockwidths_z = [int(float(k)) for k in lo_blockwidths[
        6 + n_north_blocks + n_east_blocks:6 + n_north_blocks + n_east_blocks + n_z_blocks]]
...
def getmeshblockcoordinates(ModEM_modelfile):
    ...
    for idx_z in range(n_depth_blocks - 1):
        current_depth += (depth_blockwidths[idx_z] /
                          2. + depth_blockwidths[idx_z + 1] / 2.)
        lo_depths.append(current_depth)
    ...
    for idx_n in range(n_north_blocks - 1):
        current_north += (north_blockwidths[idx_n] /
                          2. + north_blockwidths[idx_n + 1] / 2.)
        lo_norths.append(current_north)
    ...
    for idx_e in range(n_east_blocks - 1):
        current_east += (east_blockwidths[idx_e] /
                         2. + east_blockwidths[idx_e + 1] / 2.)
        lo_easts.append(current_east)
