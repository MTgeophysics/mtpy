...
def winglinkmesh2modelfile(
    WLoutputfile, modelfilename="ModEM_initmodel", res_value=100
):
    ...
    for idx_depth in range(nz):
        # empty line required, if resistivity values are given instead of
        # resistivity indices
        init_modelFH.write("\n")
...
def latlon2xy(lat, lon, origin):
    ...
    # using existing tool for conversion from lat/lon to northing/easting,
    # then just take planar approximation

    # comment: to be checked for overlapping UTM zones!!

    dummy1, utm_local_east, utm_local_north = mtpy.utils.gis_tools.ll_to_utm(
        23, lat, lon
    )
    dummy2, utm_origin_east, utm_origin_north = mtpy.utils.gis_tools.ll_to_utm(
...
def edis2datafile(
    ...
    edilist,
    sites_file,
    comment="Generic datafile, generated from Python script",
):
    ...
    def closetoexisting(f, l):
        # check, if list l contains an element, which deviates less than 3%
        # from value f
        a_l = np.array(l)
        dev = np.abs(f - a_l).min() / f * 100.0
        if dev < 3:
            return True
        else:
            return False
    ...
    # iterate over general dictionary and write data file lines successively
    # sorted by stations:
    for station in data_dict:
        ...
        # WingLink indices go from North to South, but here we have South as
        # reference
        ...
        for idx_freq, tmp_freq in enumerate(station_frequencies):
            ...
            correct_frequency = lo_frequencies[
                np.abs(tmp_freq - np.array(lo_frequencies)).argmin()
            ]
            period = 1.0 / correct_frequency

            for idx_comp, comp in enumerate(z_components):
                row = int(idx_comp / 2.0)
                column = idx_comp % 2
...
def generate_edilist(edifolder):
    ...
    lo_edifiles = [
        op.abspath(i) for i in glob.glob(op.join(edifolder, "*.[eE][dD][iI]"))
    ]
...
def winglink2modem(
    edifolder, winglinkoutput, sites_file, modelfilename="init_model", resistivity=100
):
    """
    ...
    # set up model file
    modelfn = winglinkmesh2modelfile(
        WL_outfile, modelfilename=modelfilename, res_value=HS_rho_value
    )
...
def wsinv2modem_data(wsinv_datafile, sites_file=None):
    ...
    if not int(indata_list[2]) == 8:
        outFH.close()
        sys.exit(
            "cannot handle input file - need 8 components, but %i given!"
            % int(indata_list[2])
        )
    ...
    try:
        ...
        if not len(SF_data) == n_stations:
            print "ERROR -- sites file contains wrong number of stations (%i instead of %i)!" % (
                len(SF_data),
                n_stations,
            )
            raise
    ...
    # loop for wrting data to output file; main looping over periods rather
    # than stations for simplicity
    for idx_period in range(n_periods):
        current_starting_line_data = idx_first_data_row + (
            idx_period * (n_stations + 1)
        )
        print current_starting_line_data
        current_period = float(
            indata_raw[current_starting_line_data].strip().split(":")[1]
        )

        current_starting_line_error = idx_first_error_row + (
            idx_period * (n_stations + 1)
        )
...
def wsinv2modem_model(wsinv_modelfile, modeltype="halfspace"):
    ...
    lo_blockwidths_north = [
        int(float(i)) for i in lo_blockwidths[6 : 6 + n_north_blocks]
    ]
    lo_blockwidths_east = [
        int(float(j))
        for j in lo_blockwidths[6 + n_north_blocks : 6 + n_north_blocks + n_east_blocks]
    ]
    lo_blockwidths_z = [
        int(float(k))
        for k in lo_blockwidths[
            6
            + n_north_blocks
            + n_east_blocks : 6
            + n_north_blocks
...
def getmeshblockcoordinates(ModEM_modelfile):
    ...
    center_ew = total_width_ew / 2.0

    total_width_ns = np.sum(north_blockwidths)
    center_ns = total_width_ns / 2.0

    total_depth = np.sum(depth_blockwidths)

    # depths
    lo_depths = []
    current_depth = depth_blockwidths[0] / 2.0
    lo_depths.append(current_depth)

    for idx_z in range(n_depth_blocks - 1):
        current_depth += (
            depth_blockwidths[idx_z] / 2.0 + depth_blockwidths[idx_z + 1] / 2.0
        )
        lo_depths.append(current_depth)

    lo_norths = []
    current_north = north_blockwidths[0] / 2.0
    lo_norths.append(current_north)
    for idx_n in range(n_north_blocks - 1):
        current_north += (
            north_blockwidths[idx_n] / 2.0 + north_blockwidths[idx_n + 1] / 2.0
        )
        lo_norths.append(current_north)
    ...
    lo_easts = []
    current_east = east_blockwidths[0] / 2.0
    lo_easts.append(current_east)
    for idx_e in range(n_east_blocks - 1):
        current_east += (
            east_blockwidths[idx_e] / 2.0 + east_blockwidths[idx_e + 1] / 2.0
        )
        lo_easts.append(current_east)
