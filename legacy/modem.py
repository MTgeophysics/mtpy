...
import mtpy.modeling.ModEM        >>> import mtpy.modeling.modem as modem
    ...
    def get_header_string(self, error_type, error_value, rotation_angle):
        ...
        h_str = ",".join(
            [
                "# Created using MTpy calculated {0} error of {1:.0f}%",
    ...
    def _set_station_locations(self, station_obj=None):
        ...
        if self.data_array is None:
            ...
            self.data_array = np.zeros(station_locations.size, dtype=self._dtype)
...
import mtpy.modeling.ModEM                >>> import mtpy.modeling.modem as modem
    ...
    def write_vtk_station_file(
        ...
        pointsToVTK(
            ...
            self.station_locations.elev / 1000.0,
    ...
    def change_data_elevation(self, model_fn, data_fn=None, res_air=1e12):
        """
...
class Model(object):
    ...
    def nodes_east(self, nodes):
        ...
        self.grid_east = np.array(
            [-nodes.sum() / 2 + nodes[0:ii].sum() for ii in range(nodes.size)]
            + [nodes.sum() / 2]
        )
    ...
    def make_mesh(self):
        ...
        if self.pad_method == "extent1":
            ...
            padding_east = mtmesh.get_padding_cells2(
                self.cell_size_east, inner_east[-1], self.ew_ext / 2.0, self.pad_east
            )
            padding_north = mtmesh.get_padding_cells2(
                self.cell_size_north, inner_north[-1], self.ns_ext / 2.0, self.pad_north
            )
        ...
        z_nodes = np.array(
            [np.round(zz, -int(np.floor(np.log10(zz)) - 1)) for zz in log_z]
        )

        # padding cells in the vertical
        z_padding = mtmesh.get_padding_cells(
            z_nodes[-1], self.z_bottom - z_nodes.sum(), self.pad_z, self.pad_stretch_v
        )
        # make the blocks into nodes as oppose to total width
        z_padding = np.array(
            [z_padding[ii + 1] - z_padding[ii] for ii in range(z_padding.size - 1)]
        )
    ...
    def make_mesh_from_center(self, update_data_center=True):
        ...
        east_grid_r = np.arange(
            start=west_r, stop=east_r + self.cell_size_east, step=self.cell_size_east
        )
        ...
        for s_east in sorted(self.station_locations.rel_east):
            try:
                node_index = np.where(
                    abs(s_east - east_grid_r) < shift_station * self.cell_size_east
                )[0][0]
        ...
        north_grid_r = np.arange(
            start=south_r,
            stop=north_r + self.cell_size_north,
            step=self.cell_size_north,
        )
        if self.data_obj.rotation_angle == 0:
            self.data_obj.center_point["north"] -= np.mean(north_grid_r)
            self.station_locations.station_locations["rel_north"] += np.mean(
                north_grid_r
            )
        ...
        for s_north in sorted(self.station_locations.rel_north):
            try:
                node_index = np.where(
                    abs(s_north - north_grid_r) < shift_station * self.cell_size_north
                )[0][0]
        ...
        if (
            ...
            try:
                self.data_obj.center_position = self.data_obj.project_xy(
                    self.data_obj.center_position_EN[0],
    ...
    def read_dem_ascii(
        ...
        new_x, new_y = np.meshgrid(
            np.arange(0, east.size, num_cells),
        ...
        elevation[np.where(elevation == -9999.0)] = elevation[
            np.where(elevation != -9999.0)
        ].min()
    ...
    def interpolate_elevation(
        ...
        grid_east, grid_north = np.broadcast_arrays(
            elev_east[:, None], elev_north[None, :]
        )
    ...
    def make_elevation_model(
        ...
        new_nodes_z = np.append(
            np.repeat(elevation_cell, num_elev_cells), model_nodes_z
        )
    ...
    def add_topography_to_model(
        ...
        e_east, e_north, elevation = self.read_dem_ascii(
            dem_ascii_fn,
            cell_size=cell_size,
        ...
        m_elev = self.interpolate_elevation(
            e_east,
            e_north,
            elevation,
            self.grid_east,
        ...
        mod_elev, elev_nodes_z = self.make_elevation_model(
            m_elev, self.nodes_z, elevation_cell=elev_cell
        )
        ...
        **inputs**
        ...
        if "topography" in self.surface_dict.keys():
            ...
            if surfacename == "topography":
                ...
                top = np.zeros_like(surfacedata) + np.amin(surfacedata) - 1.0
            else:
        ...
        for j in range(len(self.res_model)):
            for i in range(len(self.res_model[j])):
                ...
                if surfacename == "topography":
                    iisea = np.where((gcz <= surfacedata[j, i]) & (gcz > 0.0))[0]
                    self.res_model[j, i, iisea] = 0.3
                print j, i, ii

    def interpolate_elevation2(
        self, surfacefile=None, surface=None, surfacename=None, method="nearest"
    ):
        """
        ...
        **inputs**
        ...
        xs, ys, utm_zone = gis_tools.project_points_ll2utm(
            y,
            x,
        ...
        x0, y0 = [
            np.median(
                self.station_locations.station_locations[dd]
    ...
    def add_topography_to_model2(
        ...
        if self.n_airlayers is None or self.n_airlayers == 0:
            ...
            self.grid_z = np.concatenate(
                [new_airlayers[:-1], self.grid_z + new_airlayers[-1]], axis=0
            )
        ...
        new_res_model = (
            np.ones((self.nodes_north.size, self.nodes_east.size, self.nodes_z.size))
            * self.res_starting_value
        )
...
class Residual:
    ...
    def read_residual_file(self, residual_fn=None):
        ...
        for fieldname in ["rms", "rms_z", "rms_tip"]:
            self.rms_array = recfunctions.append_fields(
                self.rms_array.copy(),
                fieldname,
    ...
    def get_rms(self, residual_fn=None):
        ...
        for stname in self.rms_array["station"]:
            ...
            if np.amax(np.abs(resvals["z"])) > 0:
                ...
                znorm = np.abs(resvals["z"]) / (np.real(resvals["z_err"]) * 2.0 ** 0.5)
                ...
                rms_z_comp[sta_ind] = (
                    ((znorm ** 2.0).sum(axis=0)) / (znorm.shape[0])
                ) ** 0.5
            ...
            if np.amax(np.abs(resvals["tip"])) > 0:
                ...
                # need to divide by sqrt(2) to normalise (code applies same error to real and imag components)
                tipnorm = np.abs(resvals["tip"]) / (
                    np.real(resvals["tip_err"]) * 2.0 ** 0.5
                )
                ...
                rms_tip_comp[sta_ind] = (
                    ((tipnorm ** 2.0).sum(axis=0)) / len(tipnorm)
                ) ** 0.5
                rms_valuelist.append(rms_tip_comp[sta_ind])

            rms_valuelist = np.vstack(rms_valuelist).flatten()

            rms_value = ((rms_valuelist ** 2.0).sum() / rms_valuelist.size) ** 0.5

            self.rms_array[sta_ind]["rms"] = rms_value

            if znorm is not None:
                self.rms_array[sta_ind]["rms_z"] = (
                    (rms_z_comp[sta_ind] ** 2.0).sum() / rms_z_comp[sta_ind].size
                ) ** 0.5
            if tipnorm is not None:
                self.rms_array[sta_ind]["rms_tip"] = (
                    (rms_tip_comp[sta_ind] ** 2.0).sum() / rms_z_comp[sta_ind].size
                ) ** 0.5

        self.rms = np.mean(rms_valuelist_all ** 2.0) ** 0.5
        self.rms_z = np.mean(rms_valuelist_z ** 2.0) ** 0.5
        self.rms_tip = np.mean(rms_valuelist_tip ** 2.0) ** 0.5
            ...
            for ee, nn, elev, name in zip(
                ...
                if elev <= self.grid_z[self.index_vertical]:
                    self.ax_en.text(
                        ...
                        verticalalignment="center",
                        horizontalalignment="center",
                        fontdict={"size": 7, "weight": "bold", "color": (0.75, 0, 0)},
                    )
                    self.ax_en.text(
                        ee,
                        nn,
