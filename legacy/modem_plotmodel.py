...
if __name__ == "__main__":
    ...
    modeldir = op.join(workdir, "ModEM_files")  # folder where *.rho files exist
    ...
    if plotdir == "ew":
        ...
        X, Y, resf, resi = (
            moo.grid_north,
            moo.grid_z,
            np.log10(moo.res_model[:, snons, :].T),
        )
        xlim = (-162500, 162500)
        ylim = (1e4, 0)
        sliceinfo = ""
    ...
    if plotdir == "z":
        ...
        for ss in range(len(doo.station_locations)):
            ...
            plt.text(
                doo.station_locations["rel_east"][ss],
                doo.station_locations["rel_north"][ss],
                doo.station_locations["station"][ss],
                fontsize=8,
            )
    # plt.clim(0.3,3.7)
    plt.colorbar()
