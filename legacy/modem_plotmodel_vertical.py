...
gceast, gcnorth = [
    np.mean([arr[:-1], arr[1:]], axis=0) for arr in [moo.grid_east, moo.grid_north]
]
...
if plotdir == "ew":
    ...
    ss = np.where(
        np.abs(doo.station_locations["rel_north"] - np.median(gcnorth)) < stationdist
    )[0]
    ...
    ss = np.where(
        np.abs(doo.station_locations["rel_east"] - np.median(gceast)) < stationdist
    )[0]
