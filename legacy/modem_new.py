...
import mtpy.modeling.ModEM
    ...
    def _set_header_string(self):
        ...
        if self.error_type == "egbert":
            self.header_strings[0] = h_str.format(
                self.error_type, self.error_egbert, self._rotation_angle
            )
            ...
            self.header_strings[0] = h_str.format(
                self.error_type, self.error_value, self._rotation_angle
            )
    ...
    def get_mt_dict(self):
        ...
        if self.edi_list is None:
            raise ModEMError(
                "edi_list is None, please input a list of "
    ...
    def get_relative_station_locations(self):
        ...
        for c_arr in self.data_array:
            if c_arr["lat"] != 0.0 and c_arr["lon"] != 0.0:
                (
                    ...
                    c_arr["east"],
                    c_arr["north"],
                ) = mtpy.utils.gis_tools.ll_to_utm(
                    self._utm_ellipsoid, c_arr["lat"], c_arr["lon"]
                )
        ...
        if len(utm_zone_list) != 1:
            ...
            utm_zone_dict = dict(
                [(utm_zone_dict[key], key) for key in utm_zone_dict.keys()]
            )
            ...
            for c_index in diff_zones:
                ...
                zone_shift = 1 - abs(
                    utm_zones_dict[c_utm_zone[-1]] - utm_zones_dict[main_utm_zone[-1]]
                )
                ...
                if zone_shift > 1:
                    ...
                    print (
                        "--> adding {0:.2f}".format(north_shift)
                ...
                # if odd ball zone is south of main zone, subtract 88960 m
                    ...
                    north_shift = self._utm_grid_size_north * zone_shift
                    print (
                        "--> subtracting {0:.2f}".format(north_shift)
                        + " meters N to place station in "
                        + "proper coordinates relative to all other "
                        + "staions."
                    )
                ...
                if int(c_utm_zone[0:-1]) > int(main_utm_zone[0:-1]):
                    east_shift = self._utm_grid_size_east * abs(
                        int(c_utm_zone[0:-1]) - int(main_utm_zone[0:-1])
                    )
                    print (
                        "--> adding {0:.2f}".format(east_shift)
                    ...
                    print (
                        "--> subtracting {0:.2f}".format(east_shift)
                        + " meters E to place station in "
                        + "proper coordinates relative to all other "
                        + "staions."
                    )
        ...
        self.data_array["rel_east"] = (
            self.data_array["east"] - self.data_array["east"].mean()
        )
        self.data_array["rel_north"] = (
            self.data_array["north"] - self.data_array["north"].mean()
        )
        ...
        if self.rotation_angle != 0:
            ...
            coords = np.array(
                [self.data_array["rel_east"], self.data_array["rel_north"]]
            )
        ...
        east_center = (
            self.data_array["rel_east"].max()
            - np.abs(self.data_array["rel_east"].min())
        ) / 2
        north_center = (
            self.data_array["rel_north"].max()
            - np.abs(self.data_array["rel_north"].min())
        ) / 2
    ...
    def get_period_list(self):
        ...
        self.data_period_list = np.array(
        ...
        for ii, s_key in enumerate(sorted(self.mt_dict.keys())):
            ...
            for kk, ff in enumerate(interp_periods):
                ...
                if mt_obj.Tipper.tipper is not None:
                    ...
                    self.data_array[ii]["tip_err"][jj] = interp_t.tipper_err[kk, :, :]
    ...
    def _set_station_locations(self, station_locations):
        ...
        for s_arr in station_locations:
            try:
                d_index = np.where(self.data_array["station"] == s_arr["station"])[0][0]
            except IndexError:
    ...
    def _get_station_locations(self):
        ...
        station_locations = self.data_array[
            ["station", "lat", "lon", "north", "east", "elev", "rel_north", "rel_east"]
        ]
...
import mtpy.modeling.ModEM            >>> md = mtpy.modeling.ModEM.Data(edi_list, period_min=.1, period_max=300,\
        ...
        nper = len(
            np.where(
                np.mean(
                    np.mean(np.mean(np.abs(self.data_array["z"]), axis=0), axis=1),
                    axis=1,
                )
                > 0
            )[0]
        ...
        for inv_mode in self.inv_mode_dict[self.inv_mode]:
            ...
            dlines.append(
                "> {0} {1}\n".format(
                    self.data_array["z"].shape[1], self.data_array["z"].shape[0]
                )
            )
                        ...
                        if (
                            ...
                            if compute_error:
                                ...
                                if comp.find("t") == 0:
                                    ...
                                    if self.error_type == "floor":
                                        rel_err = self.data_array[ss][c_key + "_err"][
                                            ff, z_ii, z_jj
                                        ] / abs(zz)
                                        if rel_err < self.error_floor / 100.0:
                                            rel_err = self.error_floor / 100.0
                                        abs_err = rel_err * abs(zz)
                                    elif self.error_type == "value":
                                        abs_err = abs(zz) * self.error_value / 100.0
                                        ...
                                        abs_err = (
                                            np.sqrt(abs(d_zxy * d_zyx))
                                            * self.error_egbert
                                            / 100.0
                                        )
                                    elif self.error_type == "floor_egbert":
                                        abs_err = self.data_array[ss][c_key + "_err"][
                                            ff, z_ii, z_jj
                                        ]
                                        d_zxy = self.data_array[ss]["z"][ff, 0, 1]
                                        d_zyx = self.data_array[ss]["z"][ff, 1, 0]
                                ...
                                if abs_err == 0.0:
                                    abs_err = 1e3
                                    print (
                                        """error at {0} is 0 for period {1} \n
                                            for {2}({3}, {4}) set to 1e3\n
                                            data = {5:.4e}+j{6:.4e}""".format(
                                            sta, per, comp, z_ii, z_jj, zz.real, zz.imag
                                        )
                                    )
                                    if self.units == "ohm":
                                        abs_err /= 796.0
        ...
        for dline in dlines:
            if dline.find("#") == 0:
                ...
                if dline.find("exp") > 0:
                    ...
                    value_list = [float(value) for value in dline[1:].strip().split()]
...
import mtpy.modeling.ModEM
    ...
    def get_station_locations(self):
        ...
        east_center = (
            ...
            - np.abs(self.station_locations["rel_east"].min())
        ) / 2
        north_center = (
            self.station_locations["rel_north"].max()
            - np.abs(self.station_locations["rel_north"].min())
        ) / 2
    ...
    def make_mesh(self, update_data_center=False):
        ...
        for s_east in sorted(self.station_locations["rel_east"]):
            try:
                node_index = np.where(
                    abs(s_east - east_gridr) < 0.02 * self.cell_size_east
                )[0][0]
                if s_east - east_gridr[node_index] > 0:
                    east_gridr[node_index] -= 0.02 * self.cell_size_east
        ...
        for s_north in sorted(self.station_locations["rel_north"]):
            try:
                node_index = np.where(
                    abs(s_north - north_gridr) < 0.02 * self.cell_size_north
                )[0][0]
                if s_north - north_gridr[node_index] > 0:
                    north_gridr[node_index] -= 0.02 * self.cell_size_north
        ...
        z_nodes = np.array([zz - zz % 10 ** np.floor(np.log10(zz)) for zz in log_z])
        # padding cells in the east-west direction
        ...
        print "  Stations rotated by: {0:.1f} deg clockwise positive from N".format(
            self.mesh_rotation_angle
        )
        print ""
    ...
    def plot_mesh(self, east_limits=None, north_limits=None, z_limits=None, **kwargs):
        ...
        if east_limits == None:
            ax1.set_xlim(
                plot_east.min() - 10 * self.cell_size_east,
        ...
        if north_limits == None:
            ax1.set_ylim(
                plot_north.min() - 10 * self.cell_size_north,
    ...
    def read_model_file(self, model_fn=None):
        ...
        self.grid_z = np.array([self.nodes_z[: ii + 1].sum() for ii in range(n_z + 1)])
...
def add_topography_to_model(
    dem_ascii_fn, model_fn, model_center=(0, 0), rot_90=0, cell_size=500, elev_cell=30
):
    """
    ...
    m_obj.write_model_file(
        model_fn_basename="{0}_topo.rho".format(os.path.basename(m_obj.model_fn)[0:-4])
    )
...
def change_data_elevation(data_fn, model_fn, new_data_fn=None, res_air=1e12):
    ...
    for key in d_obj.mt_dict.keys():
        ...
        z_index = np.where(m_obj.res_model[n_index, e_index, :] < res_air * 0.9)[0][0]
...
class ModelManipulator(Model):
    ...
    def __init__(self, model_fn=None, data_fn=None, **kwargs):
        ...
        self.subplot_wspace = kwargs.pop("subplot_wspace", 0.3)
        self.subplot_hspace = kwargs.pop("subplot_hspace", 0.0)
        self.subplot_right = kwargs.pop("subplot_right", 0.8)
        self.subplot_left = kwargs.pop("subplot_left", 0.01)
        self.subplot_top = kwargs.pop("subplot_top", 0.93)
        self.subplot_bottom = kwargs.pop("subplot_bottom", 0.1)
    ...
    def plot(self):
        ...
        self.ax2 = self.fig.add_axes([0.81, 0.45, 0.16, 0.03])
        ...
        slider_ax = self.fig.add_axes([0.81, 0.5, 0.16, 0.03])
        self.slider_res = widgets.Slider(
            slider_ax, "Resistivity", self.cmin, self.cmax, valinit=2
        )
    ...
    def _on_key_callback(self, event):
        ...
        if self.event_change_depth.key == "=":
            ...
            print "Plotting Depth {0:.3f}".format(
...
class moved_PlotResponse(object):
    ...
    def __init__(self, data_fn=None, resp_fn=None, **kwargs):
        ...
        self.subplot_top = kwargs.pop("subplot_top", 0.85)
        self.subplot_bottom = kwargs.pop("subplot_bottom", 0.1)

        self.legend_loc = "upper center"
        self.legend_pos = (0.5, 1.18)
...
class PlotSlices(object):
    ...
    def plot(self):
        ...
        if self.z_limits == None:
            depth_limit = max(
                [
                    (abs(self.ew_limits[0]) + abs(self.ew_limits[1])),
        ...
        # plot color bar
        cbx = mcb.make_axes(self.ax_map, fraction=0.15, shrink=0.75, pad=0.15)
        cb = mcb.ColorbarBase(
            cbx[0],
            cmap=self.cmap,
            norm=Normalize(vmin=self.climits[0], vmax=self.climits[1]),
    ...
    def _update_map(self):
        ...
        # --> e-w indication line
        self.ax_map.plot(
            [self.grid_east[self.index_east + 1], self.grid_east[self.index_east + 1]],
            [self.grid_north.min(), self.grid_north.max()],
            lw=1,
            color="b",
        )
...
class moved_Plot_RMS_Maps(object):
    ...
    def __init__(self, residual_fn, **kwargs):
        ...
        self.subplot_left = kwargs.pop("subplot_left", 0.1)
        self.subplot_right = kwargs.pop("subplot_right", 0.9)
        self.subplot_top = kwargs.pop("subplot_top", 0.95)
        self.subplot_bottom = kwargs.pop("subplot_bottom", 0.1)
        self.subplot_hspace = kwargs.pop("subplot_hspace", 0.1)
        self.subplot_vspace = kwargs.pop("subplot_vspace", 0.01)
        ...
        self.plot_z_list = [
            {"label": r"$Z_{xx}$", "index": (0, 0), "plot_num": 1},
    ...
    def plot(self):
        ...
        if self.tick_locator is None:
            x_locator = np.round(
                (
                    self.residual.data_array["lon"].max()
                    - self.residual.data_array["lon"].min()
                )
                / 5,
                2,
            )
            y_locator = np.round(
                (
                    self.residual.data_array["lat"].max()
                    - self.residual.data_array["lat"].min()
                )
                / 5,
                2,
            )
        ...
        for p_dict in self.plot_z_list:
            ...
            ax.text(
                self.residual.data_array["lon"].min() + 0.005 - self.pad_x,
                self.residual.data_array["lat"].max() - 0.005 + self.pad_y,
                p_dict["label"],
                verticalalignment="top",
                horizontalalignment="left",
                bbox={"facecolor": "white"},
            ...
            ax.set_xlim(
                self.residual.data_array["lon"].min() - self.pad_x,
                self.residual.data_array["lon"].max() + self.pad_x,
            )

            ax.set_ylim(
                self.residual.data_array["lat"].min() - self.pad_y,
        ...
        self.fig.suptitle(
            "period = {0:.5g} (s)".format(self.residual.period_list[self.period_index]),
