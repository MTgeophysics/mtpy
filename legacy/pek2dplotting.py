    ...
    def plot(self):
        ...
        for i in range(nsp):
            if (2 * i) % spc == 0:
                row += 1
            spn = 2 * i + 1 + spc * row
            for parameter, meas, mod in [
                ["resistivity", self.resistivity_meas[i], self.resistivity_mod[i]],
                ...
                for ii in range(2):
                    for jj in range(2):
                        ...
                        self.__set_axis_params(parameter, component=cpt, labels=labels)
