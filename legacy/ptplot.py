...
class ptplots(Z):
    ...
    def plotPhaseTensor(
        ...
        xticklabels = [
            "%2.2g" % periodlst[ii] for ii in np.arange(start=0, stop=n, step=3)
        ]
        ...
        if save == "y":
            ...
            fig.savefig(
                os.path.join(self.savepath, self.z.station + "PT." + fmt), fmt=fmt
            )
            print "Saved figure to: " + os.path.join(
                self.savepath, self.z.station + "PT." + fmt
            )
        elif len(save) > 1:
            fig.savefig(os.path.join(save, self.z.station + "PT." + fmt), fmt=fmt)
            print "Saved figure to: " + os.path.join(save, self.z.station + "PT." + fmt)
        elif save == "n":
            pass
    ...
    def plotStrikeangle(self, save="n", fmt="pdf", fignum=1, thetar=0):
        ...
        if save == "y":
            if not os.path.exists(self.savepath):
                ...
                print "Made Directory: " + self.savepath
            fig.savefig(
                os.path.join(self.savepath, self.z.station + "Strike." + fmt), fmt=fmt
            )
            print "Saved figure to: " + os.path.join(
                self.savepath, self.z.station + "Strike." + fmt
            )
        elif len(save) > 1:
            fig.savefig(os.path.join(save, self.z.station + "Strike." + fmt), fmt=fmt)
            print "Saved figure to: " + os.path.join(
                save, self.z.station + "Strike." + fmt
            )
        elif save == "n":
            pass
    ...
    def plotMinMaxPhase(self, save="n", fmt="pdf", fignum=1, thetar=0):
        ...
        plt.title("Min and Max Phase for " + stationstr, fontsize=12, fontweight="bold")
        plt.show()
        ...
        if save == "y":
            if not os.path.exists(self.savepath):
                ...
                print "Made Directory: " + self.savepath
            fig.savefig(
                os.path.join(self.savepath, self.z.station + "MMPhase." + fmt), fmt=fmt
            )
            print "Saved figure to: " + os.path.join(
                self.savepath, self.z.station + "MMPhase." + fmt
            )
        elif len(save) > 1:
            fig.savefig(os.path.join(save, self.z.station + "MMPhase." + fmt), fmt=fmt)
            print "Saved figure to: " + os.path.join(
                save, self.z.station + "MMPhase." + fmt
            )
        elif save == "n":
            pass
    ...
    def plotAzimuth(self, save="n", fmt="pdf", fignum=1, thetar=0, dpi=300):
        ...
        plt.grid(alpha=0.25)
        plt.xlabel("Period (s)", fontsize=10, fontweight="bold")
        plt.ylabel("Azimuth (deg. clockwise from N)", fontsize=10, fontweight="bold")
        plt.title(
            "Azimuth of Max Phase for " + stationstr, fontsize=10, fontweight="bold"
        )
        plt.show()
        ...
        if save == "y":
            if not os.path.exists(self.savepath):
                ...
                print "Made Directory: " + self.savepath
            fig.savefig(
                os.path.join(self.savepath, self.z.station + "Az." + fmt), fmt=fmt
            )
            print "Saved figure to: " + os.path.join(
                self.savepath, self.z.station + "Az." + fmt
            )
        elif len(save) > 1:
            fig.savefig(os.path.join(save, self.z.station + "Az." + fmt), fmt=fmt)
            print "Saved figure to: " + os.path.join(save, self.z.station + "Az." + fmt)
        elif save == "n":
            pass
    ...
    def plotSkew(self, save="n", fmt="pdf", fignum=1, thetar=0):
        ...
        if save == "y":
            if not os.path.exists(self.savepath):
                ...
                print "Made Directory: " + self.savepath
            fig.savefig(
                os.path.join(self.savepath, self.z.station + "Skew." + fmt), fmt=fmt
            )
            print "Saved figure to: " + os.path.join(
                self.savepath, self.z.station + "Skew." + fmt
            )
        elif len(save) > 1:
            fig.savefig(os.path.join(save, self.z.station + "Skew." + fmt), fmt=fmt)
            print "Saved figure to: " + os.path.join(
                save, self.z.station + "Skew." + fmt
            )
        elif save == "n":
            pass
    ...
    def plotEllipticity(self, save="n", fmt="pdf", fignum=1, thetar=0):
        ...
        plt.title("Ellipticity for " + stationstr, fontsize=12, fontweight="bold")
        plt.show()
        ...
        if save == "y":
            if not os.path.exists(self.savepath):
                ...
                print "Made Directory: " + self.savepath
            fig.savefig(
                os.path.join(self.savepath, self.z.station + "Ellip." + fmt), fmt=fmt
            )
            print "Saved figure to: " + os.path.join(
                self.savepath, self.z.station + "Ellip." + fmt
            )
        elif len(save) > 1:
            fig.savefig(os.path.join(save, self.z.station + "Ellip." + fmt), fmt=fmt)
            print "Saved figure to: " + os.path.join(
                save, self.z.station + "Ellip." + fmt
            )
        elif save == "n":
            pass

    def plotAll(self, xspacing=6, esize=5, save="n", fmt="pdf", fignum=1, thetar=0):
        ...
        for dd in range(len(self.z)):
            ...
            xticklabels = [
                "%2.2g" % period[ii] for ii in np.arange(start=0, stop=n, step=3)
            ]
            plt.xlabel("Period (s)", fontsize=8, fontweight="bold")
            # plt.title('Phase Tensor Ellipses for '+stationstr,fontsize=14)
            plt.xticks(
                np.arange(start=0, stop=xspacing * n, step=3 * xspacing), xticklabels
            )
            ax1.set_ylim(-1 * (xspacing + 3), xspacing + 3)
            ax1.set_xlim(-xspacing, n * xspacing + 3)
            plt.grid()
            if dd == 0:
                ax1cb = make_axes(ax1, shrink=0.3, orientation="horizontal", pad=0.30)
                cb = ColorbarBase(
                    ax1cb[0],
                    cmap=ptcmap,
                    norm=Normalize(vmin=min(pt.phiminang), vmax=max(pt.phiminang)),
            ...
            realarrowvar = np.zeros(len(realarrow)) + 0.00000001

            ax2 = plt.subplot(3, 2, 3)
            erxy = plt.errorbar(
                period,
                zinv.strike,
                marker=self.pcmlst[dd][0],
                ms=4,
                mfc="None",
                mec=self.pcmlst[dd][1],
                mew=1,
                ls="None",
                yerr=zinv.strikeerr,
            ...
            eraz = plt.errorbar(
                period,
                az,
                marker=self.pcmlst[dd + 1][0],
                ms=4,
                mfc="None",
                mec=self.pcmlst[dd + 1][1],
                mew=1,
                ls="None",
                yerr=azvar,
            ...
            plt.title("Strike Angle, Azimuth", fontsize=tfs, fontweight="bold")
        ...
        plt.suptitle(
            "Phase Tensor Elements for: " + stationstr, fontsize=12, fontweight="bold"
        )
        ...
        if save == "y":
            if not os.path.exists(self.savepath):
                ...
                print "Made Directory: " + self.savepath
            fig.savefig(
                os.path.join(self.savepath, self.z[0].station + "All." + fmt), fmt=fmt
            )
            print "Saved figure to: " + os.path.join(
            ...
            fig.savefig(os.path.join(save, self.z[0].station + "All." + fmt), fmt=fmt)
            print "Saved figure to: " + os.path.join(
                save, self.z[0].station + "All." + fmt
            )
            plt.close()
        elif save == "n":
            pass
    ...
    def plotResPhase(self, ffactor=1, save="n", fmt="pdf", fignum=1, thetar=0):
        ...
        plt.xlabel("Period (s)", fontsize=10, fontweight="bold")
        plt.ylabel(
            "Addarent Resistivity ($\mathbf{\Omega \cdot m}$)",
            fontsize=10,
            fontweight="bold",
        )
        ax.set_yscale("log")
        ax.set_xscale("log")
        plt.xlim(
            xmin=10 ** (np.ceil(np.log10(period[0]))),
        ...
        if save == "y":
            if not os.path.exists(self.savepath):
                ...
                print "Made Directory: " + self.savepath
            fig.savefig(
                os.path.join(self.savepath, self.z.station + "Resxy." + fmt), fmt=fmt
            )
            print "Saved figure to: " + os.path.join(
                self.savepath, self.z.station + "Resxy." + fmt
            )
        elif len(save) > 1:
            fig.savefig(os.path.join(save, self.z.station + "Resxy." + fmt), fmt=fmt)
            print "Saved figure to: " + os.path.join(
                save, self.z.station + "Resxy." + fmt
            )
        elif save == "n":
            pass
        ...
        if save == "y":
            if not os.path.exists(self.savepath):
                ...
                print "Made Directory: " + self.savepath
            fig2.savefig(
                os.path.join(self.savepath, self.z.station + "Resxx." + fmt), fmt=fmt
            )
            print "Saved figure to: " + os.path.join(
                self.savepath, self.z.station + "Resxx." + fmt
            )
        elif len(save) > 1:
            fig2.savefig(os.path.join(save, self.z.station + "Resxx." + fmt), fmt=fmt)
            print "Saved figure to: " + os.path.join(
                save, self.z.station + "Resxx." + fmt
            )
        elif save == "n":
            pass
