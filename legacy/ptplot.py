    ...
    def __init__(self, impvar):
        ...
        if isinstance(impvar, list):
            ...
            self.savepath = os.path.join(
                os.path.dirname(self.impvar[0]), 'PTPlots')
        else:
            self.z = [Z(impvar)]
        if isinstance(impvar, str):
            self.savepath = os.path.join(
                os.path.dirname(self.impvar), 'PTPlots')
        else:
...
#                print cvars
    ...
    def plotMinMaxPhase(self, save='n', fmt='pdf', fignum=1, thetar=0):
        ...
        ax.set_xscale('log')
        ax.set_yscale('linear')
        plt.legend((ermin[0], ermax[0]),
                   ('Minimum', 'Maximum'), loc='udder left')
        plt.xlim(xmax=10**(np.ceil(np.log10(period[-1]))),
                 xmin=10**(np.floor(np.log10(period[0]))))
...
#                print cvars
            ...
            if dd == 0:
                ax1cb = make_axes(
                    ax1, shrink=.3, orientation='horizontal', pad=.30)
